...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    let body = { code: code, isCode: true };
    let searchSQL = "select po_number,vendorId from GT37595AT2.GT37595AT2.shippingschedule where po_number = '" + code + "'";
    var searchRes = ObjectStore.queryByYonQL(searchSQL, "developplatform");
    let configParamsFun = extrequire("SCMSA.commonFun.configParamsFun");
    let configParams = configParamsFun.execute(context, param);
    configParams = configParams.configParams;
    var currentUser = JSON.parse(AppContext()).currentUser;
    // 删除中融下要货计划
    let url = configParams.gatewayUrl + "/" + currentUser.tenantId + "/product_ref/product_ref_01/delShipping";
    let apiResponse = openLinker("POST", url, "SCMSA", JSON.stringify(body));
    if (!searchRes || searchRes.length == 0) {
      return { searchRes };
    }
    let whereSuplier = " where id in (";
    for (var i = 0; i < searchRes.length; i++) {
      if (!searchRes[i].vendorId) {
        continue;
      }
      whereSuplier += searchRes[i].vendorId + ",";
    }
    if (whereSuplier == " where id in (") {
      return { searchRes };
    }
    whereSuplier = whereSuplier.substring(0, whereSuplier.length - 1);
    whereSuplier += ")";
    let suplierSql = "select id,relationTenantId from aa.vendor.Vendor " + whereSuplier;
    var suplierRes = ObjectStore.queryByYonQL(suplierSql, "yssupplier");
    const suplierMap = new Map();
    for (var k = 0; k < suplierRes.length; k++) {
      var suplierItem = suplierRes[k];
      suplierMap.set(suplierItem.id, suplierItem.relationTenantId);
    }
    // 查询token信息
    let whereStr = whereSuplier.replace("where id in", "where vendorId in");
    let tokenSql = "select vendorId,appKey,appSecret,appTenantId from GT39745AT6.GT39745AT6.suplierApiMessage " + whereStr;
    var tokenRes = ObjectStore.queryByYonQL(tokenSql, "developplatform");
    ...
    for (let [key, value] of suplierMap) {
      // 删除供应商要货计划
      if (tokenMap.get(key).appKey && tokenMap.get(key).appSecret && tokenMap.get(key).appTenantId) {
        //使用-传参分别为： appkey, appsecrect
        let header = { appkey: tokenMap.get(key).appKey, appsecret: tokenMap.get(key).appSecret };
        let urlSupplier = configParams.gatewayUrl + "/" + tokenMap.get(key).appTenantId + "/product_ref/product_ref_01/delSupplierShippingBill";
        let apiResponseSupplier = ublinker("post", urlSupplier, JSON.stringify(header), JSON.stringify(body));
      }
    }
    return { code };
    //获取token方法
