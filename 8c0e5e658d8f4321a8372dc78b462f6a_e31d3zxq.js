viewModel.on("customInit", function (data) {
          ...
          promises.push(
            getQualifiedVendor(vendorArrays[i].id).then((vendorWtsInfo) => {
              for (let j = 0; j < vendorWtsInfo.length; j++) {
                if (typeof vendorWtsInfo[j].extend_salesman_name != "undefined") {
                  objs.push({ code: vendorArrays[i].code, name: vendorArrays[i].name, wts_name: vendorWtsInfo[j].extend_salesman_name });
                }
              }
            })
          );
  ...
  let prev = viewModel.get("button5mj");
  ...
  //下一页
  let next = viewModel.get("button11ke");
