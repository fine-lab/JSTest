      ...
      for (var i = mainArr.length - 1; i >= 0; i--) {
        ...
        if (mergeArr.length == 0) {
          ...
          for (var s = mergeArr.length - 1; s >= 0; s--) {
            let x = mergeArr[s];
            if (d.skuCode == x.skuCode && d.product_code == x.product_code && d.batchno == x.batchno && d.A925 == x.A925 && d.sellType == x.sellType) {
              ...
              x.A2315 = String(parseFloat(x.A2315) + parseFloat(d.A2315)); // 行折扣额
              mergeArr[s] = x;
              subscript.push(i);
              break;
            } else if (0 == s) {
