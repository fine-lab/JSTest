...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    let deletedAttrFeatureObj = function (obj) {
      Reflect.deleteProperty(obj, "characteristicsId");
      Reflect.deleteProperty(obj, "ytenant");
      Reflect.deleteProperty(obj, "id");
      Reflect.deleteProperty(obj, "pubts");
      Reflect.deleteProperty(obj, "dr");
      return obj;
    };
    let update = function (billInfo) {
      let selectCustomerCode = "select code from 	aa.merchant.Merchant where id = '" + billInfo.customer + "'";
      let yonqlSelectedCustomerCode = ObjectStore.queryByYonQL(selectCustomerCode, "productcenter")[0].code;
      let apiResponse = extrequire("GT22176AT10.publicFunction.queryMerchantInfor").execute({ code: yonqlSelectedCustomerCode, orgId: billInfo.org_id });
    ...
    //防止重复生成
    let validateIsInsertSql = "select id,firstMarketingBillno from GT22176AT10.GT22176AT10.sy01_customers_file where firstMarketingBillno = '" + billInfo.code + "'";
    let validateIsInsertRes = ObjectStore.queryByYonQL(validateIsInsertSql, "sy01");
    if (validateIsInsertRes.length > 0) {
      return;
    }
      ...
      for (let i = 0; i < extend_lincenseList.length; i++) {
        ...
        if (typeof get_license_sub_res != "undefined" && get_license_sub_res != null) {
          for (let j = 0; j < get_license_sub_res.length; j++) {
            ...
            if (get_license_sub_res[j].feature != undefined && JSON.stringify(get_license_sub_res[j].feature) != "{}") {
              licenseRangeInfo.feature = deletedAttrFeatureObj(get_license_sub_res[j].feature);
            }
            ...
            licenseRangeInfo.treatmentrange = get_license_sub_res[j].treatmentrange;
      ...
      for (let i = 0; i < authInfos.length; i++) {
        ...
        if (get_auth_sub_res != undefined && get_auth_sub_res != null) {
          for (let j = 0; j < get_auth_sub_res.length; j++) {
            ...
            if (get_auth_sub_res[j].feature != undefined && JSON.stringify(get_auth_sub_res[j].feature) != "{}") {
              authRangeInfo.feature = deletedAttrFeatureObj(get_auth_sub_res[j].feature);
            }
            ...
            authRangeInfo.treatmentrange = get_auth_sub_res[j].treatmentrange;
    ...
    if (sy_report != undefined && sy_report != null) {
      for (let i = 0; i < sy_report.length; i++) {
        ...
        info.reportCode = sy_report[i].reportCode; //注意这里是证件号，不是编码
    ...
    let updateJson = {
      ...
      //经营范围
      marketingRange: billInfo.managerange,
      //法人代表
      corporateRepresentative: billInfo.corporateRepresentative,
      //办公地址
      businessAddress: billInfo.businessAddress,
      //仓库地址
      warehouseAddress: billInfo.warehouseAddress,
      //企业类型
      enterpriseType: billInfo.enterpriseType,
      //联系电话
      contactNumber: billInfo.contactNumber,
      //传真号码
      faxNumber: billInfo.faxNumber,
      //企业营业执照注册号
      businessLicenseNumber: billInfo.businessLicenseNumber,
      //营业执照有效期
      businessLicenseValidity: billInfo.businessLicenseValidity,
      //医疗机构执业许可证号
      practiceLicenseNumber: billInfo.practiceLicenseNumber,
      //收货员姓名
      consigneeName: billInfo.consigneeName,
      //收货人员身份证号
      consigneeIdNumber: billInfo.consigneeIdNumber,
      //收货授权书有效期
      receiptAuthValidity: billInfo.receiptAuthValidity,
      //质量负责人
      qualityPerson: billInfo.qualityperson,
      //销售状态
      saleState: "1",
      //企业负责人
      chargeperson: billInfo.chargeperson,
      //住址
      residence: billInfo.residence,
      //经营场所
      businessPlace: billInfo.businessPlace,
      //有无纸质材料
      paperMaterials: getSwitchValue(billInfo.paperMaterials)
    };
