...
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValueEnumerate2gb() {}
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValueEnumerate2gb: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_formStore_Enumerate2gb: reaction(
      () => [rootStore.formStore.getValue("Enumerate2g")],
      async ([formStore_Enumerate2g]) => {
        debounce(
          async () => {
            // 获取当前页面所有数据
            ...
            // 公式结束loading
            rootStore.formStore.setValue("Enumerate2g", resultData);
          },
          "formula_customSetValue_formStore_Enumerate2gb" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
