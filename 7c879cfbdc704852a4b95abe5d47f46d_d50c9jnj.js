  ...
  var MyExternal = {
    resolveExcelData(params) {
      ...
      if (excelData && excelData[0].length > 0) {
        ...
        let quantitiy = viewModel.get("suppliershippingschedulebList").getData()[focusedRowIndex].bill_quantitiy;
        ...
        cb.rest.invokeFunction("GT39325AT4.backDesignerFunction.getSnCount", searchParam, function (err, res) {
          if (res) {
            if (!quantitiy || quantitiy.length <= 0) {
              cb.utils.alert("上传失败！原因:请在编辑页面输入发货数量！", "error");
              return;
            }
            if (res.snCount[0].snCount + excelData[0].length != quantitiy) {
              cb.utils.alert("上传失败！原因:要货计划子行的【发货数量】与SN行数不相等！", "error");
              return;
            }
          }
