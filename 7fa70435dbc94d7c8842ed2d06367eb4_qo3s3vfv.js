...
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValueReferInput10mb_refer() {}
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValueReferInput10mb_refer: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_formStore_ReferInput10mb_refer: reaction(
      () => [rootStore.formStore.getValue("ReferInput4if_Enumerate3r")],
      async ([formStore_ReferInput4if_Enumerate3r]) => {
        debounce(
          async () => {
            const allData = getAllDataByCardStore({
              cardStoreAlias: "cardStore",
              storeControlType: "FormStore",
              storeField: "ReferInput10mb",
              storeForm: "",
            ...
            const formulaResult = await countFormula(
              {
                hasFormula: "true",
                formula: "ReferInput4if.Enumerate3r",
                formulaFuns: "",
                formulaTriggers: "ReferInput4if.Enumerate3r"
              },
              {
                mainDataSourceAlias: "mainEntity",
                field: "ReferInput10mb",
                billnum: "bfsyryzzb001",
            ...
            rootStore.formStore.setValue("ReferInput10mb", resultData);
          },
          "formula_customSetValue_formStore_ReferInput10mb_refer" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
