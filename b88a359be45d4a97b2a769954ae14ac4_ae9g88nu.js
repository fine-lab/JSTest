...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    if (isSourceOrder == 1) {
      //有源单校验UDI
      //查询UDI数据中心是否有数据
      let UDIFileInfo = ObjectStore.selectByMap("GT22176AT10.GT22176AT10.UDIFile", { UDI: udi });
      ...
      if (orderInfo.length > 0) {
        ...
        for (let i = 0; i < orderInfo.length; i++) {
          if (orderInfo[i].batchno == UDIFileInfo[0].batchNo && orderInfo[i].invaliddate == UDIFileInfo[0].validateDate && orderInfo[i].producedate == UDIFileInfo[0].produceDate) {
            isQualified = false;
            break;
          }
        }
        ...
        if (orderInfo.batchno != UDIFileInfo[0].batchNo) {
          throw new Error("UDI批次号信息与单据不一致,无法绑定！");
        }
        if (orderInfo.invaliddate != UDIFileInfo[0].validateDate) {
      ...
      //查询包装信息
      let packging = ObjectStore.selectByMap("GT22176AT10.GT22176AT10.sy01_udi_product_list_bzbsxx", { bzcpbs: UDIFileInfo[0].packageIdentification });
      //绑定UDI时需要
      ...
      let productPacking = ObjectStore.selectByMap("GT22176AT10.GT22176AT10.sy01_udi_product_configure2", { bzcpbs: productPackingLogo }); //查询包装产品标识
      if (productPacking == null || productPacking.length == 0) {
        throw new Error("没有对应包装产品标识！");
      }
      let udiCreateConfig = ObjectStore.selectByMap("GT22176AT10.GT22176AT10.sy01_udi_create_config_son", { sy01_udi_create_config_id: productPacking[0].udiCreateConfigId });
      if (udiCreateConfig != null && udiCreateConfig.length > 0) {
        //查询产品标识
        let productLogo = ObjectStore.queryByYonQL(
          "select bzcpbs,sy01_udi_product_info_id.product product,sy01_udi_product_info_id.productCode materialCode,sy01_udi_product_info_id.productSpecifications productSpecifications,sy01_udi_product_info_id.productName materialName from  GT22176AT10.GT22176AT10.sy01_udi_product_configure2    where cpbzjb like '最小' and sy01_udi_product_info_id = '" +
