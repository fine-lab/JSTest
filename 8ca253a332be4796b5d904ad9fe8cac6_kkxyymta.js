...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    let treatmentrange = prodInfor[0].treatmentrange; //诊疗范围
    let d = new Date(),
      month = "" + (d.getMonth() + 1),
      day = "" + d.getDate(),
      year = d.getFullYear();
    ...
    if (isgspzz) {
      ...
      let treatmentrangeMap = new Map(); //诊疗范围
        ...
        for (let i = 0; i < customerInfor.sy01_customers_file_licenseList.length; i++) {
          ...
          if (vouchdate >= startDate && vouchdate <= endDate) {
            ...
            if (licese.authType == "1" && liceseSub != null) {
              ...
              liceseSub.forEach((item) => {
                treatmentrangeMap.set(item.treatmentrange, item.treatmentrange);
              });
            }
          }
        }
      ...
      if (
        ...
        !treatmentrangeMap.has(treatmentrange) &&
        !allTypeMap.size > 0 &&
        !tzcheck
      ) {
        throw new Error(rowNoInfor + "客户无物料【" + materialName + "】相关证照\r\n");
