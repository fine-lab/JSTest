...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    let syTypeRes = ObjectStore.queryByYonQL(queryType, "sy01");
    if (syTypeRes.length == 0) {
      return { code: "wlwsy", proLicInfo: null, message: "物料未首营过" };
    }
    let syType = syTypeRes[0].isSku;
    if (syType == 0 || syType == "0") {
      return { code: "w2Feature", proLicInfo: null, message: "首营方式为物料" };
    }
    //先判断是否传的值，是否符合敏感
