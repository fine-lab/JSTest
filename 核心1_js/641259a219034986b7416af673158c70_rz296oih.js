...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    let sql = "select id,code,creation_date,errorMessage,dataStatus,dd.item_type item_type,dd.batchNumber batchNumber,dd.deviceSupplier deviceSupplier,dd.productType productType,";
    ...
    sql += "dd.orderType orderType, dd.replenishmentItemSn replenishmentItemSn,";
    sql += "dd.replenishmentRemark replenishmentRemark,";
    sql += "dd.projectName projectName,";
    sql += "dd.originalOrderHeaderNum originalOrderHeaderNum,";
    sql += "dd.defectiveItemCode itemdefectiveItemCodeRemark,";
    sql += "dd.defectiveSn defectiveSn,";
    sql += "dd.oriOrderHeaderDesc oriOrderHeaderDesc,";
    sql += "dd.entireDeviceCode entireDeviceCode,";
    sql += "dd.mainEquipmentSn mainEquipmentSn,";
    sql += "dd.defectiveAsd defectiveAsd,";
    sql += "dd.defectiveAta defectiveAta,";
    sql += "dd.supplier_po_activation_time supplier_po_activation_time ";
    ...
    for (let i = 0; i < res.length; i++) {
      ...
      htData.ATTRIBUTE4 = "";
      htData.ATTRIBUTE5 = "";
      htData.orderType = item.orderType;
      htData.replenishmentItemSn = item.replenishmentItemSn;
      htData.replenishmentRemark = item.replenishmentRemark;
      htData.projectName = item.projectName;
      htData.originalOrderHeaderNum = item.originalOrderHeaderNum;
      htData.defectiveItemCode = item.itemdefectiveItemCodeRemark;
      htData.defectiveSn = item.defectiveSn;
      htData.oriOrderHeaderDesc = item.oriOrderHeaderDesc;
      htData.entireDeviceCode = item.entireDeviceCode;
      htData.mainEquipmentSn = item.mainEquipmentSn;
      htData.defectiveAsd = item.defectiveAsd;
      htData.defectiveAta = item.defectiveAta;
      htData.supplier_po_activation_time = item.supplier_po_activation_time;
      let errorMessage = "";
      let dataStatus = "";
    ...
    // 记录日志
    try {
      let logHtObj = { methodName: "saveCpoResult", requestParams: JSON.stringify(htBody), requestTime: exeHtResponseDate, respResult: htResponse, respTime: exeHtResponseDate, url: htUrl };
      var resHtLog = ObjectStore.insert("AT173E4CEE16E80007.AT173E4CEE16E80007.logRecord", logHtObj, "yb6b993e05");
    } catch (e) {}
    if (htResponseObj.status && htResponseObj.status.toLowerCase() == "success") {
