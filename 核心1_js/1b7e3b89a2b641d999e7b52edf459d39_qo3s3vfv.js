...
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValueInputNumber6rf() {}
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValueInputNumber6rf: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_formStore_InputNumber6rf: reaction(
      () => [rootStore.yctj_child1ListStore.getValue().length, rootStore.yctj_child1ListStore.getValue()],
      () => {
        if (!rootStore.yctj_child1ListStore.dismissStack) {
          rootStore.yctj_child1ListStore.dismissStack = [];
        }
        while (rootStore.yctj_child1ListStore.dismissStack.length) {
          rootStore.yctj_child1ListStore.dismissStack.pop();
        }
        rootStore.yctj_child1ListStore.getValue().forEach((rowStore) => {
          dismiss = reaction(
            () => [rowStore.getTrueValue("InputNumber2a")],
            async ([yctj_child1ListStore_InputNumber2a]) => {
              debounce(
                async () => {
                  // 获取当前页面所有数据
                  ...
                  let result = await countFormula(
                    {
                      hasFormula: "",
                      formula: 'sum(yctj_child1List,"yctj_child1List.InputNumber2a")',
                      formulaFuns: "sum",
                      formulaTriggers: "yctj_child1List,yctj_child1List.InputNumber2a"
                    },
                    {
                  ...
                  // 公式结束loading
                  rootStore.formStore.setValue("InputNumber6r", resultData);
                },
                "formula_customSetValue_formStore_InputNumber6rf" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
                300
              )();
            },
            {
