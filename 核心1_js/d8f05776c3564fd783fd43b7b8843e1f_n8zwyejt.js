...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    let statusCodeInfo = " ";
    let resDataList =
      "select *  from voucher.salereturn.SaleReturn " +
      begindate +
      enddateInfo +
      statusCodeInfo +
      " and saleReturnStatus = 'CONFIRMSALERETURNORDER' limit " +
      request.pageIndex +
      ", " +
      request.pageSize;
    ...
    //获取采购订单列表 中的详情
    let resDetailsList = "select * from voucher.salereturn.SaleReturnDetail where saleReturnId in (" + resRsIds + ")";
    ...
    for (let listi = 0; listi < resArrivalRs.length; listi++) {
      ...
      for (let detailsi = 0; detailsi < resDetailsRs.length; detailsi++) {
        ...
        if (resArrivalRs[listi].id === resDetailsRs[detailsi].saleReturnId) {
