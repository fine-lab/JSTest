  ...
  execute(request) {
    ...
    for (var i = 0; i < resInjs.length; i++) {
      ...
      let sqlInjsb =
        ...
        danjuid +
        "' and bizbillno is not null and (bizbilltype = '188' or bizbilltype = '14' or bizbilltype = '2' or bizbilltype = '10' or bizbilltype = '7' or bizbilltype = '25' or bizbilltype = '26' or bizbilltype = '27' or bizbilltype = '28' or bizbilltype = '11' or bizbilltype = '12' or bizbilltype = '13' or bizbilltype = '3') "; //结算工作台
      ...
      for (var j = 0; j < resInjsb.length; j++) {
        ...
        if (bizbilltype == 14) {
          ...
          for (var d = 0; d < resInfk.length; d++) {
            ...
            if (resInfk[d].freeChId !== undefined) {
              if (resInfk[d].freeChId.PJDQR !== undefined) {
                object.piaodaoqiri = resInfk[d].freeChId.PJDQR; //票到日期号
              } else {
            ...
            let sqlInfk4 = "SELECT freeChId from earap.payment.PaymentBody where headerId = '" + danjuid + "'"; //付款单子表
            let resInfk4 = ObjectStore.queryByYonQL(sqlInfk4, "yonbip-fi-earapbill");
            for (var e = 0; e < resInfk4.length; e++) {
              if (resInfk4[e].freeChId && resInfk4[e].freeChId.PJH) {
                object.huipiaohao = resInfk4[e].freeChId.PJH; //票据号
              }
            }
          }
          ...
          for (var d = 0; d < resInsk.length; d++) {
            ...
            if (resInsk[d].freeChId !== undefined) {
              if (resInsk[d].freeChId.PJDQR !== undefined) {
                object.piaodaoqiri = resInsk[d].freeChId.PJDQR; //票到日期号
              } else {
                object.piaodaoqiri = null; //票到日期号
              }
            } else {
            ...
            let sqlInsk4 = "SELECT freeChId from earap.collection.CollectionBody where headerId = '" + danjuid + "'"; //收款单子表
            let resInsk4 = ObjectStore.queryByYonQL(sqlInsk4, "yonbip-fi-earapbill");
            for (var e = 0; e < resInsk4.length; e++) {
              if (resInsk4[e].freeChId && resInsk4[e].freeChId.PJH) {
                object.huipiaohao = resInsk4[e].freeChId.PJH; //票据号
              }
            }
          }
          ...
          for (var b = 0; b < resInzjsk.length; b++) {
            ...
            if (resInzjsk[b].characterDef !== undefined) {
              if (resInzjsk[b].characterDef.PJDQR !== undefined) {
                object.piaodaoqiri = resInzjsk[b].characterDef.PJDQR; //票到日期号
              } else {
                object.piaodaoqiri = null; //票到日期号
              }
            } else {
            ...
            let sqlInzjsk4 = "SELECT characterDefb from cmp.fundcollection.FundCollection_b where mainid = '" + danjuid + "'"; //资金收款单子表
            let resInzjsk4 = ObjectStore.queryByYonQL(sqlInzjsk4, "ctm-cmp");
            for (let c = 0; c < resInzjsk4.length; c++) {
              if (resInzjsk4[c].characterDefb && resInzjsk4[c].characterDefb.PJH) {
                object.huipiaohao = resInzjsk4[c].characterDefb.PJH; // 汇票号
              }
            }
          }
          ...
          for (var b = 0; b < resInzj.length; b++) {
            ...
            if (resInzj[b].characterDef !== undefined) {
              if (resInzj[b].characterDef.PJDQR !== undefined) {
                object.piaodaoqiri = resInzj[b].characterDef.PJDQR; //票到日期号
              } else {
                object.piaodaoqiri = null; //票到日期号
              }
            } else {
            ...
            let sqlInzj4 = "SELECT characterDefb from cmp.fundpayment.FundPayment_b where mainid = '" + danjuid + "'"; //资金付款单子表
            let resInzj4 = ObjectStore.queryByYonQL(sqlInzj4, "ctm-cmp");
            for (let c = 0; c < resInzj4.length; c++) {
              if (resInzj4[c].characterDefb && resInzj4[c].characterDefb.PJH) {
                object.huipiaohao = resInzj4[c].characterDefb.PJH; // 汇票号
              }
            }
          }
        ...
        } else if (bizbilltype == 188) {
          //统收统支协同单
          debugger;
          console.log(object.danjubianhao);
        }
