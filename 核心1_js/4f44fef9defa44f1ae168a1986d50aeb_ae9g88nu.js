...
viewModel.get("button51xe") &&
  viewModel.get("button51xe").on("click", function (data) {
    // 生成UDI--单击
    //初始化已生成UDI列表
...
function querySonUdiList(param) {
  return new Promise((resolve) => {
    cb.rest.invokeFunction("GT22176AT10.backDefaultGroup.querySonUdiList", param, function (err, res) {
      if (typeof res != "undefined") {
        let udiList = res.result;
...
function queryUdiList(param) {
  return new Promise((resolve) => {
    cb.rest.invokeFunction("GT22176AT10.backDefaultGroup.queryUdiList", param, function (err, res) {
      if (typeof res != "undefined") {
        let udiList = res.result;
        if (udiList != null && udiList.length > 0) {
