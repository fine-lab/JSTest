...
const computedAttrStore = {
  // 扩展get 方法
  interactionRulevisibleDatePicker1zg() {}
};
// 方法改成计算方法
const computedAttr = {
  interactionRulevisibleDatePicker1zg: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    interactionRule_visible_formStore_DatePicker1zg: reaction(
      () => [],
      async ([]) => {
        debounce(
          async () => {
            if (false) {
              rootStore.computedAttrStore.interactionRulevisibleDatePicker1zg = true;
            }
          },
          "interactionRule_visible_formStore_DatePicker1zg " + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
