...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    if (param.billDO.id == null || param.billDO.id == undefined || param.billDO.id == "") {
      //获取项目终验单据数据
      ys_projectEnd_id = param.billDO.id;
      projectAcceptanceSql = `select projectId,acceptDate from ${projectAcceptanceUrl} where id=${ys_projectEnd_id}`;
    } else {
      ys_projectEnd_id = param.billDO.id;
      projectAcceptanceSql = `select projectId,acceptDate from ${projectAcceptanceUrl} where id=${ys_projectEnd_id}`;
    }
    ...
    if (projectEnding_json.data == null || projectEnding_json.data == undefined) {
      return {};
    }
    //获取订单状态
