run = function (event) {
  ...
  //物料过滤
  viewModel.get("material_code").on("beforeBrowse", function (data) {
  ...
  viewModel.get("material_code").on("afterValueChange", function (data) {
  ...
  function getProAllList(orgId) {
    return new Promise((resolve) => {
      cb.rest.invokeFunction("ISY_2.backOpenApiFunction.getProAllList", { orgId: orgId }, function (err, res) {
        let data;
        if (typeof res != "undefined") {
          let mIdArr = res.mIdRes;
          resolve(mIdArr);
