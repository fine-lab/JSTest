...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    let resDataList = "select *, vendor.name from pu.arrivalorder.ArrivalOrder " + begindate + enddateInfo + " limit " + request.pageIndex + ", " + request.pageSize;
    ...
    if (resRsIds.length === 0) {
      return { resArrivalRs };
    }
    //获取到货列表 中的详情
    let resDetailsList =
      "select *,product.cCode,product.cName,product.realProductAttribute,productsku.cCode,productsku.skuName,unit.name from pu.arrivalorder.ArrivalOrders where mainid in (" + resRsIds + ")";
    let resDetailsRs = ObjectStore.queryByYonQL(resDetailsList, "upu");
    if (resPurinstockys.length === 0) {
      return { resArrivalRs, resDetailsRs };
    }
    //获取验收单 列表
    let resPurinstockysSql = "select * from GT22176AT10.GT22176AT10.SY01_purinstockysv2 where source_id in (" + resPurinstockys + ")";
    let resPurinstockysRs = ObjectStore.queryByYonQL(resPurinstockysSql, "sy01");
    ...
    let resPurinstockysDetailsRs = ObjectStore.queryByYonQL(resPurinstockysDetailsSql, "sy01");
    //返回列表总数
    ...
    for (let listi = 0; listi < resArrivalRs.length; listi++) {
      ...
      for (let detailsi = 0; detailsi < resDetailsRs.length; detailsi++) {
        ...
        if (resArrivalRs[listi].id === resDetailsRs[detailsi].mainid) {
