...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    let saleReturnSql = "select extendIsWms,transactionTypeId.name as bustypeName  from 		voucher.salereturn.SaleReturn where id ='" + param.data[0].id + "' ";
    let saleReturnRes = ObjectStore.queryByYonQL(saleReturnSql, "udinghuo")[0];
    if (saleReturnRes.extendIsWms != 1 && saleReturnRes.extendIsWms != "1") {
      return;
    ...
    var strResponse = postman("post", apiRestPre + "/gsp/wmsSalesReturnPush", null, JSON.stringify(obj));
    return { strResponse };
  }
}
