...
// 计算属性代码监听开始
let XSQZDD_child1ListStoreExtendRowStore = RowStore;
if (rootStore.XSQZDD_child1ListStore.extendRowStore) {
  XSQZDD_child1ListStoreExtendRowStore = rootStore.XSQZDD_child1ListStore.extendRowStore;
}
class XSQZDD_child1ListStoreRowStore extends XSQZDD_child1ListStoreExtendRowStore {
  constructor() {
    super(...arguments);
    this.formulacustomSetValueInput38ua = undefined;
    makeObservable(this, {
      formulacustomSetValueInput38ua: observable
    });
  }
}
rootStore.XSQZDD_child1ListStore.extendRowStore = XSQZDD_child1ListStoreRowStore;
// 扩展新的store
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_XSQZDD_child1ListStore_Input38ua: reaction(
      () => [rootStore.XSQZDD_child1ListStore.getValue().length, rootStore.XSQZDD_child1ListStore.getValue()],
      () => {
        if (!rootStore.XSQZDD_child1ListStore.dismissStack) {
          rootStore.XSQZDD_child1ListStore.dismissStack = [];
        }
        while (rootStore.XSQZDD_child1ListStore.dismissStack.length) {
          rootStore.XSQZDD_child1ListStore.dismissStack.pop();
        }
        rootStore.XSQZDD_child1ListStore.getValue().forEach((rowStore, tableStoreRowIndex) => {
          dismiss = reaction(
            () => [rowStore.getTrueValue("ReferInput7hc")],
            async ([XSQZDD_child1ListStore_ReferInput7hc]) => {
              debounce(
                async () => {
                  const allData = getAllDataByCardStore({
                    ...
                    storeField: "Input38u",
                    storeForm: "",
                    dataSource: "XSQZDD_child1List"
                  });
                  const formulaResult = await countFormula(
                    {
                      hasFormula: "true",
                      formula: "XSQZDD_child1List.ReferInput7hc.unit.name",
                      formulaFuns: "",
                      formulaTriggers: "XSQZDD_child1List.ReferInput7hc.unit.name"
                    },
                    {
                      mainDataSourceAlias: "mainEntity",
                      field: "XSQZDD_child1List.Input38u",
                      billnum: "XSQZDD",
                  ...
                  rowStore.setValue("Input38u", resultData);
                },
                "formula_customSetValue_XSQZDD_child1ListStore_Input38ua" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
                300
              )();
            },
            {
