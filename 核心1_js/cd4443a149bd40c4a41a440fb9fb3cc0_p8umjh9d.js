...
function makeSaleOutBill(allCheck) {
  ...
  proxy.queryData(param, function (err, result) {
    ...
    viewModel.get("btnAdd").fireEvent("click"); //自动进入新增界面 230922 fengqin add
  });
}
function soundAlert() {
...
viewModel.get("button24uf") &&
  viewModel.get("button24uf").on("click", function (data) {
    ...
    try {
      ...
      viewModel.get("button24uf").setDisabled(false);
    }
  });
...
function getSysTime() {
  ...
  return sysDate.getFullYear() + "-" + sysDate.getMonth() + "-" + sysDate.getDate() + " " + sysDate.getHours() + ":" + sysDate.getMinutes() + ":" + sysDate.getSeconds() + " ";
}
// 打印校验
viewModel.on("beforePrintnow", function (args) {
  console.log(getSysTime() + "beforePrintnow start");
  if (!valitBillCode()) {
    return false;
  }
  return valiZx("直接打印");
...
function valiZx(act) {
  ...
  // 传参
  var param = allCheck;
  console.log(getSysTime() + act + "-1");
  const reData = proxy.queryData(param);
  console.log(getSysTime() + act + "-2");
  if (!!reData.result && reData.result.status != 200) {
    alert(result.error);
    return false;
  }
  console.log(getSysTime() + act + "-3");
  if (!reData.error.success) {
    alert(reData.error.msg);
    return false;
  }
  console.log(getSysTime() + act + "-4");
  return true;
}
viewModel.get("button35oe") &&
  viewModel.get("button35oe").on("click", function (data) {
    console.log("打印：" + data);
    let args = cb.utils.extend(
      ...
      {},
      {
        ...
        cHttpMethod: "POST",
        authOperate: false,
        fieldName: "button35oe",
        fieldRuntimeState: false,
        cItemName: "button35oe",
        cCaption: "打印预览",
        ...
        cmdParameter: '{"classifyCode":"GT14722AT8.357a4b42","params":{"billno":"357a4b42"}}',
