...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    let personObj = openLinker("GET", personUrl, "ycReqManagement", JSON.stringify({}));
    let psnCode = JSON.parse(personObj).data.code;
    //查询部门code
    let deptUrl = "https://www.example.com/" + deptId;
    let deptObj = openLinker("GET", deptUrl, "ycReqManagement", JSON.stringify({}));
    let deptCode = JSON.parse(deptObj).data.code;
    //查询组织code
    let orgUrl = "https://www.example.com/";
    let orgObj = openLinker("GET", orgUrl + orgId, "ycReqManagement", JSON.stringify({}));
