...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    for (let k = 0; k < data.length; k++) {
      ...
      let tempArr = [];
      let tempStr = "";
      let infoLength = requestMaterialInfo.length;
      for (let i = 0; i < materalIds.length; i++) {
      ...
      for (let i = 0; i < requestMaterialInfo.length; i++) {
        if (requestMaterialInfo[i].type == undefined) {
          requestMaterialInfo[i].type = ["common"];
        }
      }
      //规则匹配
      for (let i = 0; i < rule.length; i++) {
        ...
        if (rule[i].type == "9" || rule[i].type1 == "9") {
          let typeNumber = rule[i].type == "9" ? rule[i].type1 : rule[i].type;
          let otherType = map[typeNumber].type;
