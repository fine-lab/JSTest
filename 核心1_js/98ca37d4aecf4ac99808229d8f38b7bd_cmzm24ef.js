...
// 获取卡片页全数据
const getPageAllData = ({ rootStore, storeControlType, storeField, currDataSourceAlias, rowStore }) => {
  // 如果是主表 则field节点清空返回
  if (storeControlType === "FormStore") {
    const allData = rootStore?.cardStore.getValue() || {};
    allData[storeField] = "";
    return allData;
  } else {
    const allData = rootStore?.cardStore.getValue();
    const rowData = rowStore.getValue() || {};
    rowData[storeField] = "";
