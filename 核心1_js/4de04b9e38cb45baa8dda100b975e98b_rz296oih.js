...
function recursionPush(allData, numsPerBatch, batchIndex) {
  let shippingscheduleSNList = allData.shippingscheduleSNList;
  let shippingschedulebList = allData.shippingschedulebList;
  let totalBatchSize = Math.ceil(shippingscheduleSNList.length / numsPerBatch);
  let batchNumber = new Date().getTime();
  debugger;
  for (let i = 0; i * numsPerBatch < shippingscheduleSNList.length; i++) {
    let pushData = shippingscheduleSNList.slice(i * numsPerBatch, (i + 1) * numsPerBatch);
    if (pushData.length > 0) {
      let result = cb.rest.invokeFunction(
        "GT37595AT2.backOpenApiFunction.ReplyShipSnToS",
        {
          replyData: {
            currentBatchNo: i + 1,
            batchNumber: batchNumber,
            totalBatchSize: totalBatchSize,
            totalNumSS: shippingscheduleSNList.length,
            shippingschedulebList,
            shippingscheduleSNList: pushData
          }
        },
        function (err, res) {},
        viewModel,
      ...
      if (result.result.code === "200") {
        ...
        return {
          success: false,
          message: result.result.message + ":" + result.result.data
        };
      }
    }
  }
...
viewModel.get("button114qh") &&
  viewModel.get("button114qh").on("click", function (data) {
    ...
    debugger;
    cb.rest.invokeFunction("GT37595AT2.backOpenApiFunction.deleteAllSN", { id }, function (err, res) {
      debugger;
    });
  });
...
// 删除全量SN
viewModel.get("button167qb") &&
  viewModel.get("button167qb").on("click", function (data) {
    let id = viewModel.get("id");
    cb.rest.invokeFunction("GT37595AT2.backOpenApiFunction.deleteAllSN", { id }, function (err, res) {});
  });