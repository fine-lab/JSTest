...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    let ys_projectEnd;
    let ys_projectEnd_id;
    //项目终验实体URL
    var projectAcceptanceUrl = "ckam.projectacceptance.ProjectAcceptance";
    var domainKey = "yourKeyHere";
    var projectAcceptanceSql;
    if (param.requestData.id == null || param.requestData.id == undefined || param.requestData.id == "") {
      //获取项目终验单据数据
      ys_projectEnding = param.requestData;
      ys_projectEnd = JSON.parse(ys_projectEnding);
      ys_projectEnd_id = ys_projectEnd[0].id;
      projectAcceptanceSql = `select projectId,acceptDate from ${projectAcceptanceUrl} where id=${ys_projectEnd[0].id}`;
    } else {
      ys_projectEnd = param.requestData;
      ys_projectEnd_id = ys_projectEnd.id;
      projectAcceptanceSql = `select projectId,acceptDate from ${projectAcceptanceUrl} where id=${ys_projectEnd.id}`;
    }
    //获取项目终验行id
    ...
    for (var i = 0; i < other.length; i++) {
      ...
      let crm_item = {
        ...
        code: ys_projectEnd_id //交付流水号     (主表id）
      };
      var rows = new Array();
      rows.push(crm_item);
      //定义crm接口数据
