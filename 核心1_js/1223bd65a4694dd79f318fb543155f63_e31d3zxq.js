    ...
    if (type == "购进入库验收") {
      ...
      if (childRes.length > 0) {
        for (let i = 0; i < childRes.length; i++) {
          ...
          if (index == -1) {
            arrivalChildIds.push(childRes[i].sourcechild_id);
            arrivalCheckQty[childRes[i].sourcechild_id] = childRes[i].fcheckqty == NaN ? 0 : Number.parseFloat(childRes[i].fcheckqty);
          } else {
            arrivalCheckQty[childRes[i].sourcechild_id] += childRes[i].fcheckqty == NaN ? 0 : Number.parseFloat(childRes[i].fcheckqty);
          }
        }
      ...
      if (childRes.length > 0) {
        for (let i = 0; i < childRes.length; i++) {
          ...
          if (index == -1) {
            arrivalChildIds.push(childRes[i].sourcechild_id);
            arrivalCheckQty[childRes[i].sourcechild_id] = childRes[i].checkNum == NaN ? 0 : Number.parseFloat(childRes[i].checkNum);
          } else {
            arrivalCheckQty[childRes[i].sourcechild_id] += childRes[i].checkNum == NaN ? 0 : Number.parseFloat(childRes[i].checkNum);
          }
        }
