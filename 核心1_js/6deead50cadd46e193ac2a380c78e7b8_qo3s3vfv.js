...
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValueInput20wg() {},
  formulacustomSetValueInputNumber3ci() {}
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValueInput20wg: observable,
  formulacustomSetValueInputNumber3ci: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_formStore_Input20wg: reaction(
      () => [rootStore.formStore.getValue("UserReferInput0hc")],
      async ([formStore_UserReferInput0hc]) => {
        debounce(
          async () => {
            ...
            const formulaResult = await countFormula(
              {
                hasFormula: "true",
                formula: "UserReferInput0hc.mobile",
                formulaFuns: "",
                formulaTriggers: "UserReferInput0hc.mobile"
              },
              {
            ...
            rootStore.formStore.setValue("Input20w", resultData);
          },
          "formula_customSetValue_formStore_Input20wg" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
    ...
    formula_customSetValue_formStore_InputNumber3ci: reaction(
      () => [rootStore.formStore.getValue("DateTimePicker9j"), rootStore.formStore.getValue("DateTimePicker7f")],
      async ([formStore_DateTimePicker9j, formStore_DateTimePicker7f]) => {
        debounce(
          async () => {
            const allData = getAllDataByCardStore({
              cardStoreAlias: "cardStore",
              storeControlType: "FormStore",
              storeField: "InputNumber3c",
              storeForm: "",
              dataSource: "mainEntity"
            });
            const formulaResult = await countFormula(
              {
                hasFormula: "true",
                formula: "dateDiff(DateTimePicker9j,DateTimePicker7f,4)",
                formulaFuns: "dateDiff",
                formulaTriggers: "DateTimePicker9j,DateTimePicker7f"
              },
              {
                mainDataSourceAlias: "mainEntity",
                field: "InputNumber3c",
            ...
            //对公式结果进行处理
            const resultData = formulaResult.resultData;
            rootStore.formStore.setValue("InputNumber3c", resultData);
          },
          "formula_customSetValue_formStore_InputNumber3ci" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
