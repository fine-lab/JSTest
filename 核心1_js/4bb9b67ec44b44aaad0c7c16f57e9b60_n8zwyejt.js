...
viewModel.get("button33dd") &&
  viewModel.get("button33dd").on("click", function (datas) {
    ...
    getBillList(billurl, startDate, endDate);
  });
      ...
      if (err) {
        ...
        for (let i = 0; i < dataList.length; i++) {
          let object = {
            ...
            batchNo: dataList[i].batchno,
            invaliddate: dataList[i].invaliddate,
            producedate: dataList[i].producedate,
            subQty: dataList[i].materialNum,
            unitName: dataList[i].unitName,
            item9nh: dataList[i].materialName,
            item11ge: dataList[i].materialNum
          };
          list.push(object);
        }
