  ...
  execute(request) {
    ...
    if (billType == "return") {
      for (var i = 0; i < griddata.length; i++) {
        ...
        if (Math.abs(reviewQty - (qualifieQty + unqualifieQty + rejectQty)) > 0.00000001) {
      ...
      for (var i = 0; i < griddata.length; i++) {
        ...
        if (Math.abs(reviewQty - (qualifieQty + unqualifieQty)) > 0.00000001) {
    ...
    for (var i = 0; i < griddata.length; i++) {
      ...
      // 查询上游不确定总数
      var upsqlstr = "select id," + upQtyColum + " as upQty from " + upDetailUri + "  where id = '" + sourcechild_id + "'";
      var upsqlstr = "select id," + upQtyColum + " as upQty from " + upDetailUri + "  where id = '" + sourcechild_id + "'";
