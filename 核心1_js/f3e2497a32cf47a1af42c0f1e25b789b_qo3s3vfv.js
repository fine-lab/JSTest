...
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValueInput19ib() {}
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValueInput19ib: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_formStore_Input19ib: reaction(
      () => [rootStore.formStore.getValue("ReferInput2yi")],
      async ([formStore_ReferInput2yi]) => {
        debounce(
          async () => {
            // 获取当前页面所有数据
            ...
            let result = await countFormula(
              {
                hasFormula: "",
                formula: "ReferInput2yi.shortname",
                formulaFuns: "",
                formulaTriggers: "ReferInput2yi.shortname"
              },
              {
            ...
            // 公式结束loading
            rootStore.formStore.setValue("Input19i", resultData);
          },
          "formula_customSetValue_formStore_Input19ib" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
