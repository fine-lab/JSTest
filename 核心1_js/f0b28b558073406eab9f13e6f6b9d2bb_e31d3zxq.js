...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    let selectReviewQl =
      "select id,sourcechild_id,batch_no,manufact_date,valid_until,qualifie_qty,quaInState,quaInStateName," +
      "unqualifie_qty,unquaDealType,unQuaNeedInQty,unQuaInState,unQuaInStateName,uncertain_qty,position, position.name positionName from " +
    ...
    for (let i = 0; i < res.length; i++) {
      ...
      map[res[i].sourcechild_id][res[i].id]["1"] = {
        ...
        goodsposition: res[i].position,
        goodspositionName: res[i].positionName
      };
      map[res[i].sourcechild_id][res[i].id]["2"] = {
        batch: res[i].batch_no,
        produceDate: res[i].manufact_date,
        expireDate: res[i].valid_until,
        qty: res[i].unQuaNeedInQty == undefined ? 0 : res[i].unQuaNeedInQty,
      ...
      if (res[i].uncertain_qty != undefined && res[i].uncertain_qty > 0) {
        let selectReCheckwQl =
          "select id,SY01_quareventryv1Fk.code code,sourcechild_id,fchgsl,qualifiedstate,qualifiedstateName,fcbhgsl,noQualifiedResult,noQuaNeedInQty,noqualifiedstate,noqualifiedstateName " +
        ...
        if (reCheckRes.length == 1) {
          ...
          if (reCheckRes[0].noQuaNeedInQty != undefined && reCheckRes[0].noQuaNeedInQty > 0) {
            map[res[i].sourcechild_id][res[i].id]["2"]["qty"] += reCheckRes[0].noQuaNeedInQty;
          }
        }
      }
    }
    ...
    if (request.id == undefined || request.id == null || request.id == "" || JSON.stringify(request.id) == "{}") {
      request.id = "-1";
    }
