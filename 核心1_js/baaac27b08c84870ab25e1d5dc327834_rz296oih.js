      ...
      res.map((item) => {
        ...
        if (codeToSN[tempCode].sn && codeToSN[tempCode].assetcode) {
          ...
          if (item.fixedAssetsInfoCharacter) {
            item.fixedAssetsInfoCharacter.attrext2 = codeToSN[tempCode].sn;
            item.fixedAssetsInfoCharacter.attrext3 = codeToSN[tempCode].equipcode || "";
            item.fixedAssetsInfoCharacter.attrext4 = codeToSN[tempCode].assetcode || "";
            item.fixedAssetsInfoCharacter.attrext8 = codeToSN[tempCode].requiremcode || "";
            item.fixedAssetsInfoCharacter.attrext10 = codeToSN[tempCode].contractnumber || "";
            item.fixedAssetsInfoCharacter.attrext11 = codeToSN[tempCode].showTime || "";
            item.fixedAssetsInfoCharacter._status = "Update";
          } else {
            item.fixedAssetsInfoCharacter = {
              // 特征升级
              attrext2: codeToSN[tempCode].sn,
              attrext3: codeToSN[tempCode].equipcode,
              attrext4: codeToSN[tempCode].assetcode,
              attrext8: codeToSN[tempCode].requiremcode,
              attrext10: codeToSN[tempCode].contractnumber,
              attrext11: codeToSN[tempCode].showTime,
              _status: "Insert"
            };
          }
        }
