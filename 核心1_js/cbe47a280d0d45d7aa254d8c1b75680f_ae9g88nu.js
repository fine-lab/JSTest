run = function (event) {
  ...
  viewModel.get("button41xh").on("click", function () {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (data[i].bustype_name == "销售出库" || data.bustype_name == "GSP销售出库") {
        let index = ids.indexOf(data[i].id);
        if (index == -1) {
          ids.push(data[i].id);
        }
      } else {
        cb.utils.alert("编码为" + data[i].code + "的单据的交易类型不是[销售出库/GSP销售出库]");
        return false;
      }
    }
  ...
  viewModel.get("button59yi").on("click", function () {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (data[i].bustype_name == "GSP销售退货") {
        let index = ids.indexOf(data[i].id);
        if (index == -1) {
          ids.push(data[i].id);
        }
      } else {
        cb.utils.alert("编码为" + data[i].code + "的单据的交易类型不是[GSP销售退货]");
        return false;
      }
    }
    getSaleOrders = function (type) {
      ...
      for (let i = 0; i < orderInfos.length; i++) {
        ...
        for (let j = 0; j < orderInfos[i].entry.length; j++) {
          ...
          sheetData2.push({
            ...
            退货原因: orderInfos[i].memo,
            销售单号: orderInfos[i].entry[j].firstupcode,
            本位码: orderInfos[i].entry[j].extend_standard_code,
            包装规格: orderInfos[i].entry[j].extend_package_specification
          });
        }
  ...
  viewModel.get("button77ri").on("click", function () {
    ...
    getSaleOrders("抽检出库").then((orderInfos) => {
      ...
      for (let i = 0; i < orderInfos.length; i++) {
        sheetData1.push({
          出库日期: exportDate(orderInfos[i].vouchdate),
      ...
      for (let i = 0; i < orderInfos.length; i++) {
        ...
        for (let j = 0; j < orderInfos[i].entry.length; j++) {
          ...
          sheetData2.push({
            生产批号: orderInfos[i].entry[j].batchno,
            生产日期: exportDate(orderInfos[i].entry[j].producedate),
            有效期至: exportDate(orderInfos[i].entry[j].invaliddate),
