viewModel.on("customInit", function (data) {
  ...
  viewModel.get("curing_dept_name").on("beforeBrowse", function (data) {
    var externalData = {};
    externalData.ref_parentorgid = viewModel.get("org_id").getValue();
    (externalData.funcCode = "all"), (externalData.accountdelegate = "true"), viewModel.get("curing_dept_name").setState("externalData", externalData);
  });
  //部门切换，或清空，清空验收员
  viewModel.get("curing_dept_name").on("afterValueChange", function (data) {
    let id = viewModel.get("id").getValue();
    if (data.value == null || (data.oldValue != null && data.value.id != data.oldValue.id)) {
      viewModel.get("curing_man").setValue(null);
      viewModel.get("curing_man_name").setValue(null);
      let curingMan = viewModel.get("curing_man").getValue();
      //同步修改其他养护的人员和部门
    }
  });
  //人员切换
  viewModel.get("curing_man_name").on("afterValueChange", function (data) {
    let id = viewModel.get("id").getValue();
    if (data.value == null || (data.oldValue != null && data.value.id != data.oldValue.id)) {
      let deptId = viewModel.get("curing_dept").getValue();
      let deptName = viewModel.get("curing_dept_name").getValue();
      //同步修改其他养护的人员和部门
    }
  });
  //选择仓库后
  ...
  viewModel.get("button56xd") &&
    viewModel.get("button56xd").on("click", function (data) {
      // 提取商品--单击
      //根据养护过滤方案查询重点商品养护明细
      mainprocSonList.deleteAllRows();
      let param = { filterSchemeId: viewModel.get("curing_filter_scheme").getValue(), warehouseId: viewModel.get("warehouse").getValue(), orgId: viewModel.get("finOrg").getValue() };
      ...
      cb.rest.invokeFunction("GT22176AT10.backDefaultGroup.getMainprocoSon", param, function (err, res) {
        if (res != undefined && res.result != undefined) {
          ...
          if (sonList == undefined || sonList.length == 0) {
            cb.utils.alert("未能提取到对应的商品！");
          } else {
            let characteristicsKey = "yourKeyHere";
            for (let i = 0; i < sonList.length; i++) {
              mainprocSonList.insertRow(i, sonList[i]);
              if (mainprocSonList.getRowModel(i) && mainprocSonList.getRowModel(i).get(characteristicsKey)) {
                let freeCTModel = mainprocSonList.getRowModel(i).get(characteristicsKey);
  ...
  viewModel.get("curing_filter_scheme_name").on("beforeBrowse", function (data) {
    let orgId = viewModel.get("finOrg").getValue();
  ...
  viewModel.on("beforePush", function (data) {
    ...
    let request = { id: id, uri: "GT22176AT10.GT22176AT10.SY01_commodity_plan" };
    //判断下游单据是否存在
  ...
  function mainProPushCheck(request) {
  ...
  function syncUpdateCuring(id, deptId, curingMan) {
    return new Promise(function (resolve) {
      invokeFunction1(
        "GT22176AT10.mainpro.syncUpdateCuring",
        {
          billId: id,
          deptId: deptId,
          curingMan: curingMan,
