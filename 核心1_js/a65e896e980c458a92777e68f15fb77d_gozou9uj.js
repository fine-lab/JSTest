...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    let { orders, sum: sumOrder, billData, batchData } = request;
    ...
    let batchId = batchData.map((item) => item.id).join(",");
    let sql = `select *,(select * from batch_price_detailList) batch_price_detailList from GT18AT2.GT18AT2.batch_price where id in (${batchId}) order by batch_time`;
    ...
    for (let item of res) {
      ...
      for (let obj of detailList) {
        ...
        if (tax_rate && (!tax_rate_code || !tax_rate_num)) {
          let querySql = `select * from bd.taxrate.TaxRateVO where id=${tax_rate} `;
          let rateRes = ObjectStore.queryByYonQL(querySql, "ucfbasedoc");
          if (rateRes && rateRes.length > 0) {
            tax_rate_code = rateRes[0].code;
            tax_rate_num = rateRes[0].ntaxRate;
          }
        }
        flatBatchList.push({
          matieral,
    ...
    // 开始处理需要修改的批次价格数据和采购订单数据(获价逻辑正式开始)
    debugger;
    for (let i = 0; i < orders.length; i++) {
      let { product, qty: num, id } = orders[i];
      overrideOrder.purchaseOrders.push({});
      // 若此时order是新插入的，还没有id,则把这条order的状态标记为Insert(新拆的行)
      ...
      if (remain_num >= num) {
        ...
        // 给采购订单新拆一行，并在structureNewOrder中设置好新行里的固定数据，在下一个循环中继续获价
        orders.splice(i + 1, 0, {});
    ...
    // 通过已经取过价的changedBatchList，计算回写接口的数据overrideBatchList
    ...
    debugger;
    // 计算overrideBatchList中表头的剩余总量remain_total
...
function structureData(purchaseorder, order, flatBatchItem, status, num, exchRate) {
  ...
  // 含税单价
  purchaseorder.oriTaxUnitPrice = price_with_tax.toFixed(2);
  // 含税金额
