...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    if (isgspzz) {
      ...
      if (!prodMap.has(materialId) && !gspTyepMap.has(extend_gsp_spfl) && !drugFormMap.has(extend_jx) && !skuMap.has(materialSkuId) && !allTypeMap.size > 0 && !tzcheck) {
        throw new Error(rowNoInfor + "客户无物料【" + materialName + "】相关证照\r\n");
      }
    } //End 物料证照验证
    if (poacontrol == "1" && listHeadCustomSalesman == undefined) {
