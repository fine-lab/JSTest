...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    let shippingschedulebSql =
      "select *, main.code as code, main.id as mainId,main.creation_date as pushVendorTime,main.vendorId as vendorId from	GT37595AT2.GT37595AT2.shippingscheduleb left join GT37595AT2.GT37595AT2.shippingschedule main " +
    ...
    for (var i = 0; i < lineDataList.length; i++) {
      for (var j = 0; j < shippingschedulebRes.length; j++) {
        if (lineDataList[i].extendOrdersMapping && shippingschedulebRes[j].orders_mapping && lineDataList[i].extendOrdersMapping == shippingschedulebRes[j].orders_mapping) {
          ...
          if (!vendorIdToVendor[shippingschedulebRes[j].vendorId].extendPushDown) {
            break;
          }
          let itemSql = "";
          // 风冷整机柜编码相同的，去重，只发送一条
          ...
          if (shippingschedulebRes[j].deviceSupplier == sconfig.BASE.deviceSupplier) {
    ...
    let updateurl = envUrl + "/iuap-api-gateway/" + tenantId + "/product_ref/product_ref_01/updateShipping?access_token=" + access_token;
