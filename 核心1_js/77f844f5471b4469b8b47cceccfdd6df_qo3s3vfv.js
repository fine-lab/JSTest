...
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValueInputNumber8dk() {}
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValueInputNumber8dk: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_formStore_InputNumber8dk: reaction(
      () => [rootStore.formStore.getValue("DateTimePicker13n"), rootStore.formStore.getValue("DateTimePicker8f")],
      async ([formStore_DateTimePicker13n, formStore_DateTimePicker8f]) => {
        debounce(
          async () => {
            // 获取当前页面所有数据
            ...
            let result = await countFormula(
              {
                hasFormula: "",
                formula: "numToStr(dateDiff(DateTimePicker13n,DateTimePicker8f,4))",
                formulaFuns: "numToStr,dateDiff",
                formulaTriggers: "DateTimePicker13n,DateTimePicker8f"
              },
              {
          ...
          "formula_customSetValue_formStore_InputNumber8dk" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
