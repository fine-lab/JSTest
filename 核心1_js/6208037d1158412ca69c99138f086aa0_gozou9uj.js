...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    if (param.data[0]._fromApi) {
      ...
      if (!param.data[0].phymaterial_code || param.data[0].phymaterial_code == "") {
        param.data[0].set("phymaterial_code", phymaterial_id_code);
      }
      if (!param.data[0].productname_code || param.data[0].productname_code == "") {
        param.data[0].set("productcode", productId_code);
      }
      if (!param.data[0].phymaterial_name || param.data[0].phymaterial_name == "") {
        param.data[0].set("phymaterial_name", phymaterial_name);
      }
      if (!param.data[0].productname || param.data[0].productname == "") {
        param.data[0].set("productname", productname);
      }
      if (!param.data[0].is_quote || param.data[0].is_quote == "") {
        param.data[0].set("is_quote", "0");
      }
      if (!param.data[0].org_id || param.data[0].org_id == "") {
        param.data[0].set("org_id", ObjectStore.user().orgId || "1490021994727473155");
      }
      param.data[0].set("phymaterial_id_name", phymaterial_name);
      param.data[0].set("phymaterial_id", phymaterial_id);
      param.data[0].set("productId_name", productname);
      param.data[0].set("productId", productId);
    } else {
      //查询已存在的映射关系
      let sql = "select productcode, phymaterial_code from AT160194EA17D00009.AT160194EA17D00009.calcmappingtable where phymaterial_code = '" + list[0].phymaterial_id_code + "' and  enable = 1";
      var result = ObjectStore.queryByYonQL(sql);
