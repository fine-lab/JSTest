run = function (event) {
  ...
  viewModel.on("afterLoadData", function (data) {
    ...
    for (let i = 0; i < rows.length; i++) {
      if (materialIds.indexOf(rows[i].product) == -1) {
        materialIds.push(rows[i].product);
      }
    }
  ...
  let djType = "委外入库单";
  let btnUdiClike = "button68da"; //udi管理按钮
  let btnUdiText = "item666eg"; //udi上下文
  ...
  function saveUdidata() {
    // 保存udi--保存后触发
    let udiInfoValue = viewModel.get(btnUdiText).getValue(); //获取保存的上下文
