...
// 计算属性代码监听开始
let fxzzzz1_child1ListStoreExtendRowStore = RowStore;
if (rootStore.fxzzzz1_child1ListStore.extendRowStore) {
  fxzzzz1_child1ListStoreExtendRowStore = rootStore.fxzzzz1_child1ListStore.extendRowStore;
}
class fxzzzz1_child1ListStoreRowStore extends fxzzzz1_child1ListStoreExtendRowStore {
  constructor() {
    super(...arguments);
    this.formulacustomSetValueInput36vg = undefined;
    makeObservable(this, { formulacustomSetValueInput36vg: observable });
  }
}
rootStore.fxzzzz1_child1ListStore.extendRowStore = fxzzzz1_child1ListStoreRowStore;
// 扩展新的store
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValueInputNumber2we() {},
  interactionRulevisibleInput32xg() {}
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValueInputNumber2we: observable,
  interactionRulevisibleInput32xg: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_fxzzzz1_child1ListStore_Input36vg: reaction(
      () => [rootStore.fxzzzz1_child1ListStore.getValue().length, rootStore.fxzzzz1_child1ListStore.getValue()],
      () => {
        if (!rootStore.fxzzzz1_child1ListStore.dismissStack) {
          rootStore.fxzzzz1_child1ListStore.dismissStack = [];
        }
        while (rootStore.fxzzzz1_child1ListStore.dismissStack.length) {
          rootStore.fxzzzz1_child1ListStore.dismissStack.pop();
        }
        rootStore.fxzzzz1_child1ListStore.getValue().forEach((rowStore) => {
          dismiss = reaction(
            () => [rowStore.getTrueValue("InputNumber5n")],
            async ([fxzzzz1_child1ListStore_InputNumber5n]) => {
              const allData = getPageAllData({
                rootStore,
                currUiStore: "fxzzzz1_child1ListStore",
                currTableUiStore: "",
                storeControlType: "TableStore",
                storeField: "Input36v",
                currDataSourceAlias: "fxzzzz1_child1List",
                rowStore: rowStore
              });
              let result = await countFormula(
                {
                  hasFormula: "",
                  formula: "fxzzzz1_child1List.InputNumber5n",
                  formulaFuns: "",
                  formulaTriggers: "fxzzzz1_child1List.InputNumber5n"
                },
                {
                  mainDataSourceAlias: "mainEntity",
                  field: "fxzzzz1_child1List.Input36v",
                  billnum: "fxzzzz1",
              ...
              console.error("formula", result);
              rowStore.setValue("Input36v", resultData);
            },
            {
              fireImmediately: formulaFireImmediately(),
              equals: function () {
                if (rootStore.cardStore) {
                  if (rootStore.cardStore.uiState === "browse") {
                    return true;
    ...
    formula_customSetValue_formStore_InputNumber2we: reaction(
      () => [rootStore.fxzzzz1_child1ListStore.getValue().length, rootStore.fxzzzz1_child1ListStore.getValue()],
      () => {
        if (!rootStore.fxzzzz1_child1ListStore.dismissStack) {
          rootStore.fxzzzz1_child1ListStore.dismissStack = [];
          ...
          dismiss = reaction(
            ...
            async ([fxzzzz1_child1ListStore_InputNumber5n]) => {
              ...
              let result = await countFormula(
                {
                  hasFormula: "",
                  formula: 'sum(fxzzzz1_child1List,"fxzzzz1_child1List.InputNumber5n")',
                  formulaFuns: "sum",
                  formulaTriggers: "fxzzzz1_child1List,fxzzzz1_child1List.InputNumber5n"
                },
                {
                  mainDataSourceAlias: "mainEntity",
                  field: "InputNumber2w",
              ...
              const resultData = result.resultData;
              console.error("formula", result);
              rootStore.formStore.setValue("InputNumber2w", resultData);
            },
            {
              fireImmediately: formulaFireImmediately(),
              equals: function () {
    ...
    interactionRule_visible_formStore_Input32xg: reaction(
      () => [],
      async ([]) => {
        if (false) {
        } else {
