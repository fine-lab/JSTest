...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    var sql1 =
      "select id,code,name,orgId,productApplyCharacterDef from pc.productapply.ProductApply where productUpdateTime < '" +
      strDate +
      "' and productUpdateTime >= '" +
      strDate6 +
      "' and effectStatus = 2";
    ...
    if (res1.length > 0) {
      for (var i = 0; i < res1.length; i++) {
        ...
        //查询物料是否已经创建  sy01
        var sql3 = "select id as num from ISY_2.ISY_2.SY01_gmp_supplies_file where material = '" + material + "' and enable = '1'";
      ...
      if (arr.length > 0) {
        var res = ObjectStore.insertBatch("ISY_2.ISY_2.SY01_gmp_supplies_file", arr, "26949c33List");
      }
      return res;
    }
    return res1;
