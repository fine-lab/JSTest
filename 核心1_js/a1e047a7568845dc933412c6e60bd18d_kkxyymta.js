...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    let audiYonql = "select id,org_id,code from GT22176AT10.GT22176AT10.SY01_fccusauditv4 where dr = 0 and id = '" + param.data[0].id + "'";
    let auditRes = ObjectStore.queryByYonQL(audiYonql, "sy01")[0];
    let materialFileYonql = "select id,material from GT22176AT10.GT22176AT10.SY01_material_file where dr = 0 and firstBusinessOrderNo = '" + auditRes.code + "'";
    let materialFileRes = ObjectStore.queryByYonQL(materialFileYonql, "sy01");
    if (materialFileRes.length != 0) {
      let sql = "select * from 	GT22176AT10.GT22176AT10.sy01_tenant_param_config where dr = 0 and ( param like 'CQGYWms' or param like 'CQGYWMS' or param like 'CQGY')"; //租户是否开启wms
      ...
      // 拼装请求报文
      var obj = { tenantId: tenantid, logId: "yourIdHere", time: timestampStr, uuid: sign, productId: materialFileRes[0].material, userId: ObjectStore.user().id };
      //获取api前缀等信息 参数： 请求方式 ， 地址  ，header ， body
      var strResponse = postman("post", apiRestPre + "/gsp/wms/cqgyProductPush", null, JSON.stringify(obj));
    }
  }
}
exports({
