...
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValueInput26rj() {},
  formulacustomSetValueInput24ne() {}
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValueInput26rj: observable,
  formulacustomSetValueInput24ne: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_formStore_Input26rj: reaction(
      () => [rootStore.formStore.getValue("UserReferInput1uf")],
      async ([formStore_UserReferInput1uf]) => {
        debounce(
          async () => {
            ...
            const formulaResult = await countFormula(
              {
                hasFormula: "true",
                formula: "UserReferInput1uf.mobile",
                formulaFuns: "",
                formulaTriggers: "UserReferInput1uf.mobile"
              },
              {
            ...
            rootStore.formStore.setValue("Input26r", resultData);
          },
          "formula_customSetValue_formStore_Input26rj" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
    ...
    formula_customSetValue_formStore_Input24ne: reaction(
      () => [rootStore.formStore.getValue("DateTimePicker9p"), rootStore.formStore.getValue("DateTimePicker9d")],
      async ([formStore_DateTimePicker9p, formStore_DateTimePicker9d]) => {
        debounce(
          async () => {
            const allData = getAllDataByCardStore({
              cardStoreAlias: "cardStore",
            ...
            const formulaResult = await countFormula(
              {
                hasFormula: "true",
                formula: "dateDiff(DateTimePicker9p,DateTimePicker9d,4)",
                formulaFuns: "dateDiff",
                formulaTriggers: "DateTimePicker9p,DateTimePicker9d"
              },
              {
                mainDataSourceAlias: "mainEntity",
                field: "Input24n",
            ...
            //对公式结果进行处理
            const resultData = formulaResult.resultData;
            rootStore.formStore.setValue("Input24n", resultData);
          },
          "formula_customSetValue_formStore_Input24ne" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
