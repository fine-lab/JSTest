...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    let skuId = [];
    let licenceChildRes = [];
    let licenceChildAllRes = [];
    let sqwtsChildRes = [];
    let sqwtsChildAllRes = [];
    //预审单主表
        ...
        if (request.type == "证照范围") {
          ...
          if (licenceChildRes.length > 0) {
            for (let j = 0; j < licenceChildRes.length; j++) {
              licenceChildAllRes.push(licenceChildRes[j]);
              //预审单证照孙表
        ...
        if (request.type == "授权委托书范围") {
          ...
          if (sqwtsChildRes.length > 0) {
            for (let j = 0; j < sqwtsChildRes.length; j++) {
              sqwtsChildAllRes.push(sqwtsChildRes[j]);
              //预审单授权委托书孙表
    ...
    let object = { proId: proId, skuId: skuId, licenceChildRes: licenceChildAllRes, sqwtsChildRes: sqwtsChildAllRes };
    return { object };
  }
}
