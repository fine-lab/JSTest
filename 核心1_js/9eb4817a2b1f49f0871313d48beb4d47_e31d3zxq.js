viewModel.on("customInit", function (data) {
  ...
  viewModel.on("afterLoadData", function () {
    ...
    let rows = gridModelInfo.getRows();
    if (currentState == "add") {
      let rowDatas = gridModelInfo.getData();
      for (let i = 0; i < rowDatas.length; i++) {
        let product = gridModelInfo.getCellValue(i, "material");
        let batchno = gridModelInfo.getCellValue(i, "batchno");
        console.log(product);
        console.log(batchno);
        getGSPBatchnoID(product, batchno).then((batchnoRes) => {
          if (batchnoRes.length > 0) {
            gridModelInfo.setCellValue(i, "batchno", batchnoRes[0].id);
            gridModelInfo.setCellValue(i, "batchno_batchno", batchno);
          }
        });
      }
      ...
      //根据上游单据子表ID查询采购订单子表库存组织ID和名称，赋值给当前表头收货组织
      let loadOrgUrl = "GT22176AT10.gspPUR.getOrgFromOrderD";
      let requestParam = {
        orderDetailId: rows[0].sourcechild_id
      };
      let loadPromise = new cb.promise();
      invokeFunction1(
        loadOrgUrl,
        requestParam,
        function (err, res) {
          if (res) {
            viewModel.get("org_id_name").setValue(res.returnOrgName);
            viewModel.get("org_id").setValue(res.returnOrgid);
          }
          loadPromise.resolve();
        },
        { domainKey: "sy01" }
  ...
  viewModel.get("checkdep_name").on("beforeBrowse", function (data) {
    var externalData = {};
    externalData.ref_parentorgid = viewModel.get("org_id").getValue();
    (externalData.funcCode = "all"), (externalData.accountdelegate = "true"), viewModel.get("checkdep_name").setState("externalData", externalData);
  });
  //退货供应商过滤
  ...
  gridModelInfo.on("afterCellValueChange", function (data) {
    ...
    if (data.cellName == "unqualifie_qty") {
      let noQualifedResult = gridModelInfo.getCellValue(data.rowIndex, "noQualifedResult");
      if (noQualifedResult == 1 || noQualifedResult == "1") {
        gridModelInfo.setCellValue(data.rowIndex, "unQuaOutQty", data.value);
      }
    }
    if (data.cellName == "noQualifedResult") {
      let unqualifie_qty = gridModelInfo.getCellValue(data.rowIndex, "unqualifie_qty");
      if (data.value.value == 1 || data.value.value == "1") {
        gridModelInfo.setCellValue(data.rowIndex, "unQuaOutQty", unqualifie_qty);
      } else {
        gridModelInfo.setCellValue(data.rowIndex, "unQuaOutQty", 0);
      }
    }
  });
  //复制行时，检验数量不复制
  ...
  viewModel.on("beforeSave", function () {
    ...
    for (let i = 0; i < gridModelInfo.getRows().length; i++) {
      ...
      if (!qtyInfo.hasOwnProperty(sourceEntryId)) {
        ...
        qtyInfo[sourceEntryId]["checkQty"] = 0;
      ...
      if (parseFloat(gridModelInfo.getCellValue(i, "checkQty")) != 0) {
        //复核数量
        let checkQty = isNaN(parseFloat(gridModelInfo.getCellValue(i, "checkQty"))) ? 0 : parseFloat(gridModelInfo.getCellValue(i, "checkQty"));
        qtyInfo[sourceEntryId]["checkQty"] += checkQty;
      }
      //复核合格数量
    ...
    if (orgId == undefined) {
      orgId = viewModel.get("org_id").getValue();
    }
    let passwordsJson = [];
    ...
    let sourceId = gridModelInfo.getCellValue(0, "source_id");
    promises.push(validateSourceQty(gridModelInfo.getRows(), sourceId).then(handerMessage));
    ...
    Promise.all(promises).then(() => {
      ...
      for (let i = 0; i < sourceChildId.length; i++) {
        if (JSON.stringify(checkTotleQty) != "{}") {
          if (qtyInfo[sourceChildId[i]]["checkQty"] > checkTotleQty[sourceChildId[i]]) {
            state = false;
          }
        }
      }
  ...
  let validateSourceQty = function (rows, sourceId) {
    return new Promise(function (resolve) {
      let message = "";
      cb.rest.invokeFunction("GT22176AT10.publicFunction.getBillInfo", { billNo: "purchaseOrder", Ids: [sourceId] }, function (err, res) {
        if (typeof res !== "undefined") {
          let entry = res.info[0].entry;
          for (let i = 0; i < rows.length; i++) {
            for (let j = 0; j < entry.length; j++) {
              if (rows[i].sourcechild_id == entry[j].id) {
                if (rows[i].checkQty > -entry[j].qty - entry[j].extend_review_qty) {
