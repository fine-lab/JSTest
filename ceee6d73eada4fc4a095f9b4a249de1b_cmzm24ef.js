...
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValueTextArea1bi() {},
  formulacustomSetValueInput37yf() {},
  formulacustomSetValueInputNumber2bb() {}
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValueTextArea1bi: observable,
  formulacustomSetValueInput37yf: observable,
  formulacustomSetValueInputNumber2bb: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_formStore_TextArea1bi: reaction(
      () => [],
      async ([]) => {
        ...
        rootStore.formStore.setValue("TextArea1b", resultData);
      },
      {
    ...
    formula_customSetValue_formStore_Input37yf: reaction(
      () => [],
      async ([]) => {
        const allData = getPageAllData({
          rootStore,
        ...
        const resultData = result.resultData;
        console.error("formula", result);
        rootStore.formStore.setValue("Input37y", resultData);
      },
      {
        fireImmediately: formulaFireImmediately(),
        equals: function () {
    ...
    formula_customSetValue_formStore_InputNumber2bb: reaction(
      () => [rootStore.test0003_child1ListStore.getValue().length, rootStore.test0003_child1ListStore.getValue()],
      () => {
        if (!rootStore.test0003_child1ListStore.dismissStack) {
          rootStore.test0003_child1ListStore.dismissStack = [];
        }
        while (rootStore.test0003_child1ListStore.dismissStack.length) {
          rootStore.test0003_child1ListStore.dismissStack.pop();
        }
        rootStore.test0003_child1ListStore.getValue().forEach((rowStore) => {
          dismiss = reaction(
            () => [rowStore.getTrueValue("Input32s")],
            async ([test0003_child1ListStore_Input32s]) => {
              const allData = getPageAllData({
                rootStore,
                currUiStore: "formStore",
                currTableUiStore: "",
              ...
              let result = await countFormula(
                {
                  hasFormula: "",
                  formula: 'sum(test0003_child1List,"test0003_child1List.Input32s")',
                  formulaFuns: "sum",
                  formulaTriggers: "test0003_child1List,test0003_child1List.Input32s"
                },
                {
                  mainDataSourceAlias: "mainEntity",
                  field: "InputNumber2b",
