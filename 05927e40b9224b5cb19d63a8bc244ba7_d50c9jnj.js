...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    for (var i = 0; i < res.length; i++) {
      ...
      let access_token = getToken(tokenMap.get(bill.vendorId).appKey, tokenMap.get(bill.vendorId).appSecret);
      let url = configParams.gatewayUrl + "/" + relationTenantIdItem + "/product_ref/product_ref_01/batchSaveSupplierBill";
      let apiResponse = postman("post", url + "?access_token=" + access_token, null, JSON.stringify(billData));
      rrr = apiResponse;
    }
    // 更新主表状态，防止重复提交
    var returnRes = ObjectStore.updateBatch("GT37595AT2.GT37595AT2.shippingschedule", bills, "02a3de71");
    return { rrr };
    //获取token方法
