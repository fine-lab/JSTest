...
viewModel.get("button44uj") &&
  viewModel.get("button44uj").on("click", function (data) {
    ...
    if (rows[0].createUdiNum > rows[0].maxReleaseNum - rows[0].alreadyReleaseNum) {
      cb.utils.alert("生成数量不能超过最大发布数量，可生成数量为：" + (rows[0].maxReleaseNum - rows[0].alreadyReleaseNum <= 0 ? 0 : rows[0].maxReleaseNum - rows[0].alreadyReleaseNum), "error");
      return;
    }
