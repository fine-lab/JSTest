...
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValueuserdefinegroup30_ynf_02() {}
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValueuserdefinegroup30_ynf_02: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_formStore_userdefinegroup30_ynf_02: reaction(
      () => [rootStore.teces01zListStore.getValue().length, rootStore.teces01zListStore.getValue()],
      () => {
        if (!rootStore.teces01zListStore.dismissStack) {
          rootStore.teces01zListStore.dismissStack = [];
        }
        while (rootStore.teces01zListStore.dismissStack.length) {
          rootStore.teces01zListStore.dismissStack.pop();
        }
        rootStore.teces01zListStore.getValue().forEach((rowStore, tableStoreRowIndex) => {
          dismiss = reaction(
            () => [rowStore.getTrueValue("ziding1.ynf_02")],
            async ([teces01zListStore_ziding1_ynf_02]) => {
              debounce(
                async () => {
                  const allData = getAllDataByCardStore({
                    cardStoreAlias: "cardStore",
                    storeControlType: "FormStore",
                    storeField: "ziding1.ynf_02",
                    storeForm: "",
                  ...
                  const formulaResult = await countFormula(
                    {
                      hasFormula: "true",
                      formula: 'sum(teces01zList,"teces01zList.ziding1.ynf_02")',
                      formulaFuns: "sum",
                      formulaTriggers: "teces01zList,teces01zList.ziding1.ynf_02"
                    },
                    {
                      serviceCode: rootStore?.pageStore?.params?.serviceCode || rootStore?.pageStore?.query?.serviceCode || "",
                      mainDataSourceAlias: "mainEntity",
                      field: "ziding1.ynf_02",
                      billnum: "teces01a",
                  ...
                  rootStore.formStore.setValue("ziding1.ynf_02", resultData);
                },
                "formula_customSetValue_formStore_userdefinegroup30_ynf_02" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
                300
              )();
            },
            {
