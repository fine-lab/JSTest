...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    if (address == "yonbip_scm_storeprorecord_list" || address.indexOf("storeprorecord") > -1) {
      ...
      //销售出库单
      sql =
        "select id,id sourceautoid,mainid.id sourceid,mainid.org.name orgName,mainid.code billCode,mainid.vouchdate vouchDate,batchno,producedate" +
        ",invaliddate,qty materialNum,unit.name unitName,product.cName materialName,product.id materialId,saleStyle sellType" +
        "from st.salesout.SalesOuts  where  1=1 ";
    } else if (address == "yonbip_scm_arrivalorder_list" || address.indexOf("arrivalorder") > -1) {
      //采购到货
      sql =
        "select rowno lineno,id,id sourceautoid,mainid.id sourceid,mainid.org.name orgName,mainid.code billCode,mainid.vouchdate vouchDate,batchno,producedate,invaliddate,qty materialNum,unit.name unitName,product.cName materialName,product.id materialId,totalInQuantity from pu.arrivalorder.ArrivalOrders  where 1=1 ";
      domain = "upu";
    } else if (address == "finishedReport" || address.indexOf("finishedReport") > -1) {
      //完工报告
      sql =
        "select id,id sourceautoid,finishedReportId.id sourceid,finishedReportId.orgId.name orgName,finishedReportId.code billCode" +
        ",finishedReportId.vouchdate vouchDate,warehouseId warehouse,batchNo batchno,produceDate producedate,expirationDate invaliddate" +
        ",quantity materialNum,productId.unit.name unitName,productId.name materialName,productId materialId ,freeCharacteristics features,productId" +
        " from	po.finishedreport.FinishedReportDetail  where 1=1 ";
      domain = "productionorder";
      sql += " and finishedReportId ='" + mainId + "'";
      ...
      //销售发货
      sql =
        "select lineno,id,id sourceautoid,deliveryId.id sourceid,deliveryId.stockOrgId.name orgName,deliveryId.code billCode,deliveryId.vouchdate vouchDate" +
        ",stockId warehouse,batchNo batchno,productDate producedate,invalidDate invaliddate,subQty materialNum,masterUnitId.name unitName,productId.name materialName" +
        ",productId.detail.isSerialNoManage isSerialNoManage,productId.id materialId,orderProductType sellType,skuId productSku,skuId.code productSku_code from voucher.delivery.DeliveryDetail  where  (deliveryId.statusCode = 'PARTOUTSTOCKED' or deliveryId.statusCode = 'DELIVERING')  ";
