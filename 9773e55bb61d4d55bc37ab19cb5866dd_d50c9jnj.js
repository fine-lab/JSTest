...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    for (var i = 0; i < curPage; i++) {
      let hwUrl = funcRes.CONFIG.API_HW_MSG.URL_DOMAIN + funcRes.CONFIG.API_HW_MSG.URL_RESERVOIRIN + "?suffix_path=/" + pageSize + "/" + (i + 1);
      let startDate = getDate();
      let hwResponse = postman("post", hwUrl, JSON.stringify(hwHeader), JSON.stringify(hwBody));
      let endDate = getDate();
      // 记录日志
      try {
        let logObj = { methodName: "saveInFromS", requestParams: JSON.stringify(hwBody), requestTime: startDate, respResult: hwResponse, respTime: endDate, url: hwUrl };
      ...
      let saveFunc = extrequire("AT181E613C1770000A.backFun.saveInToYs");
