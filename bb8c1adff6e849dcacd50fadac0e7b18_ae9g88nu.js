...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    let createMaterialInfo = function (materialInfo, sourceIdProperty, batchProperty, stateProperty, stateIdProperty, qtyProperty, unqtyProperty, startdateProperty, enddateProperty, isSame, isHG) {
      let ishave = false;
      let qty = 0;
      ...
      if (!ishave) {
        ...
        newMaterialInfo.producedate = materialInfo[startdateProperty];
        newMaterialInfo.invaliddate = materialInfo[enddateProperty];
    ...
    //购进退出复核
    let ySql =
      "select sourcechild_id,id,batchID,batch_no,qualifie_qty,unqualifie_qty,noqualifiedstate,noqualifiedstateName,qualifiedstate,qualifiedstateName,noQualifedResult,manufact_date,valid_until from GT22176AT10.GT22176AT10.SY01_gjtcfh_l" +
      " where sourcechild_id = '" +
      purChildId +
    ...
    for (let i = 0; i < purOutRes.length; i++) {
      if (purOutRes[i].noQualifedResult == 1) {
        ...
        if (purOutRes[i].noqualifiedstate != purOutRes[i].qualifiedstate) {
          createMaterialInfo(purOutRes[i], "sourcechild_id", "batch_no", "qualifiedstateName", "qualifiedstate", "qualifie_qty", "unqualifie_qty", "manufact_date", "valid_until", false, true);
          createMaterialInfo(purOutRes[i], "sourcechild_id", "batch_no", "noqualifiedstateName", "noqualifiedstate", "qualifie_qty", "unqualifie_qty", "manufact_date", "valid_until", false, false);
        } else {
          createMaterialInfo(purOutRes[i], "sourcechild_id", "batch_no", "qualifiedstateName", "qualifiedstate", "qualifie_qty", "unqualifie_qty", "manufact_date", "valid_until", true, true);
        }
      } else {
        createMaterialInfo(purOutRes[i], "sourcechild_id", "batch_no", "qualifiedstateName", "qualifiedstate", "qualifie_qty", "unqualifie_qty", "manufact_date", "valid_until", false, true);
      }
