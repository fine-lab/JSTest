...
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValueDateTimePicker13pd() {},
  formulacustomSetValueDateTimePicker15ya() {},
  interactionRulecustomSetValueInput22hh() {}
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValueDateTimePicker13pd: observable,
  formulacustomSetValueDateTimePicker15ya: observable,
  interactionRulecustomSetValueInput22hh: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_formStore_DateTimePicker13pd: reaction(
      () => [],
      async ([]) => {
        debounce(
          async () => {
            ...
            rootStore.formStore.setValue("DateTimePicker13p", resultData);
          },
          "formula_customSetValue_formStore_DateTimePicker13pd" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
    ...
    formula_customSetValue_formStore_DateTimePicker15ya: reaction(
      () => [],
      async ([]) => {
        debounce(
          async () => {
            ...
            //对公式结果进行处理
            const resultData = formulaResult.resultData;
            rootStore.formStore.setValue("DateTimePicker15y", resultData);
          },
          "formula_customSetValue_formStore_DateTimePicker15ya" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
    ...
    interactionRule_customSetValue_formStore_Input22hh: reaction(
      () => [rootStore.formStore.getValue("Input22h")],
      async ([formStore_Input22h]) => {
        debounce(
          async () => {
            if (
              expressionParsing({
                leftValue: formStore_Input22h,
                operational: "eq",
                rightValue: formStore_Input22h,
                extLeftValue: {
                  bizType: "quote"
                },
                extRightValue: {
              ...
              rootStore?.formStore.setValue("Input22h", "");
            }
          },
          "interactionRule_customSetValue_formStore_Input22hh" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
