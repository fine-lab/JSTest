...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    if (res.length == 0) {
      res.push({});
      //查询原厂物料属性
      let yonql = "select productId.unit mainUnit,productId.unit.name mainUnitName,isExpiryDateManage,isBatchManage from pc.product.ProductDetail  where  productId.id = '" + request.materialId + "'";
      let materialProInfo = ObjectStore.queryByYonQL(yonql, "productcenter");
      if (materialProInfo[0].isBatchManage) {
        res[0].isBatchManage = 1;
      } else {
        res[0].isBatchManage = 0;
      }
      if (materialProInfo[0].isExpiryDateManage) {
        res[0].isExpiryDateManage = 1;
      } else {
        res[0].isExpiryDateManage = 0;
      }
      res[0].mainUnit = materialProInfo[0].mainUnit;
      res[0].mainUnitName = materialProInfo[0].mainUnitName;
      return { proLicInfo: res[0] };
    }
    let id = res[0].id;
    //主表字段
    let infoFieldsArray = [];
