...
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValuekaipiaojinedaxie5bf() {},
  formulacustomSetValuekaipiaojinehejidaxie5jf() {}
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValuekaipiaojinedaxie5bf: observable,
  formulacustomSetValuekaipiaojinehejidaxie5jf: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_formStore_kaipiaojinedaxie5bf: reaction(
      () => [rootStore.htkpxxListStore.getValue().length, rootStore.htkpxxListStore.getValue()],
      () => {
        if (!rootStore.htkpxxListStore.dismissStack) {
          rootStore.htkpxxListStore.dismissStack = [];
        }
        while (rootStore.htkpxxListStore.dismissStack.length) {
          rootStore.htkpxxListStore.dismissStack.pop();
        }
        rootStore.htkpxxListStore.getValue().forEach((rowStore) => {
          dismiss = reaction(
            () => [rowStore.getTrueValue("kaipiaojine")],
            async ([htkpxxListStore_kaipiaojine]) => {
              debounce(
                async () => {
                  // 获取当前页面所有数据
                  ...
                  let result = await countFormula(
                    {
                      hasFormula: "",
                      formula: 'sum(htkpxxList,"htkpxxList.kaipiaojine")',
                      formulaFuns: "sum",
                      formulaTriggers: "htkpxxList,htkpxxList.kaipiaojine"
                    },
                    {
                      mainDataSourceAlias: "mainEntity",
                      field: "kaipiaojinedaxie",
                      billnum: "gchttz",
                  ...
                  // 公式结束loading
                  rootStore.formStore.setValue("kaipiaojinedaxie", resultData);
                },
                "formula_customSetValue_formStore_kaipiaojinedaxie5bf" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
                300
              )();
            },
            {
    ...
    formula_customSetValue_formStore_kaipiaojinehejidaxie5jf: reaction(
      () => [rootStore.formStore.getValue("kaipiaojinedaxie")],
      async ([formStore_kaipiaojinedaxie]) => {
        debounce(
          async () => {
            // 获取当前页面所有数据
            const allData = getPageAllData({
              ...
              currTableUiStore: "",
              storeControlType: "FormStore",
              storeField: "kaipiaojinehejidaxie",
              currDataSourceAlias: "mainEntity",
              rowStore: {}
            });
            // 计算公式时,设置loading
            ...
            let result = await countFormula(
              {
                hasFormula: "",
                formula: 'convertMnyToBlock(kaipiaojinedaxie,"zh")',
                formulaFuns: "convertMnyToBlock",
                formulaTriggers: "kaipiaojinedaxie"
              },
              {
                mainDataSourceAlias: "mainEntity",
                field: "kaipiaojinehejidaxie",
                billnum: "gchttz",
                domainKey: "yourKeyHere",
                allData: allData || {}
              }
            ...
            const resultData = result.resultData;
            // 公式结束loading
            rootStore.formStore.setValue("kaipiaojinehejidaxie", resultData);
          },
          "formula_customSetValue_formStore_kaipiaojinehejidaxie5jf" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
