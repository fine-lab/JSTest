...
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValueEnumerate5df() {}
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValueEnumerate5df: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_formStore_Enumerate5df: reaction(
      () => [rootStore.formStore.getValue("UserReferInput1lh")],
      async ([formStore_UserReferInput1lh]) => {
        debounce(
          async () => {
            const allData = getAllDataByCardStore({
              cardStoreAlias: "cardStore",
              storeControlType: "FormStore",
              storeField: "Enumerate5d",
              storeForm: "",
            ...
            const formulaResult = await countFormula(
              {
                hasFormula: "true",
                formula: "UserReferInput1lh.sex",
                formulaFuns: "",
                formulaTriggers: "UserReferInput1lh.sex"
              },
              {
            ...
            rootStore.formStore.setValue("Enumerate5d", resultData);
          },
          "formula_customSetValue_formStore_Enumerate5df" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
