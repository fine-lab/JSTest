...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    if (address == "/yonbip/mfg/productionorder/list" || address.indexOf("productionorder") > -1) {
      //生产订单
      sql =
        "select *,produceDate producedate,batchNo batchno, productId.unit.name unitName,expirationDate invaliddate,quantity materialNum,orgId.name orgName,orderId.code code, orderId.vouchdate vouchdate,productId.name materialName ,quantity  materialNum  " +
