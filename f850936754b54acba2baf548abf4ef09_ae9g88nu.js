...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    for (var i = 0; i < detaillist.length; i++) {
      ...
      // 如果仓位管理 判断仓位现存量
      if (isGoodsPosition) {
        var yonql = "select currentqty from stock.currentstock.CurrentStockLocation where productsku = '" + detail.sussku + "'  and org = '" + orgId + "' and warehouse='" + detail.warehouse + "'";
        ...
        var res = ObjectStore.queryByYonQL(yonql, "ustock");
        ...
        // 非仓位管理 直接判断现存量
        var yonql = "select currentqty from stock.currentstock.CurrentStock where productsku = '" + detail.sussku + "'  and org = '" + orgId + "' and warehouse='" + detail.warehouse + "'";
        yonql += " and stockStatusDoc = '" + stockStatusDoc + "'";
        if (detail.isBatchManage == "1") {
          //逆向处理，如果不是批号管理从body中移除批号
          yonql += "and batchno = '" + detail.batch_no + "'";
