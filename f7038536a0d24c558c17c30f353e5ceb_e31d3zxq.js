...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    if (type == "material") {
      ...
      infoFieldsArray.push("medReminder"); //提醒
      infoFieldsArray.push("oldGspType"); //旧版商品分类
      infoFieldsArray.push("oldGspType.catagoryname oldGspTypeName"); //旧版商品分类
      infoFieldsArray.push("tymZjm"); //通用名助记码
      ...
      if (infoRes.length == 0) {
        ...
        let selectBGTreatmentDxSql =
          "select treatmentrangedx id,treatmentrangedx.name name,treatmentrangedx.path path from GT22176AT10.GT22176AT10.SY01_material_file_treatmentrangedx where fkid = '" + relationId + "'";
        let BGtreatmentDxRes = ObjectStore.queryByYonQL(selectBGTreatmentDxSql, "sy01");
        licInfo.treatmentdx = BGtreatmentDxRes;
    ...
    if (type == "customer") {
      ...
      otherReportFieldArray.push("reportCode"); //证件号
