function loadJsXlsx(viewModel) {
  ...
  //传入文件地址 subId:应用ID
  secScript.setAttribute("src", `/iuap-yonbuilder-runtime/opencomponentsystem/public/GT37522AT1/xlsx.core.min.js?domainKey=developplatform`);
  document.body.insertBefore(secScript, document.body.lastChild);
}
let fileInputId;
function createInputAndRemove() {
  if (fileInputId) {
    let fileInputDom = document.getElementById(fileInputId);
    fileInputDom.parentNode.removeChild(fileInputDom);
  }
  var fileInput = document.createElement("input");
  fileInputId = "yourIdHere" + new Date().getTime();
...
function exportMultiSheetExcelFile(excelArr, excelName) {
  //创建工作薄
  let wb = XLSX.utils.book_new();
  excelArr.forEach((obj) => {
    //创建sheet对象
    let sheet = XLSX.utils.json_to_sheet(obj.sheetData);
    //将sheet对象添加到工作薄web
    XLSX.utils.book_append_sheet(wb, sheet, obj.sheetName);
  });
  //将工作薄对象转化为文件进行下载
  XLSX.writeFile(wb, excelName + ".xlsx");
}