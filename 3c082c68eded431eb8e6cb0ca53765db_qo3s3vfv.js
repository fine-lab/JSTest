...
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValueMobile1li() {},
  formulacustomSetValueDateTimePicker8bi() {},
  formulacustomSetValueInput23bf() {}
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValueMobile1li: observable,
  formulacustomSetValueDateTimePicker8bi: observable,
  formulacustomSetValueInput23bf: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_formStore_Mobile1li: reaction(
      () => [rootStore.formStore.getValue("UserReferInput0nb")],
      async ([formStore_UserReferInput0nb]) => {
        debounce(
          async () => {
            ...
            const formulaResult = await countFormula(
              {
                hasFormula: "true",
                formula: "UserReferInput0nb.mobile",
                formulaFuns: "",
                formulaTriggers: "UserReferInput0nb.mobile"
              },
              {
            ...
            rootStore.formStore.setValue("Mobile1l", resultData);
          },
          "formula_customSetValue_formStore_Mobile1li" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
    ...
    formula_customSetValue_formStore_DateTimePicker8bi: reaction(
      () => [],
      async ([]) => {
        debounce(
          async () => {
            ...
            //对公式结果进行处理
            const resultData = formulaResult.resultData;
            rootStore.formStore.setValue("DateTimePicker8b", resultData);
          },
          "formula_customSetValue_formStore_DateTimePicker8bi" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
    ...
    formula_customSetValue_formStore_Input23bf: reaction(
      () => [rootStore.formStore.getValue("DateTimePicker10w"), rootStore.formStore.getValue("DateTimePicker8b")],
      async ([formStore_DateTimePicker10w, formStore_DateTimePicker8b]) => {
        debounce(
          async () => {
            const allData = getAllDataByCardStore({
              cardStoreAlias: "cardStore",
            ...
            const formulaResult = await countFormula(
              {
                hasFormula: "true",
                formula: "dateDiff(DateTimePicker10w,DateTimePicker8b,4)",
                formulaFuns: "dateDiff",
                formulaTriggers: "DateTimePicker10w,DateTimePicker8b"
              },
              {
                mainDataSourceAlias: "mainEntity",
                field: "Input23b",
            ...
            //对公式结果进行处理
            const resultData = formulaResult.resultData;
            rootStore.formStore.setValue("Input23b", resultData);
          },
          "formula_customSetValue_formStore_Input23bf" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
