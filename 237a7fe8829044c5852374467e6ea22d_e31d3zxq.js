run = function (event) {
  ...
  viewModel.get("material_name").on("beforeBrowse", function (data) {
    ...
    Promise.all(promises).then(
      () => {
        let proId = [];
        if (huopinRes.length == 0 || proIdRes.length == 0) {
          proId.push("-1");
        }
        for (let j = 0; j < huopinRes.length; j++) {
          for (let i = 0; i < proIdRes.length; i++) {
            if (huopinRes[j] == proIdRes[i]) {
  ...
  function getProAllList() {
    return new Promise((resolve) => {
      cb.rest.invokeFunction("ISY_2.backOpenApiFunction.getProAllList", {}, function (err, res) {
        let data;
        if (typeof res != "undefined") {
