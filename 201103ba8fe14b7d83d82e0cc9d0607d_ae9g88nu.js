  ...
  execute(request) {
        ...
        if (batchNo !== null && typeof batchNo != "undefined") {
          ...
          if (typeof mainCurRes != "undefined" && mainCurRes != null) {
            ...
            if (count < 1) {
              ...
              currentqty.push({ materialId: materialId, batchNo: batchNo, currentqty: merchantQtyInfo[batch].currentqty });
            } else if (count > 0) {
    ...
    for (let i = 0; i < proIdList.length; i++) {
      for (let j = 0; j < batchNoList.length; j++) {
        ...
        if (typeof batchInfoList.batchno != "undefined" && proIdList[i] == batchInfoList.product && batchNoList[j] == batchInfoList.batchno) {
    ...
    if (typeof recordList != "undefined" && recordList != null) {
      for (let j = 0; j < recordList.length; j++) {
        if (typeof recordList[j].invaliddate != "undefined" && typeof proList != "undefined" && proList != null) {
          let endDate = recordList[j].invaliddate;
          let date = new Date(endDate);
          let nowDate = new Date();
          let difValue = Math.floor((date - nowDate) / (1000 * 60 * 60 * 24));
          if (typeof endDate != "undefined" && difValue >= 30) {
            if (apiResponseProduct.length > 0) {
              for (let k = 0; k < proList.length; k++) {
                for (let l = 0; l < apiResponseProduct.length; l++) {
                  if (apiResponseProduct[l].material == proList[k].id && recordList[j].product == proList[k].id && apiResponseProduct[l].curingTypeName == "一般养护") {
                    let curQty;
                    for (let i = 0; i < currentqty.length; i++) {
                      if (apiResponseProduct[l].material == currentqty[i].materialId && recordList[j].batchno == currentqty[i].batchNo) {
                        curQty = currentqty[i].currentqty;
                        break;
                      }
                    }
                    qtyBatchno.push(recordList[j].batchno);
                    apiResponseProduct[l].batchNo = recordList[j].batchno;
                    apiResponseProduct[l].currentqty = curQty;
