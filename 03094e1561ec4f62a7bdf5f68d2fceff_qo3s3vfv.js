...
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValueInputNumber10ze() {}
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValueInputNumber10ze: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_formStore_InputNumber10ze: reaction(
      () => [rootStore.formStore.getValue("InputNumber6s"), rootStore.formStore.getValue("InputNumber8n")],
      async ([formStore_InputNumber6s, formStore_InputNumber8n]) => {
        debounce(
          async () => {
            // 获取当前页面所有数据
            ...
            let result = await countFormula(
              {
                hasFormula: "",
                formula: "InputNumber6s-InputNumber8n",
                formulaFuns: "",
                formulaTriggers: "InputNumber6s,InputNumber8n"
              },
              {
                mainDataSourceAlias: "mainEntity",
                field: "InputNumber10z",
                billnum: "cldazmz",
            ...
            // 公式结束loading
            rootStore.formStore.setValue("InputNumber10z", resultData);
          },
          "formula_customSetValue_formStore_InputNumber10ze" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
