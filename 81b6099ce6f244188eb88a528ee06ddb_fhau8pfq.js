  ...
  execute(request) {
    ...
    for (var i = 0; i < res.length; i++) {
      ...
      if (visitDate != null && details != null && details.length > 0 && visitDays > 1) {
        ...
        var durationDate = formatTimeToYYYYMMDD(visitDate, visitDays);
    ...
    for (var i = 0; i < conditionResult.length; i++) {
      ...
      for (var j = 0; j < details.length; j++) {
        ...
        //发送短信
        sendMessage(phone, randomNumbers[j], visitDate);
      }
    }
    //格式化时间
    ...
    function sendMessage(phone, code, date) {
      ...
      var content =
        ...
        "月" +
        day +
        "日在B公司接待大堂 [绿区]大门保安亭办理接待手续。" +
        "如您进入办公区，请勿在办公区域内拍照、摄像、录音，除有“吸烟区”标识的区域外，其它区域均禁烟，祝您来访愉快。" +
        "疫期请持24小时核酸检测阴性结果并配合保安登记、测温等工作，并自行配戴口罩，在接待人全程陪同下入园。" +
        "应防疫需要，保安有权拒绝有风险的访客入园，敬请配合。" +
        "【注：本区域暂不支持访客自助通行，请您联系接待人予以接待。】";
