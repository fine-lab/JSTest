...
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValueInputNumber6pd() {},
  formulacustomSetValueInputNumber10pd() {}
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValueInputNumber6pd: observable,
  formulacustomSetValueInputNumber10pd: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_formStore_InputNumber6pd: reaction(
      () => [rootStore.formStore.getValue("InputNumber2k"), rootStore.formStore.getValue("InputNumber4w")],
      async ([formStore_InputNumber2k, formStore_InputNumber4w]) => {
        debounce(
          async () => {
            // 获取当前页面所有数据
            ...
            let result = await countFormula(
              {
                hasFormula: "",
                formula: "InputNumber2k*InputNumber4w",
                formulaFuns: "",
                formulaTriggers: "InputNumber2k,InputNumber4w"
              },
              {
          ...
          "formula_customSetValue_formStore_InputNumber6pd" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
    ...
    formula_customSetValue_formStore_InputNumber10pd: reaction(
      () => [rootStore.formStore.getValue("InputNumber6p"), rootStore.formStore.getValue("InputNumber8s")],
      async ([formStore_InputNumber6p, formStore_InputNumber8s]) => {
        debounce(
          async () => {
            // 获取当前页面所有数据
            const allData = getPageAllData({
            ...
            let result = await countFormula(
              {
                hasFormula: "",
                formula: "InputNumber6p-InputNumber8s",
                formulaFuns: "",
                formulaTriggers: "InputNumber6p,InputNumber8s"
              },
              {
                mainDataSourceAlias: "mainEntity",
                field: "InputNumber10p",
                billnum: "xsrbsb",
                domainKey: "yourKeyHere",
                allData: allData || {}
              }
            ...
            const resultData = result.resultData;
            // 公式结束loading
            rootStore.formStore.setValue("InputNumber10p", resultData);
          },
          "formula_customSetValue_formStore_InputNumber10pd" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
