  ...
  execute(request) {
    ...
    if (address == "productionorder_list" || address.indexOf("productionorder") > -1) {
      ...
      sql =
        ...
        "qty materialNum,unit.name unitName,product materialId,product.cCode materialCode,product.cName materialName,product.id materialId " +
        ",productskun materialSku,productskun.code materialSkuCode  from st.purinrecord.PurInRecords " +
      ...
      sql =
        ...
        "materialNum,product.oUnitId.name unitName,product materialId,product.cCode materialCode,product.cName materialName,product.id materialId" +
        ",productsku materialSku,productsku.cCode materialSkuCode from st.storeprorecord.StoreProRecords where " +
      ...
      //完工报告
      sql =
        "select id,finishedReportId.id mainId,finishedReportId.orgId.name orgName,finishedReportId.code billCode,finishedReportId.vouchdate vouchDate" +
        ",batchNo batchno,produceDate producedate,expirationDate invaliddate,quantity materialNum,productId.unit.name unitName,productId.name materialName" +
        ",productId materialId,productId,productId.code materialCode,freeCharacteristics features from	po.finishedreport.FinishedReportDetail  where 1=1 ";
      domain = "productionorder";
      if (baseOrg != undefined) {
        sql += " and finishedReportId.org ='" + baseOrg + "'";
      }
      sql += "order by finishedReportId.vouchdate desc";
    } else {
      let apiResponse = [];
      return { apiResponse };
    }
