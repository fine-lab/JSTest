...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    let audiYonql = "select id,org_id,code from 	GT22176AT10.GT22176AT10.SY01_firstcampcusv3 where dr = 0 and id = '" + param.data[0].id + "'";
    let auditRes = ObjectStore.queryByYonQL(audiYonql, "sy01")[0];
    let clientYonql = "select id,customer from 	GT22176AT10.GT22176AT10.sy01_customers_file where dr = 0 and firstMarketingBillno = '" + auditRes.code + "'";
    let clientFileRes = ObjectStore.queryByYonQL(clientYonql, "sy01");
    if (clientFileRes.length != 0) {
      ...
      // 拼装请求报文
      var obj = { tenantId: tenantid, logId: "yourIdHere", time: timestampStr, type: "1", uuid: sign, orgId: auditRes.org_id, clientId: clientFileRes[0].customer, userId: ObjectStore.user().id };
