...
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValueinputnumber38() {}
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValueinputnumber38: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_formStore_inputnumber38: reaction(
      () => [rootStore.ywliu01zListStore.getValue().length, rootStore.ywliu01zListStore.getValue()],
      () => {
        if (!rootStore.ywliu01zListStore.dismissStack) {
          rootStore.ywliu01zListStore.dismissStack = [];
        }
        while (rootStore.ywliu01zListStore.dismissStack.length) {
          rootStore.ywliu01zListStore.dismissStack.pop();
        }
        rootStore.ywliu01zListStore.getValue().forEach((rowStore, tableStoreRowIndex) => {
          dismiss = reaction(
            () => [rowStore.getTrueValue("shuzhi")],
            async ([ywliu01zListStore_shuzhi]) => {
              debounce(
                async () => {
                  ...
                  const formulaResult = await countFormula(
                    {
                      hasFormula: "true",
                      formula: 'sum(ywliu01zList,"ywliu01zList.shuzhi")',
                      formulaFuns: "sum",
                      formulaTriggers: "ywliu01zList,ywliu01zList.shuzhi"
                    },
                    {
                  ...
                  rootStore.formStore.setValue("shuzhi", resultData);
                },
                "formula_customSetValue_formStore_inputnumber38" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
                300
              )();
            },
            {
