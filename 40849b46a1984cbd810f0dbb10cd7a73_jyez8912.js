...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    let arrivalOrderSql = "select extendIsWms from 	pu.arrivalorder.ArrivalOrder where id ='" + param.data[0].id + "' ";
    let arrivalOrderRes = ObjectStore.queryByYonQL(arrivalOrderSql, "upu")[0];
    if (arrivalOrderRes.extendIsWms != 1 && arrivalOrderRes.extendIsWms != "1") {
      return;
    }
    ...
    // 拼装请求报文
    var obj = { tenantId: tenantid, logId: "yourIdHere", time: timestampStr, uuid: sign, arrivalOrderId: param.data[0].id, userId: ObjectStore.user().id };
    //获取api前缀等信息 参数： 请求方式 ， 地址  ，header ， body
    var strResponse = postman("post", apiRestPre + "/gsp/wmsPurinRecordPush", null, JSON.stringify(obj));
    return { strResponse };
  }
}
