  ...
  viewModel.on("afterMount", function (event) {
    ...
    viewModel.on("afterLoadData", function () {
      ...
      if (gridModel.getRows().length > 0) {
        ...
        let extendGspType = viewModel.get("extendGspType").getValue(); //gsp
        if (extendGspType) {
          for (let i = 0; i < gridModel.getRows().length; i++) {
            let sbMaterial = gridModel.getCellValue(i, "productId"); //商品
            let extendStorageCondition = gridModel.getCellValue(i, "extendStorageCondition"); //存储条件
            let extendStorageConditionName = gridModel.getCellValue(i, "extendStorageCondition_storageName"); //存储条件
            if (extendStorageCondition == null || (extendStorageCondition != null && extendStorageConditionName == null)) {
              //查询 并赋值
              invokeFunction1(
