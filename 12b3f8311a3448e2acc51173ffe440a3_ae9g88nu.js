...
viewModel.get("button41qh").setVisible(false);
if (viewModel.getParams().mode === "add" || viewModel.getParams().mode === "edit") {
  viewModel.get("button41qh").setVisible(true);
}
...
function getBsInfo(cpbsId) {
  let sqlName = "GT22176AT10.GT22176AT10.sy01_udi_product_list where zxxsdycpbs='" + cpbsId + "'";
  invokeFunction(
    "GT22176AT10.publicFunction.getCurrencyApi",
    {
      sqlTableInfo: sqlName,
      sqlCg: "sy01",
      rsEntity: "*"
    },
    function (err, res) {
...
viewModel.get("button42ae") &&
  viewModel.get("button42ae").on("click", function (data) {
    // 清空刷新--单击
