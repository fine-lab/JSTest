...
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValueInput20hj() {},
  formulacustomSetValueInputNumber5ea() {},
  formulacustomSetValueReferInput2ae_refer() {}
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValueInput20hj: observable,
  formulacustomSetValueInputNumber5ea: observable,
  formulacustomSetValueReferInput2ae_refer: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_formStore_Input20hj: reaction(
      () => [rootStore.formStore.getValue("UserReferInput0ve")],
      async ([formStore_UserReferInput0ve]) => {
        debounce(
          async () => {
            ...
            const formulaResult = await countFormula(
              {
                hasFormula: "true",
                formula: "UserReferInput0ve.mobile",
                formulaFuns: "",
                formulaTriggers: "UserReferInput0ve.mobile"
              },
              {
            ...
            rootStore.formStore.setValue("Input20h", resultData);
          },
          "formula_customSetValue_formStore_Input20hj" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
    ...
    formula_customSetValue_formStore_InputNumber5ea: reaction(
      () => [rootStore.formStore.getValue("DateTimePicker11j"), rootStore.formStore.getValue("DateTimePicker9f")],
      async ([formStore_DateTimePicker11j, formStore_DateTimePicker9f]) => {
        debounce(
          async () => {
            const allData = getAllDataByCardStore({
              cardStoreAlias: "cardStore",
              storeControlType: "FormStore",
              storeField: "InputNumber5e",
              storeForm: "",
              dataSource: "mainEntity"
            });
            const formulaResult = await countFormula(
              {
                hasFormula: "true",
                formula: "dateDiff(DateTimePicker11j,DateTimePicker9f,4)",
                formulaFuns: "dateDiff",
                formulaTriggers: "DateTimePicker11j,DateTimePicker9f"
              },
              {
                mainDataSourceAlias: "mainEntity",
                field: "InputNumber5e",
            ...
            //对公式结果进行处理
            const resultData = formulaResult.resultData;
            rootStore.formStore.setValue("InputNumber5e", resultData);
          },
          "formula_customSetValue_formStore_InputNumber5ea" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
    ...
    formula_customSetValue_formStore_ReferInput2ae_refer: reaction(
      () => [rootStore.formStore.getValue("ReferInput3uc_Input19k")],
      async ([formStore_ReferInput3uc_Input19k]) => {
        debounce(
          async () => {
            const allData = getAllDataByCardStore({
              cardStoreAlias: "cardStore",
              storeControlType: "FormStore",
              storeField: "ReferInput2ae",
              storeForm: "",
              dataSource: "mainEntity"
            });
            const formulaResult = await countFormula(
              {
                hasFormula: "true",
                formula: "ReferInput3uc.Input19k",
                formulaFuns: "",
                formulaTriggers: "ReferInput3uc.Input19k"
              },
              {
            ...
            //对公式结果进行处理
            const resultData = formulaResult.resultData;
            rootStore.formStore.setValue("ReferInput2ae", resultData);
          },
          "formula_customSetValue_formStore_ReferInput2ae_refer" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
