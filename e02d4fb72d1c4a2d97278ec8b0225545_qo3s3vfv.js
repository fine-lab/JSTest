...
// 计算属性代码监听开始
let CSDD_child1ListStoreExtendRowStore = RowStore;
if (rootStore.CSDD_child1ListStore.extendRowStore) {
  CSDD_child1ListStoreExtendRowStore = rootStore.CSDD_child1ListStore.extendRowStore;
}
class CSDD_child1ListStoreRowStore extends CSDD_child1ListStoreExtendRowStore {
  constructor() {
    super(...arguments);
    this.formulacustomSetValueInput37rj = undefined;
    makeObservable(this, {
      formulacustomSetValueInput37rj: observable
    });
  }
}
rootStore.CSDD_child1ListStore.extendRowStore = CSDD_child1ListStoreRowStore;
// 扩展新的store
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_CSDD_child1ListStore_Input37rj: reaction(
      () => [rootStore.CSDD_child1ListStore.getValue().length, rootStore.CSDD_child1ListStore.getValue()],
      () => {
        if (!rootStore.CSDD_child1ListStore.dismissStack) {
          rootStore.CSDD_child1ListStore.dismissStack = [];
        }
        while (rootStore.CSDD_child1ListStore.dismissStack.length) {
          rootStore.CSDD_child1ListStore.dismissStack.pop();
        }
        rootStore.CSDD_child1ListStore.getValue().forEach((rowStore, tableStoreRowIndex) => {
          dismiss = reaction(
            () => [rowStore.getTrueValue("ReferInput1fk")],
            async ([CSDD_child1ListStore_ReferInput1fk]) => {
              debounce(
                async () => {
                  const allData = getAllDataByCardStore({
                    ...
                    storeField: "Input37r",
                    storeForm: "",
                    dataSource: "CSDD_child1List"
                  });
                  const formulaResult = await countFormula(
                    {
                      hasFormula: "true",
                      formula: "CSDD_child1List.ReferInput1fk.unit.name",
                      formulaFuns: "",
                      formulaTriggers: "CSDD_child1List.ReferInput1fk.unit.name"
                    },
                    {
                      mainDataSourceAlias: "mainEntity",
                      field: "CSDD_child1List.Input37r",
                      billnum: "CSDD",
                  ...
                  rowStore.setValue("Input37r", resultData);
                },
                "formula_customSetValue_CSDD_child1ListStore_Input37rj" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
                300
              )();
            },
            {
