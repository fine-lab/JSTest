...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    let yonql = "select featureId,featureName,featureCode,gmpControl from GT22176AT10.GT22176AT10.sy01_freeFeature where drugFeatrueConfig_id.template = '" + template + "'";
    ...
    for (let i = 0; i < controlRes.length; i++) {
      if (controlRes[i].gmpControl == true && (!feature.hasOwnProperty(controlRes[i].featureCode) || feature[controlRes[i].featureCode] == undefined)) {
    ...
    let featureCode;
    for (let i = 0; i < controlRes.length; i++) {
      if (controlRes[i].gmpControl == true) {
        featureCode = controlRes[i].featureCode;
        extendStr += "freeFeatureGroup." + featureCode + " = '" + feature[featureCode] + "' and ";
      }
    }
    extendStr = extendStr.substring(0, extendStr.length - 4);
    let isCampYonql = "select id from ISY_2.ISY_2.SY01_supply_pre_hear_sheet where gmpProPerHear = 2 and org_id = '" + orgId + "' and productCode = '" + materialId + "' and " + extendStr;
    let isCampRes = ObjectStore.queryByYonQL(isCampYonql);
    if (isCampRes.length > 0) {
      return { code: 1001, message: "该自由特征组中，药品预审敏感特征已进行过预审" };
    } else {
      return { code: 1002, message: "该自由特征组，药品预审敏感特征未进行过预审" };
    }
  }
}
