...
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValueUserdefinegroup1rc_zy120501() {}
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValueUserdefinegroup1rc_zy120501: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_formStore_Userdefinegroup1rc_zy120501: reaction(
      () => [rootStore.formStore.getValue("feature_zy120502_name"), rootStore.formStore.getValue("feature.zy120503")],
      async ([formStore_feature_zy120502_name, formStore_feature_zy120503]) => {
        debounce(
          async () => {
            const allData = getAllDataByCardStore({
              cardStoreAlias: "cardStore",
              storeControlType: "FormStore",
              storeField: "feature.zy120501",
              storeForm: "",
              dataSource: "clm_contract"
            });
            const formulaResult = await countFormula(
              {
                hasFormula: "true",
                formula: "feature.zy120502.name+feature.zy120503.name",
                formulaFuns: "",
                formulaTriggers: "feature.zy120502.name,feature.zy120503.name"
              },
              {
                serviceCode: rootStore?.pageStore?.params?.serviceCode || rootStore?.pageStore?.query?.serviceCode || "",
                mainDataSourceAlias: "clm_contract",
                field: "feature.zy120501",
                billnum: "clmContract",
            ...
            rootStore.formStore.setValue("feature.zy120501", resultData);
          },
          "formula_customSetValue_formStore_Userdefinegroup1rc_zy120501" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
