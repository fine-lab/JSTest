...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    if (address == "/yonbip/scm/storeprorecord/list" || address.indexOf("storeprorecord") > -1) {
      ...
      sql =
        "select id,mainid mainId, mainid.code billCode,mainid.vouchdate vouchDate,batchno,producedate,invaliddate,qty materialNum,priceUOM.name unitName,product.cName materialName,product.id materialId,finishoutqty  from st.transferapply.TransferApplys where 1=1";
    } else if (address === "dcck") {
      sql =
        "select id,mainid mainId, mainid.code billCode,mainid.vouchdate vouchDate,batchno,producedate,invaliddate,qty materialNum,product.oUnitId.name unitName,product.cName materialName,product.id materialId,finishinqty  from st.storeout.StoreOutDetail where 1=1";
    }
    if (startDate != undefined && startDate != "" && startDate != null) {
