  ...
  viewModel.get("button43lh").on("click", function (data) {
    ...
    cb.rest.invokeFunction(
      "I0P_UDI.publicFunction.shareApi",
      {
        //传入参数 sqlType：类型
        sqlType: "check",
        sqlTableInfo: "select * from I0P_UDI.I0P_UDI.sy01_country_interface_datav3 where dr = 0",
        sqlCg: "isv-ud1"
      },
      function (err, res) {
...
function readWorkbookFromLocalFile(file) {
  ...
  reader.onload = function (e) {
    ...
    try {
      ...
      //四个list字段
      // 获取配件详情下表
      var qylxxxList = viewModel.get("sy01_udi_product_list_qylxxxv3List"); //表格-UDI企业联系信息
      var lcsyxxList = viewModel.get("sy01_udi_product_list_lcsyxxv3List"); //UDI临床使用尺寸信息
      var ccczxxList = viewModel.get("sy01_udi_product_list_ccczxxv3List"); //UDI储存或操作信息
      var bzbsxxList = viewModel.get("sy01_udi_product_list_bzbsxxv3List"); //表格-UDI包装标识信息
      // 清空下表数据
      qylxxxList.clear();
      lcsyxxList.clear();
      ccczxxList.clear();
      ...
      if (packageNum === 0 && xmlDoc.getElementsByTagName("packingInfoList") != null && typeof xmlDoc.getElementsByTagName("packingInfoList") != "undefined") {
        packageNum = xmlDoc.getElementsByTagName("packingInfo").length;
      }
      if (packageNum > 0) {
        //如果都为空，则启动默认最小，如果长度大于1，则默认添加一个最小包装标识
        for (let pi = 0; pi < packageNum; pi++) {
          try {
            let bznhxyjbzcpbs = "";
            try {
              bznhxyjbzcpbs = xmlDoc.getElementsByTagName("bznhxyjbzcpbs")[pi].childNodes[0].nodeValue;
            } catch (qy_e) {
              console.error("表格-UDI包装标识信息bznhxyjbzcpbs -error-" + qy_e);
            }
            let bzcpbs = "";
            try {
              bzcpbs = xmlDoc.getElementsByTagName("bzcpbs")[pi].childNodes[0].nodeValue;
            } catch (qy_e) {
              console.error("表格-UDI包装标识信息bzcpbs-error-" + qy_e);
            }
            let cpbzjb = "";
            try {
              cpbzjb = xmlDoc.getElementsByTagName("cpbzjb")[pi].childNodes[0].nodeValue;
            } catch (qy_e) {
              console.error("表格-UDI包装标识信息cpbzjb-error-" + qy_e);
            }
            let bznhxyjcpbssl = "";
            try {
              bznhxyjcpbssl = xmlDoc.getElementsByTagName("bznhxyjcpbssl")[pi].childNodes[0].nodeValue;
            } catch (qy_e) {
...
let invokeFunction = function (id, data, callback, options) {
...
viewModel.on("beforeSave", function (data) {
  //如果是添加 查重 提示去修改中更新
  let bzbsxxList = viewModel.get("sy01_udi_product_list_bzbsxxv3List"); //表格-UDI包装标识信息
  if (bzbsxxList.getData().length == 0) {
    cb.utils.alert("包装标识信息不能为空！");
    return false;
  }
...
function getBsInfo(cpbsId) {
  let sqlInfo = "select * from I0P_UDI.I0P_UDI.sy01_udi_product_listv3 where zxxsdycpbs='" + cpbsId + "'";
  invokeFunction(
    "I0P_UDI.publicFunction.shareApi",
    {
      sqlType: "check",
