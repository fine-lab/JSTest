viewModel.on("customInit", function (data) {
  // 线索列表--页面初始化
  // 处理智齿传入线索 是否有效字段为编码的问题
  viewModel.getGridModel().setColumnState("headDef!define1", "formatter", function (rowInfo, rowData) {
    if (rowData["headDef!define1"] == "01") {
      return {
        override: true,
        html: `<p style='margin-left: 12px;'>是<p>`
      };
    } else if (rowData["headDef!define1"] == "02") {
      return {
        override: true,
        html: `<p style='margin-left: 12px;'>否<p>`
      };
    } else if (rowData["headDef!define1"] == "03") {
      return {
        override: true,
        html: `<p style='margin-left: 12px;'>继续跟进<p>`
      };
    }
  });
});
