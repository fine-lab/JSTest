...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    let audiYonql = "select id,org_id,code from 		GT22176AT10.GT22176AT10.SY01_fccompauditv4 where dr = 0 and id = '" + param.data[0].id + "'";
    let auditRes = ObjectStore.queryByYonQL(audiYonql, "sy01")[0];
    let clientYonql = "select id,supplier from 	GT22176AT10.GT22176AT10.SY01_supplier_file where dr = 0 and firstBusinessOrderNo = '" + auditRes.code + "'";
    ...
    if (clientFileRes.length != 0) {
      ...
      // 拼装请求报文
      var obj = { tenantId: tenantid, logId: "yourIdHere", time: timestampStr, type: "2", uuid: sign, orgId: auditRes.org_id, clientId: clientFileRes[0].supplier, userId: ObjectStore.user().id };
