...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    for (let [key, value] of tokenMap) {
      let access_token = getToken(tokenMap.get(key).appKey, tokenMap.get(key).appSecret);
      let url = configParams.gatewayUrl + "/" + tokenMap.get(key).appTenantId + "/product_ref/product_ref_01/batchUpdStatus";
      let apiResponse = postman("post", url + "?access_token=" + access_token, null, JSON.stringify({ whereStr: whereStr, opeStatusCode: "1", opeStatusName: "已确认" }));
      rrr = apiResponse;
    ...
    var returnRes = ObjectStore.updateBatch("GT37595AT2.GT37595AT2.shippingschedule", updateArr, "02a3de71");
    return { rrr };
    //获取token方法
