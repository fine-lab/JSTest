...
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValuekaipiaojinehejidaxie3bd() {},
  formulacustomSetValueInput60pf() {}
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValuekaipiaojinehejidaxie3bd: observable,
  formulacustomSetValueInput60pf: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_formStore_kaipiaojinehejidaxie3bd: reaction(
      () => [rootStore.htkpxx1ListStore.getValue().length, rootStore.htkpxx1ListStore.getValue()],
      () => {
        if (!rootStore.htkpxx1ListStore.dismissStack) {
          rootStore.htkpxx1ListStore.dismissStack = [];
        }
        while (rootStore.htkpxx1ListStore.dismissStack.length) {
          rootStore.htkpxx1ListStore.dismissStack.pop();
        }
        rootStore.htkpxx1ListStore.getValue().forEach((rowStore, tableStoreRowIndex) => {
          dismiss = reaction(
            () => [rowStore.getTrueValue("kaipiaojine")],
            async ([htkpxx1ListStore_kaipiaojine]) => {
              debounce(
                async () => {
                  ...
                  const formulaResult = await countFormula(
                    {
                      hasFormula: "true",
                      formula: 'sum(htkpxx1List,"htkpxx1List.kaipiaojine")',
                      formulaFuns: "sum",
                      formulaTriggers: "htkpxx1List,htkpxx1List.kaipiaojine"
                    },
                    {
                ...
                "formula_customSetValue_formStore_kaipiaojinehejidaxie3bd" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
                300
              )();
            },
            {
    ...
    formula_customSetValue_formStore_Input60pf: reaction(
      () => [rootStore.htkpxx1ListStore.getValue().length, rootStore.htkpxx1ListStore.getValue()],
      () => {
        if (!rootStore.htkpxx1ListStore.dismissStack) {
          rootStore.htkpxx1ListStore.dismissStack = [];
              ...
              debounce(
                async () => {
                  ...
                  const formulaResult = await countFormula(
                    {
                      hasFormula: "true",
                      formula: 'convertMnyToBlock(htkpxx1List.kaipiaojine,"zh")',
                      formulaFuns: "convertMnyToBlock",
                      formulaTriggers: "htkpxx1List.kaipiaojine"
                    },
                    {
                      mainDataSourceAlias: "mainEntity",
                      field: "Input60pf",
                  ...
                  //对公式结果进行处理
                  const resultData = formulaResult.resultData;
                  rootStore.formStore.setValue("Input60pf", resultData);
                },
                "formula_customSetValue_formStore_Input60pf" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
                300
              )();
            },
            {
