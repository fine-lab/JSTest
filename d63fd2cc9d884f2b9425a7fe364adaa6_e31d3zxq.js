viewModel.on("customInit", function (data) {
  ...
  yhtj_gridModel
    .getEditRowModel()
    .get("SY01_filtercondit_field_id_fieldName")
    .on("beforeBrowse", function (model) {
      // 条件--值改变
      let is_own = viewModel.get("is_own").getValue();
      var condition = {
        isExtend: true,
        simpleVOs: []
      };
  ...
  viewModel.get("is_own").on("afterValueChange", function (data) {
    yhtj_gridModel.clear();
  });
