...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    for (let i = 0; i < gmpInfoArray.length; i++) {
      if (org_id == gmpInfoArray[i].org_id && gmpInfoArray[i].isSupPreqLicenManage == "0") {
        isChild = true;
      }
    }
    ...
    if (typeof baseInfo.productTemplate != "undefined" && baseInfo.productTemplate != null) {
      //获取物料模板信息
      let sql = "select name from pc.tpl.ProductTpl where id=" + baseInfo.productTemplate;
      var productTplRes = ObjectStore.queryByYonQL(sql, "productcenter");
      let productTplInfo = productTplRes[0];
      baseInfo.productTemplate_name = productTplInfo.name;
    } else {
      baseInfo.productTemplate_name = undefined;
    }
    if (isChild) {
      return { data: baseInfo };
    }
      ...
      if (licenceRes.length == 0) {
        ...
        for (let l = 0; l < licenceRes[0].licenceChildRes.length; l++) {
          ...
          for (let k = 0; k < baseInfo.license[j].range.length; k++) {
            ...
            } else if (licenceRes[0].licenceChildRes[l].authType == "5" && licenceRes[0].licenceChildRes[l].authType == baseInfo.license[j].range[k].authType) {
              baseInfo.license[j].range[k].authType = "5";
              let authFeatures = baseInfo.license[j].range[k].authFeatures;
              baseInfo.license[j].range[k].authFeatures = authFeatures;
            }
          }
        }
      }
    ...
    for (let j = 0; j < baseInfo.attorney.length; j++) {
      ...
      for (let l = 0; l < licenceRes[0].licenceChildRes.length; l++) {
        ...
        for (let k = 0; k < baseInfo.attorney[j].range.length; k++) {
          ...
          } else if (licenceRes[0].licenceChildRes[l].authType == "5" && licenceRes[0].licenceChildRes[l].authType == baseInfo.attorney[j].range[k].authType) {
            baseInfo.attorney[j].range[k].authType = "5";
            let authFeatures = baseInfo.attorney[j].range[k].authFeatures;
            baseInfo.attorney[j].range[k].authFeatures = authFeatures;
          }
        }
      }
    }
