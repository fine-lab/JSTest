      ...
      if (excelData && excelData.length > 2) {
        ...
        if (allRepeatSN.length > 0) {
          ...
          cb.rest.invokeFunction("GT37522AT1.fixedAssetsFunc.batchCheckSn", { snValues: [...allBillSN], billCodes: [...allBillCode] }, function (err, res) {
            ...
            if (res && res.isPass) {
              let pageSize = 50;
              let pageNum = (allBillCode.size - 1) / pageSize + 1;
              let billCodeArray = [...allBillCode];
              for (let currPage = 1; pos <= pageNum; pos++) {
                let start = (currPage - 1) * pageSize;
                let end = currPage * pageSize;
                end = end > billCodeArray.length ? billCodeArray.length : end;
                let tempCodeArray = billCodeArray.size(start, end);
                cb.rest.invokeFunction("GT37522AT1.fixedAssetsFunc.batchUpdateSN", { envUrl: serviceUrl, billCodes: tempCodeArray, codeToSN: codeToSN }, function (err, res) {
