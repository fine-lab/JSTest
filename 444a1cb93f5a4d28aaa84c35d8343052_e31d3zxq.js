...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    for (let i = 0; i < billInfo.length; i++) {
      ...
      orgId[billInfo[i].id] = billInfo[i].accountOrg;
    }
    ...
    let productInfo = {};
    let materialSql = "select * from GT22176AT10.GT22176AT10.SY01_material_file where material in (" + str_products + ")";
    ...
    if (typeof materialInfo != "undefined" && materialInfo != null) {
      for (let i = 0; i < materialInfo.length; i++) {
        if (!productInfo.hasOwnProperty(materialInfo[i].mainid)) {
          productInfo[materialInfo[i].material] = materialInfo[i];
        }
      }
    }
    let salereturnInfo = {};
    ...
    for (let i = 0; i < billChildInfo.length; i++) {
      if (productInfo[billChildInfo[i].product].org_id == orgId[billChildInfo[i].mainid]) {
