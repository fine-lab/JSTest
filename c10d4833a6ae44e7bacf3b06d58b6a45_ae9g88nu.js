run = function (event) {
          ...
          promises.push(
            getQualifiedVendor(vendorArrays[i].id).then((vendorLicenseInfo) => {
              for (let j = 0; j < vendorLicenseInfo.length; j++) {
                if (
                  (typeof vendorLicenseInfo[j].extend_license != "undefined" && typeof vendorLicenseInfo[j].extend_license_code != "undefined") ||
      ...
      cb.rest.invokeFunction("GT22176AT10.reportData.getVendors", {}, function (err, res) {
        if (typeof res !== "undefined") {
          ...
          for (let i = 0; i < vendorList.length; i++) {
            vendorArrays.push({ id: vendorList[i].id, code: vendorList[i].code, name: vendorList[i].name, extend_is_carrier: vendorList[i].extend_is_carrier });
          }
        } else if (err !== null) {
          alert(err.message);
        }
  ...
  let prev = viewModel.get("button5oj");
  ...
  //下一页
  let next = viewModel.get("button11nj");
