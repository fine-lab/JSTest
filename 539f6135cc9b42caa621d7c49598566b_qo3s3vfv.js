...
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValueInput31fg() {},
  formulacustomSetValueInput20fh() {}
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValueInput31fg: observable,
  formulacustomSetValueInput20fh: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_formStore_Input31fg: reaction(
      () => [rootStore.formStore.getValue("UserReferInput3df")],
      async ([formStore_UserReferInput3df]) => {
        debounce(
          async () => {
            ...
            const formulaResult = await countFormula(
              {
                hasFormula: "true",
                formula: "UserReferInput3df.mobile",
                formulaFuns: "",
                formulaTriggers: "UserReferInput3df.mobile"
              },
              {
            ...
            rootStore.formStore.setValue("Input31f", resultData);
          },
          "formula_customSetValue_formStore_Input31fg" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
    ...
    formula_customSetValue_formStore_Input20fh: reaction(
      () => [rootStore.formStore.getValue("DateTimePicker22w"), rootStore.formStore.getValue("DateTimePicker20n")],
      async ([formStore_DateTimePicker22w, formStore_DateTimePicker20n]) => {
        debounce(
          async () => {
            const allData = getAllDataByCardStore({
              cardStoreAlias: "cardStore",
            ...
            const formulaResult = await countFormula(
              {
                hasFormula: "true",
                formula: "dateDiff(DateTimePicker22w,DateTimePicker20n,4)",
                formulaFuns: "dateDiff",
                formulaTriggers: "DateTimePicker22w,DateTimePicker20n"
              },
              {
                mainDataSourceAlias: "mainEntity",
                field: "Input20f",
          ...
          "formula_customSetValue_formStore_Input20fh" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
