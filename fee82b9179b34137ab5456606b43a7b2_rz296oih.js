cb.defineInner([], function () {
  var MyExternal = {
    resolveSnImport(params, viewModel) {
      ...
      let detailLists = viewModel.getGridModel("deliveryDetails").getData();
      for (let i in detailLists) {
        let item = detailLists[i];
        if (item.snSalesMessageList) {
          snProductCodeMap.set(item.productCode, item.snSalesMessageList.length);
          for (let j in item.snSalesMessageList) {
            if (!snSnMap.has(item.snSalesMessageList[j].extendSn)) {
              snSnMap.set(item.snSalesMessageList[j].extendSn, 1);
            }
          }
        } else {
