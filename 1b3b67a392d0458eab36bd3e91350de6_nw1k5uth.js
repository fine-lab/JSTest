...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    let configParamsFun = extrequire("GT39325AT4.commonFun.configParamsFun");
    let configParams = configParamsFun.execute();
    ...
    //获取采购商信息
    let cgSql = "select appKey,appSecret,appTenantId from GT2935AT1.GT2935AT1.purchaseApiMessage ";
    var tokenRes = ObjectStore.queryByYonQL(cgSql);
    const tokenMap = new Map();
    for (var w = 0; w < tokenRes.length; w++) {
      var tokenItem = tokenRes[w];
      tokenMap.set(tokenItem.appTenantId, { appKey: tokenItem.appKey, appSecret: tokenItem.appSecret });
    }
    if (res && res.length > 0) {
      let rrr;
      let updateArr = [];
      let zongRongTenant = res[0].purchaser_tenant_id;
      let errMsg = "";
      //设置中融租户id map ，中融多采购商处理 lixhui
      let zongRongTenantmap = new Map();
      for (var j = 0; j < res.length; j++) {
        let zongRongarr = [];
        if (zongRongTenantmap.has(res[j].purchaser_tenant_id)) {
          zongRongarr = zongRongTenantmap.get(res[j].purchaser_tenant_id);
        }
        zongRongarr.push(res[j]);
        zongRongTenantmap.set(res[j].purchaser_tenant_id, zongRongarr);
      }
      let apiResponse;
      if (zongRongTenantmap && zongRongTenantmap.size > 0) {
        zongRongTenantmap.forEach(function (reslist, zrkey) {
          //多供应商处理
          for (var j = 0; j < reslist.length; j++) {
            let updateItem = {};
            let updateChildArr = [];
            let allSubmit = true;
            updateItem["id"] = reslist[j].id;
            reslist[j].id = reslist[j].source_id;
            delete reslist[j].pubts;
            delete reslist[j].source_id;
            delete reslist[j].tenant_id;
            var suppliershippingschedulebList = reslist[j].suppliershippingschedulebList;
            if (suppliershippingschedulebList && suppliershippingschedulebList.length > 0) {
              for (var m = 0; m < suppliershippingschedulebList.length; m++) {
                let updateChildItem = {};
                updateChildItem["id"] = suppliershippingschedulebList[m].id;
                ...
                if (!suppliershippingschedulebList[m].remaining_quantity || suppliershippingschedulebList[m].remaining_quantity == "") {
                  errMsg += res[j].po_code + ",";
                }
                if (
                  suppliershippingschedulebList[m].remaining_quantity &&
                  suppliershippingschedulebList[m].remaining_quantity != "" &&
                  parseInt(suppliershippingschedulebList[m].remaining_quantity) > 0
                ) {
                  allSubmit = false;
                }
                let sended_quantitiy = suppliershippingschedulebList[m].sended_quantitiy;
                if (sended_quantitiy && sended_quantitiy != "") {
                  sended_quantitiy = parseInt(sended_quantitiy);
                } else {
                  sended_quantitiy = 0;
                }
                suppliershippingschedulebList[m].sended_quantitiy = parseInt(suppliershippingschedulebList[m].quantitiy) - parseInt(suppliershippingschedulebList[m].remaining_quantity) + "";
                updateChildItem["sended_quantitiy"] = suppliershippingschedulebList[m].sended_quantitiy;
                updateChildItem["_status"] = "Update";
                updateChildArr.push(updateChildItem);
                ...
                if (suppliershippingschedulesnList && suppliershippingschedulesnList.length > 0) {
                  for (var n = 0; n < suppliershippingschedulesnList.length; n++) {
                    suppliershippingschedulesnList[n].source_id = suppliershippingschedulesnList[n].id;
            ...
            if (allSubmit) {
              // 已全部提交
              res[j].submit_status = "4";
              updateItem["submit_status_name"] = "全部提交";
            } else {
              res[j].submit_status = "3";
              updateItem["submit_status_name"] = "部分提交";
            }
            updateItem["submit_status"] = reslist[j].submit_status;
            updateItem["_status"] = "Update";
            updateItem["suppliershippingschedulebList"] = updateChildArr;
            updateArr.push(updateItem);
            delete res[j].suppliershippingschedulebList;
          }
          if (errMsg && errMsg != "") {
            errMsg = errMsg.substring(0, errMsg.length - 1);
            throw new Error(errMsg + " 未填入发货信息，请勿提交！");
          }
          ObjectStore.updateBatch("GT39325AT4.GT39325AT4.suppliershippingschedule", updateArr, "d9c012c1List");
          let reqData = { data: reslist };
          //根据不同的采购商做不同的处理
          var access_token = getToken(tokenMap.get(zrkey).appKey, tokenMap.get(zrkey).appSecret);
          //使用-传参分别为： appkey, appsecrect
          let url = configParams.gatewayUrl + "/" + zongRongTenant + "/product_ref/product_ref_01/batchUpdateShippingBill";
          apiResponse = postman("post", url + "?access_token=" + access_token, null, JSON.stringify(reqData));
        });
      }
      return { apiResponse };
    } else {
