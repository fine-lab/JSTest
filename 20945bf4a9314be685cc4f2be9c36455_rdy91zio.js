...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    //获取项目终验单据数据
    //获取项目终验单据code
    let ys_projectEnd = param.billDO;
    //项目终验实体URL
    var projectAcceptanceUrl = "ckam.projectacceptance.ProjectAcceptance";
    var domainKey = "yourKeyHere";
    var projectAcceptanceSql = `select projectId,acceptDate from ${projectAcceptanceUrl} where id=${ys_projectEnd.id}`;
    //获取项目终验行id
    ...
    for (var i = 0; i < other.length; i++) {
      ...
      let crm_item = {
        ...
        code: ys_projectEnd.id //交付流水号     (主表id）
      };
      var rows = new Array();
      rows.push(crm_item);
      //定义crm接口数据
