viewModel.on("customInit", function (data) {
  ...
  viewModel.on("afterLoadData", function (data) {
    ...
    if (viewModel.getParams().mode == "add") {
      let gridModelInfoRows = gridModel.getRows();
      for (let m = 0; m < gridModelInfoRows.length; m++) {
        let batchno = gridModelInfoRows[m].pcnum;
        change_price_detailList.setCellValue(m, "pcnum_batchno", batch_no);
      }
