...
class MyTrigger extends AbstractTrigger {
  // 第三个参数为贷方的科目编码，第四个参数为借方的科目编码
  execute(context, resAllSubjectInfo, codeCredit, codeDebit) {
    try {
      ...
      if (currentPeriodList != null) {
        currentPeriodList.forEach((item) => {
          if (codeCredit.includes(item.accsubject_code) && item.hasOwnProperty("localcredebit2") && item.localcredebit2 != null) {
            currentPeriod = currentPeriod + item.localcredebit2;
          }
          if (codeDebit.includes(item.accsubject_code) && item.hasOwnProperty("localdebit2") && item.localdebit2 != null) {
      ...
      throw new Error("执行脚本getDirectionPub报错：" + e);
    }
  }
}
