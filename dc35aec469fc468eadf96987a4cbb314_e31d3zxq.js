viewModel.get("st_purchaseorderlist") &&
  viewModel.get("st_purchaseorderlist").on("afterSetDataSource", function (data) {
    //采购订单列表区域--设置数据源后
    //通过查询推送结果 返回填充状态 仅限九秉用户
    var gridModel = viewModel.getGridModel();
    let tid = cb.context.getTenantId();
    if ("kegf1e24" === tid) {
      var cgddList = data;
      if (cgddList.length > 0) {
        //无单据 信息
        cb.rest.invokeFunction("PU.backDesignerFunction.getPushRtv2", { cgddList }, function (err, res) {
          if (err != null) {
            console.log(res);
            cb.utils.alert("查询状态失败!请重试!" + err.message);
            return false;
          }
          //重新赋值
          for (i = 0; i < res.request.cgddList.length; i++) {
            if (res.request.cgddList[i].item1144ac != "undefined" && res.request.cgddList[i].item1144ac != null && res.request.cgddList[i].item1144ac != "") {
              viewModel.getGridModel().setCellValue(i, "item1144ac", res.request.cgddList[i].item1144ac);
              viewModel.getGridModel().setCellValue(i, "item1147hb", res.request.cgddList[i].item1147hb);
            }
          }
        });
      }
...
viewModel.on("customInit", function (data) {
  ...
  let tid = cb.context.getTenantId();
  if ("kegf1e24" === tid) {
    viewModel.get("item1144ac").setVisible(false);
    viewModel.get("item1147hb").setVisible(true);
  }
  ...
  viewModel.on("beforeBatchpush", function (data) {
    ...
    if (data.args.cCaption == "退货复核") {
      ...
      for (let i = 0; i < selectData.length; i++) {
        ...
        //判断本单据是否能下推退库单
        let request2 = { id: id, billMetaNo: "pu.purchaseorder.PurchaseOrder", entryMetaNo: "pu.purchaseorder.PurchaseOrders", entryLinkMetaNo: "mainid" };
        promises.push(validate_tk(apiUrl, request2).then(handerMessage));
        //判断自己的每行数据状态
      }
      var returnPromise = new cb.promise();
      Promise.all(promises).then(() => {
      ...
      cb.rest.invokeFunction(apiUrl, request, function (err, res) {
        ...
        if (typeof res !== "undefined") {
          ...
          for (let i = 0; i < res.entry.length; i++) {
            ...
            if (res.entry[i].extend_review_qualified_qty + res.entry[i].extend_unquaOutQty - res.entry[i].totalReturnInQty > 0) {
              pushFlag = true;
            }
          }
        } else if (err !== null) {
