...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    let dosageForm = request.dosageForm;
    let materialClassId = request.materialClassId;
    ...
    if (window.location.href.indexOf("dbox.yyuap.com") > -1) {
      gspUrl = "https://www.example.com/"; //+storageId;
    } else {
      gspUrl = "https://www.example.com/"; //+storageId;
    }
    ...
    if (typeof vendorData.attorney_authList !== "undefined") {
      for (let i = 0; i < vendorData.attorney_authList.length; i++) {
        ...
        if (name.length < 0) {
          ...
          if (vendorData.attorney_authList[i].scope_authorityList.length > 0) {
            for (let j = 0; j < vendorData.attorney_authList[i].scope_authorityList.length; j++) {
              let attorrmeyScopeId;
              if (vendorData.attorney_authList[i].extend_auth_scope == "1") {
                attorrmeyScopeId = vendorData.attorney_authList[i].scope_authorityList[j].extend_pro_auth_type;
              } else if (vendorData.attorney_authList[i].extend_auth_scope == "2") {
                attorrmeyScopeId = vendorData.attorney_authList[i].scope_authorityList[j].extend_protype_auth_type;
              } else if (vendorData.attorney_authList[i].extend_auth_scope == "3") {
                attorrmeyScopeId = vendorData.attorney_authList[i].scope_authorityList[j].extend_dosage_auth_type;
              }
              if (productId[j] == attorrmeyScopeId) {
                attorneyId[j] = productId[j];
              } else if (materialClassId[j] == attorrmeyScopeId) {
                attorneyId[j] = materialClassId[j];
              } else if (dosageForm[j] == attorrmeyScopeId) {
                attorneyId[j] = dosageForm[j];
              }
            }
          }
    ...
    if (typeof vendorData.extend_lincenseList !== "undefined") {
      for (let i = 0; i < vendorData.extend_lincenseList.length; i++) {
        ...
        if (timeLicenceDiff >= 0) {
          ...
          let attorneyId = [];
          if (vendorData.extend_lincenseList[i].extend_licenseScopeList.length > 0) {
            for (let j = 0; j < vendorData.extend_lincenseList[i].extend_licenseScopeList.length; j++) {
              let attorrmeyScopeId;
              if (vendorData.extend_lincenseList[i].extend_auth_type_v2 == "1") {
                attorrmeyScopeId = vendorData.extend_lincenseList[i].extend_licenseScopeList[j].extend_pro_auth_type;
              } else if (vendorData.extend_lincenseList[i].extend_auth_type_v2 == "2") {
                attorrmeyScopeId = vendorData.extend_lincenseList[i].extend_licenseScopeList[j].extend_protype_auth_type;
              } else if (vendorData.extend_lincenseList[i].extend_auth_type_v2 == "3") {
                attorrmeyScopeId = vendorData.extend_lincenseList[i].extend_licenseScopeList[j].extend_dosage_auth_type;
              }
              if (productId[j] == attorrmeyScopeId) {
                attorneyId[j] = productId[j];
              } else if (materialClassId[j] == attorrmeyScopeId) {
    ...
    for (let pro = 0; pro < productData.length; pro++) {
      ...
      if (authProduct.length > 0) {
        ...
        for (let splx = 0; splx < authDosage.length; splx++) {
          if (productData[pro].extend_tsyp == "true") {
            speCount += 1;
          }
          if (productData[pro].extend_hmhj == "true") {
