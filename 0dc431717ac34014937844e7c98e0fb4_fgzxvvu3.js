...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    let saleReturnSql = "select extend_gsptype,transactionTypeId.name as bustypeName  from 		voucher.salereturn.SaleReturn where id ='" + param.data[0].id + "' ";
    let saleReturnRes = ObjectStore.queryByYonQL(saleReturnSql, "udinghuo")[0];
    if (saleReturnRes.extend_gsptype != 1 && saleReturnRes.extend_gsptype != "1") {
      return;
    } else if (saleReturnRes.bustypeName != "GSP销售退货" && saleReturnRes.bustypeName != "gsp销售退货") {
      return;
    }
    ...
    // 拼装请求报文
    var obj = { tenantId: tenantid, logId: "yourIdHere", time: timestampStr, uuid: sign, saleReturnId: param.data[0].id, userId: ObjectStore.user().id };
    //获取api前缀等信息 参数： 请求方式 ， 地址  ，header ， body
    var strResponse = postman("post", apiRestPre + "/wmsServiceCore/wmsSalesReturnPush", null, JSON.stringify(obj));
    return { strResponse };
  }
}
