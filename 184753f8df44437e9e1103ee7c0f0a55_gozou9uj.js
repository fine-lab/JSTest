...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    if (toOrdersRes && toOrdersRes.length > 0) {
      let userIdUrl = envUrl + "/iuap-api-gateway/yonbip/uspace/users/search_page_list";
    ...
    for (let i = 0; i < shiRes.length; i++) {
      ...
      for (let j = 0; j < shippingschedulebList.length; j++) {
        ...
        purchaseOrder.taxitems_code = "VATR1"; //采购订单的税率编码，新架构由VAT13变成了VATR1
      ...
      batchNumberMap.forEach((value, key) => {
        if (value != "error") {
          ...
          let orderUrl = envUrl + "/iuap-api-gateway/yonbip/scm/purchaseorder/singleSave_v1";
          ...
          if (orderResponse.code != "200") {
            ...
            if (orderResponse.message) {
              let messageObj = JSON.parse(orderResponse.message.replace(/\\n|\/|\\/g, ""));
              if (messageObj && messageObj.length > 0) {
                message = messageObj[0].message;
              }
            }
            if (shiRes[i].errorMessage) {
              shiRes[i].errorMessage += "\n生成采购订单失败！失败原因：" + message;
            } else {
              shiRes[i].errorMessage = "生成采购订单失败！失败原因：" + message;
            }
            ...
            if (persons.length > 0) {
              let todoUrl = envUrl + "/iuap-api-gateway/yonbip/uspace/rest/openapi/idempotent/todo/push";
              let todoData = {
                srcMsgId: uuid(),
                yyUserIds: persons,
                ...
                webUrl:
                  envUrl +
                  "/mdf-node/meta/voucher/st_purchaseorder/" +
