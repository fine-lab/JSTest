  ...
  var MyExternal = {
    resolveExcelData(params) {
      ...
      if (excelData && excelData[0].length > 0) {
        let focusedRowIndex = viewModel.get("shippingschedulebList").get("focusedRowIndex");
        let shippingschedulebId = viewModel.get("shippingschedulebList").getData()[focusedRowIndex].id;
        let quantitiy = viewModel.get("shippingschedulebList").getData()[focusedRowIndex].quantitiy;
        let tenantId = viewModel.get("shippingschedulebList").getData()[focusedRowIndex].tenant_id;
        let shippingschedulesnvo = [];
        let searchParam = { shippingschedulebId: shippingschedulebId };
        // 导入行数与数量校验
        cb.rest.invokeFunction("GT37595AT2.excelBackFunciton.getSnCount", searchParam, function (err, res) {
          if (res) {
          ...
          for (let i = 0; i < excelData[0].length; i++) {
            ...
            shippingschedulesnItem["shippingscheduleb_id"] = shippingschedulebId;
          ...
          // 保存
          cb.rest.invokeFunction("GT37595AT2.excelBackFunciton.batchSaveSn", { shippingschedulesnvo: shippingschedulesnvo }, function (err, res) {
            if (res) {
