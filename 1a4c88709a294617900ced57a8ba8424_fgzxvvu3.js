viewModel.on("customInit", function (data) {
  ...
  viewModel.on("beforeBatchpush", function (data) {
    debugger;
    if (data.args.key == "batchpush34c7b777-5e62-11ed-98bc-fa163e27a895") {
      if (data.params.data.length > 1) {
        cb.utils.alert("选中数据大于1条，请确认要下推的数据！");
        return false;
      }
      let code = data.params.data[0].code;
      let optionAllXiShu = { async: false };
      let resXIShuAll = cb.rest.invokeFunction("3e04159e33bf42f9bf36d2ae29fdb094", { code: code }, null, viewModel, optionAllXiShu);
      if (resXIShuAll && resXIShuAll.error == undefined) {
        if (resXIShuAll.result.res.length > 0) {
          cb.utils.alert("下游单据已存在，无法重复下推！");
          return false;
        }
      }
    }
