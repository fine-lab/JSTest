  ...
  execute(request) {
    ...
    for (let i = 0; i < ids.length; i++) {
      ...
      for (let j = 0; j < entryInfo.length; j++) {
        ...
        let materialSql = "select *  from GT22176AT10.GT22176AT10.SY01_material_file where material = '" + entryInfo[j].product + "'";
        let materialInfo = ObjectStore.queryByYonQL(materialSql, "sy01");
        if (materialInfo.length > 0) {
          entryInfo[j].extend_standard_code = materialInfo[0].standardCode;
          entryInfo[j].extend_package_specification = materialInfo[0].boxPackSpec;
