...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    for (let i = 0; i < currentStockList.length; i++) {
      ...
      let batchnoSql =
        "select id,define1,define2,define3,define4,define5 from st.batchno.Batchno where product='" +
        currentStockList[i].product_code +
        "' and batchno = '" +
        currentStockList[i].lot_num_batchno +
        "' ";
      let batchnoList = ObjectStore.queryByYonQL(batchnoSql, "yonbip-scm-scmbd");
      if (batchnoList.length > 0) {
        currentStockList[i].batchno_define1 = batchnoList[0].define1;
        currentStockList[i].batchno_define2 = batchnoList[0].define2;
        currentStockList[i].batchno_define3 = batchnoList[0].define3;
        currentStockList[i].batchno_define4 = batchnoList[0].define4;
        currentStockList[i].batchno_define5 = batchnoList[0].define5;
        currentStockList[i].lot_num = batchnoList[0].id;
      }
    }
