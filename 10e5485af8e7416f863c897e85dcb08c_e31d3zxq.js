    ...
    if (mode != "browse") {
      ...
      for (let i = 0; i < rows.length; i++) {
        if (rows[i].source) {
          ...
          let producedate = row.producedate;
          let invaliddate = row.invaliddate;
          if (rows[i].source == "82ecba4c" || rows[i].source == "60360bcd" || rows[i].source == "d15737c7") {
            ...
            if (producedate != undefined) {
              detailsModel.setCellValue(i, "producedate", FormatToDate(producedate));
            }
            if (invaliddate != undefined) {
              detailsModel.setCellValue(i, "invaliddate", FormatToDate(invaliddate));
            }
            //冻结单过来
