...
gridModel.on("afterCellValueChange", function (args) {
  ...
  if (cellName == "yhbhgsl") {
    ...
    gridModel.setCellValue(rowIndex, "yhhgsl", curingNumber - yhbhgsl);
    ...
    gridModel.setCellValue(rowIndex, "yhbhgsl", curingNumber - yhhgsl);
  }
});
...
viewModel.on("afterLoadData", function (args) {
  let orgId = viewModel.get("org_id").getValue();
  let data = gridModel.getData();
  for (let i = 0; i < data.length; i++) {
    if (data[i].picihao != null && data[i].picihao != undefined && (data[i].batchInDate == null || data[i].batchInDate == undefined)) {
      queryBatchInDate(orgId, data[i].picihao).then(
        (result) => {
          if (result != null && result != undefined) {
            gridModel.setCellValue(i, "batchInDate", result); //首次入库时间
          }
        },
        (err) => {
          cb.utils.alert(err, "error");
