...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    var po_code = param.data[0].po_code;
    let vendorId = param.data[0].vendorId;
    if (!vendorId) {
      return { po_code };
    }
    let suplierSql = "select id,relationTenantId from aa.vendor.Vendor where id = " + vendorId;
    var suplierRes = ObjectStore.queryByYonQL(suplierSql, "yssupplier");
    if (!suplierRes || suplierRes.length <= 0) {
      return { po_code };
    }
    let body = { code: po_code, isCode: false };
    let tenantStr = "w9fjvoyb";
    if (suplierRes[0].relationTenantId) {
      tenantStr = suplierRes[0].relationTenantId;
    }
    let tokenSql = "select vendorId,appKey,appSecret from GT39745AT6.GT39745AT6.suplierApiMessage where vendorId = " + vendorId;
    let tokenRes = ObjectStore.queryByYonQL(tokenSql);
    if (!tokenRes || tokenRes.length <= 0) {
      return { po_code };
    }
    var access_token = getToken(tokenRes[0].appKey, tokenRes[0].appSecret);
    let url = "https://www.example.com/" + tenantStr + "/product_ref/product_ref_01/delSupplierShippingBill";
    let apiResponse = postman("post", url + "?access_token=" + access_token, null, JSON.stringify(body));
    return { apiResponse };
    //获取token方法
