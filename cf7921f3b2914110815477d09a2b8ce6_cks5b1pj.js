...
class MyTrigger extends AbstractTrigger {
  execute(context, resAllSubjectInfo, subjectCode, lendingDirection) {
    try {
      ...
      if (currentPeriodList != null) {
        currentPeriodList.forEach((item) => {
          if (lendingDirection == "credit" && subjectCode.includes(item.accsubject_code) && item.hasOwnProperty("localcredebit2") && item.localcredebit2 != null) {
            currentPeriod = currentPeriod + item.localcredebit2;
          }
          if (lendingDirection == "debit" && subjectCode.includes(item.accsubject_code) && item.hasOwnProperty("localdebit2") && item.localdebit2 != null) {
      ...
      throw new Error("执行脚本getProfitDirect报错：" + e);
    }
  }
}
