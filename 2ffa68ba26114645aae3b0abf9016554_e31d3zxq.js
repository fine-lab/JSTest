...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    //防止重复生成
    let validateIsInsertSql = "select id,firstBusinessOrderNo from GT22176AT10.GT22176AT10.SY01_supplier_file where firstBusinessOrderNo = '" + billInfo.code + "'";
      ...
      for (let i = 0; i < extend_lincenseList.length; i++) {
        ...
        if (get_license_sub_res != undefined && get_license_sub_res != null) {
          for (let j = 0; j < get_license_sub_res.length; j++) {
            ...
            licenseRangeInfo.feature = get_license_sub_res[j].feature;
    ...
    if (authInfos != undefined && authInfos != null) {
      for (let i = 0; i < authInfos.length; i++) {
        ...
        //联系电话
        authInfo.contact_number = authInfos[i].phone;
        ...
        if (get_auth_sub_res != undefined && get_auth_sub_res != null) {
          for (let j = 0; j < get_auth_sub_res.length; j++) {
            ...
            authRangeInfo.feature = get_auth_sub_res[j].feature;
    ...
    let updateJson = {
      ...
      //公司类型
      companyType: billInfo.companyType,
      //法人代表
      ...
      //开户许可资料
      isOpenAccount: billInfo.isOpenAccount,
      //质量保证协议有效期
      qualityAssuranceValidity: billInfo.qualityAssuranceValidity
    };
