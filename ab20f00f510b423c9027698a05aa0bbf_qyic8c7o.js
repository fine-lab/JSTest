viewModel.on("customInit", function (data) {
  // 正式客户--页面初始化
  viewModel.get("principalName").on("beforeBrowse", function (args) {
    let condition = {
      isExtend: true,
      simpleVOs: [
        {
          field: "mainJobList.lastestjob",
          op: "eq",
          value1: 1
        }
      ]
    };
    viewModel.get("principalName").setFilter(condition);
  });
});
viewModel.on("customInit", function (data) {
  // 正式客户列表--页面初始化
...
viewModel.on("afterInitCommonViewModel", function () {
  ...
  filterViewModel.on("afterInit", function () {
    const referModel = filterViewModel.get("merchantAppliedDetail.specialManagementDep").getFromModel();
