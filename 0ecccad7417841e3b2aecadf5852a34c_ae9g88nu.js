...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    if (GSPParamInfo != undefined && GSPParamInfo.length != 0) {
      //如果gsp中没有配置，再去gmp中寻找
      if (GSPParamInfo == undefined || GSPParamInfo[0].stoconditioncontrol == undefined) {
        gspControlStorageFlag = 0;
      }
    ...
    if (GMPParamInfo != undefined && GMPParamInfo.length != 0) {
      //如果GMP也没有配置，那么直接返回
      if (GMPParamInfo == undefined || GMPParamInfo[0].storageControl == undefined) {
        gmpControlStorageFlag = 0;
      }
    ...
    //如果两边都没有设置，那么直接返回
    //如果gmp和gsp都没有设置，那么return
    if ((gspControlStorageFlag == 0 || gspControlStorageFlag == 2) && (gmpControlStorageFlag == 0 || gmpControlStorageFlag == 2)) {
      return {
        errorMsg
      };
    }
