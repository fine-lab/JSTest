...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    let productQuerySql =
      "select code,name,model,manageClass,manageClass.code as manageClassCode,manageClass.parent as manageClassParent from pc.product.Product where model in " + inSqlCondition + "";
    ...
    productRes.map((item) => {
      ...
      if (manageClassParent != 0) {
        while (manageClassParent != 0 && manageClassParent) {
