...
viewModel.on("afterMount", function (data) {
  ...
  setTimeout(() => {
    ...
    if (viewModel.get("transactionTypeId_name").getValue() == "备件出货" || viewModel.get("transactionTypeId_name").getValue() == "服务器出货") {
      viewModel.execute("updateViewMeta", { code: "linetabs38vi", visible: true });
      viewModel.get("button378wc").setVisible(true);
      if (viewModel.getParams().mode != "browse") {
        ...
        viewModel.get("button360qe").setVisible(true);
      } else {
      ...
      viewModel.get("button360qe").setVisible(false);
      viewModel.getGridModel("deliveryDetails").getActionsState()[0].button360qe.visible = false;
      viewModel.execute("updateViewMeta", { code: "linetabs38vi", visible: false });
      viewModel.get("button378wc").setVisible(false);
    }
  });
});
...
viewModel.get("button254uh").on("click", function () {
  ...
  let titleData = [["物料编码*", "物料", "SN*", "箱号*", "箱数*", "保质期", "生产日期", "DC日期", "出厂时间", "到期日期", "体积", "毛重"]];
  debugger;
...
viewModel.get("button378wc") &&
  viewModel.get("button378wc").on("click", function (data) {
    //导出--单击
    let detailList = viewModel.getGridModel("deliveryDetails").getData();
    let arr1 = [];
    for (let i in detailList) {
      let lists = detailList[i * 1];
      arr1 = arr1.concat(lists.snSalesMessageList || []);
    }
    if (!arr1.length) {
    ...
    // 调用公共函数导出方法
    exportExcelFile(sheetData1, "销售发货单SN信息");
    cb.utils.alert("导出完成！", "success");
  });
viewModel.get("transactionTypeId_name") &&
  viewModel.get("transactionTypeId_name").on("afterValueChange", function (data) {
    //交易类型--值改变后
    // 交易类型是备件出货，服务器出货，显示SN导入、SN导出、SN明细
    debugger;
    if (viewModel.get("transactionTypeId_name").getValue() == "备件出货" || viewModel.get("transactionTypeId_name").getValue() == "服务器出货") {
      viewModel.execute("updateViewMeta", { code: "linetabs38vi", visible: true });
      viewModel.get("button378wc").setVisible(true);
