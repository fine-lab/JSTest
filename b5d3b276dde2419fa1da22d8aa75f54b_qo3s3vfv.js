...
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValueUserReferInput1ld7kh() {}
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValueUserReferInput1ld7kh: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_formStore_UserReferInput1ld7kh: reaction(
      () => [rootStore.formStore.getValue("UserReferInput3jg")],
      async ([formStore_UserReferInput3jg]) => {
        debounce(
          async () => {
            // 获取当前页面所有数据
            ...
            let result = await countFormula(
              {
                hasFormula: "",
                formula: "UserReferInput3jg.user_id",
                formulaFuns: "",
                formulaTriggers: "UserReferInput3jg.user_id"
              },
              {
                mainDataSourceAlias: "mainEntity",
                field: "UserReferInput1ld",
                billnum: "chelaingyash001",
            ...
            // 公式结束loading
            rootStore.formStore.setValue("UserReferInput1ld", resultData);
          },
          "formula_customSetValue_formStore_UserReferInput1ld7kh" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
