...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    let uidata = param.makeBillParams.uidata;
    let purchaseOrders = uidata[0].purchaseOrders;
    for (var i in purchaseOrders) {
    }
    if (purchaseOrders.length > 0) {
      let purchaseOrder = purchaseOrders[0];
      //单据号
      let batch = purchaseOrder.code;
      //批次号
      let batchNo = purchaseOrder.extend118;
      let priceSql = "select  batch_time from GT18AT2.GT18AT2.batch_price where code = " + batchNo;
      var res = ObjectStore.queryByYonQL(priceSql, "developplatform");
      let batch_time = "";
      if (res.length > 0) {
        batch_time = res[0].batch_time;
      }
      //价格(PO的含税单价)
      let price = purchaseOrder.oriTaxUnitPrice;
      let poNo = purchaseOrder.code;
      //要货计划表id
      let shippingScheduleId = purchaseOrder.extend71;
      if (shippingScheduleId) {
      }
      shippingScheduleId = "yourIdHere";
      let lineDataList = [];
      let lineData = {
        batch: batch,
      ...
      let updateData = { id: shippingScheduleId, shippingschedulebList: lineDataList };
      var res = AppContext();
      var resJson = JSON.parse(res);
      var tenantId = resJson.currentUser.tenantId;
      let access_token = getToken("1d1a7303596f4fcbb2f858276d75cfc9", "3f46f761d8014804b01f2ec1507757e7");
      ...
      if (apiResponseObj.code != "200") {
        throw new Error("更新要货计划失败！接口返回参数：" + apiResponse);
      }
      ...
      for (var i in res) {
        ...
        let executeDate = getDate();
        //构造推送海棠数据
        let htData = {};
        htData.DEMAND_ORDER = res[i].item_type;
        htData.BATCH_NUMBER = res[i].batchNumber;
        htData.GROUP_ID = res[i].mainId;
        htData.ACTION_TYPE = "UPDATE";
        htData.SL_VENDOR = res[i].computerSupplier;
        htData.ORDER_PRIORITY = res[i].orderPriority;
        htData.PROCUREMENT_MODEL = res[i].orderMode;
        htData.ROOM_ID = res[i].equipmentRoom;
        htData.SHIP_TO_ADDRESS = res[i].shipToLocation;
        htData.SHIP_TO_CONTACT = res[i].shipToContact;
        htData.SHIP_TO_CONTACT_TEL = res[i].shipToContactPhone;
        htData.SHIP_TO_CONTRY = res[i].country;
        htData.SHIP_TO_PROVINCE = res[i].province;
        htData.SHIP_TO_CITY = res[i].city;
        htData.CUSTOMGER_PO_NO = poNo || "";
        htData.CUSTOMER_PO_STATUS = res[i].poStatus;
        htData.CUSTOMER_CODE = sconfig.BASE.CUSTOMER_CODE;
        htData.CUSTOMER_NAME = sconfig.BASE.CUSTOMER_NAME;
        htData.SHIPPING_METHOD = "By Air";
        htData.FOB = "";
        htData.FOB_ADDRESS = "";
        htData.PO_CONFIRM_DATE = null; // 海棠反推
        htData.PO_ACTIVE_DATE = null; // 海棠反推
        htData.ORIG_CPD = res[i].cpd;
        htData.ORIG_ESD = res[i].esd;
        htData.ORIG_ETA = res[i].eta;
        htData.RECEIVE_DEMAND_DATE = executeDate;
        htData.RECEIVE_CEG_PRICE_DATE = batch_time;
        htData.CURRENCY = "CNY";
        htData.DEMAND_LINE_ID = res[i].poLineId;
        htData.PRODUCT_NAME = res[i].productCode;
        htData.CUSTOMIZATION_FLAG = res[i].customLogo;
        htData.MATCH_QTY = res[i].orderedQuantity;
        htData.DESCRIPTION = res[i].itemRemark;
        htData.UNIT_PRICE = price;
        htData.QTY = res[i].nodeNumber;
        htData.ATTRIBUTE1 = "";
        htData.ATTRIBUTE2 = "";
        htData.ATTRIBUTE3 = "";
        htData.ATTRIBUTE4 = "";
      ...
      throw new Error(JSON.stringify(htResponseObj));
      if (htResponseObj.status != "success") {
        throw new Error("调取海棠接口失败！接口返回参数：" + htResponse);
      }
      return { status: 1 };
      function getTime() {
