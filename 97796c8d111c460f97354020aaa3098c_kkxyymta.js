...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    let deliveryOrderSql = "select extendIsWms from 	voucher.delivery.DeliveryVoucher where id ='" + param.data[0].id + "' ";
    let deliveryOrderRes = ObjectStore.queryByYonQL(deliveryOrderSql, "udinghuo")[0];
    if (deliveryOrderRes.extendIsWms != 1 && deliveryOrderRes.extendIsWms != "1") {
      return;
    }
    ...
    // 拼装请求报文
    var obj = { tenantId: tenantid, logId: "yourIdHere", time: timestampStr, uuid: sign, deliveryOrderId: param.data[0].id, userId: ObjectStore.user().id };
    //获取api前缀等信息 参数： 请求方式 ， 地址  ，header ， body
    var strResponse = postman("post", apiRestPre + "/gsp/wmsSalesOutPush", null, JSON.stringify(obj));
    return { strResponse };
  }
}
