...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    if (tenantId != "kegf1e24" && tenantId != "clfpjof8") {
      return;
    }
    // 时间格式化方法
    ...
    let saleReturnSql = "select extend_is_gsp,bustype.name as bustypeName  from 	pu.purchaseorder.PurchaseOrder where id ='" + param.data[0].id + "' ";
    let saleReturnRes = ObjectStore.queryByYonQL(saleReturnSql, "udinghuo")[0];
    if (saleReturnRes.extend_is_gsp != 1 && saleReturnRes.extend_is_gsp != "1") {
      return;
    } else if (saleReturnRes.bustypeName != "GSP采购退货" && saleReturnRes.bustypeName != "gsp采购退货") {
      return;
    }
    ...
    // 拼装请求报文
    var obj = { tenantId: tenantid, logId: "yourIdHere", time: timestampStr, uuid: sign, purchaseOrderId: param.data[0].id, userId: ObjectStore.user().id };
    //获取api前缀等信息 参数： 请求方式 ， 地址  ，header ， body
    var strResponse = postman("post", apiRestPre + "/wmsServiceCore/wmsPurchaseReturnPush", null, JSON.stringify(obj));
    return { strResponse };
  }
}
