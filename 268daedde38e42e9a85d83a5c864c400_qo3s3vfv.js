...
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValueInput21t8xi() {},
  formulacustomSetValueInputNumber0bd() {}
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValueInput21t8xi: observable,
  formulacustomSetValueInputNumber0bd: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_formStore_Input21t8xi: reaction(
      () => [rootStore.formStore.getValue("UserReferInput1mg")],
      async ([formStore_UserReferInput1mg]) => {
        debounce(
          async () => {
            ...
            const formulaResult = await countFormula(
              {
                hasFormula: "true",
                formula: "UserReferInput1mg.mobile",
                formulaFuns: "",
                formulaTriggers: "UserReferInput1mg.mobile"
              },
              {
            ...
            rootStore.formStore.setValue("Input21t", resultData);
          },
          "formula_customSetValue_formStore_Input21t8xi" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
    ...
    formula_customSetValue_formStore_InputNumber0bd: reaction(
      () => [rootStore.formStore.getValue("DateTimePicker12s"), rootStore.formStore.getValue("DateTimePicker10c")],
      async ([formStore_DateTimePicker12s, formStore_DateTimePicker10c]) => {
        debounce(
          async () => {
            const allData = getAllDataByCardStore({
              cardStoreAlias: "cardStore",
              storeControlType: "FormStore",
              storeField: "InputNumber0b",
              storeForm: "",
              dataSource: "mainEntity"
            });
            const formulaResult = await countFormula(
              {
                hasFormula: "true",
                formula: "dateDiff(DateTimePicker12s,DateTimePicker10c,3)",
                formulaFuns: "dateDiff",
                formulaTriggers: "DateTimePicker12s,DateTimePicker10c"
              },
              {
                mainDataSourceAlias: "mainEntity",
                field: "InputNumber0b",
            ...
            //对公式结果进行处理
            const resultData = formulaResult.resultData;
            rootStore.formStore.setValue("InputNumber0b", resultData);
          },
          "formula_customSetValue_formStore_InputNumber0bd" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
