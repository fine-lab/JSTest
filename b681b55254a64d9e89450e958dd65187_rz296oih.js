...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    let yonQl =
      "select *,busType.name,busType.code, (select * from arrivalOrders) arrivalOrders , org.name orgName,t1.name orignProvince,t2.name orignCity,t3.name destProvince,t4.name destCity from pu.arrivalorder.ArrivalOrder ";
    ...
    let saveLog = {};
    saveLog.methodName = "cancelArrayToZyy";
    ...
    let logisticStatus = apiResponse.data.isSuccess == "Y" ? 3 : 2;
    let detailUrl = funRes.BASE.arrivalDetailUrl;
    let saveUrl = funRes.BASE.arrivalSaveUrl;
    res.map((item, i) => {
      apiResponse = openLinker("GET", detailUrl + "?id=" + item, appCode, null);
