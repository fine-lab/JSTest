...
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValueDateTimePicker9ab() {},
  formulacustomSetValueInput24lc() {}
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValueDateTimePicker9ab: observable,
  formulacustomSetValueInput24lc: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_formStore_DateTimePicker9ab: reaction(
      () => [],
      async ([]) => {
        debounce(
          async () => {
          ...
          "formula_customSetValue_formStore_DateTimePicker9ab" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
    ...
    formula_customSetValue_formStore_Input24lc: reaction(
      () => [rootStore.formStore.getValue("DateTimePicker11x"), rootStore.formStore.getValue("DateTimePicker9a")],
      async ([formStore_DateTimePicker11x, formStore_DateTimePicker9a]) => {
        debounce(
          async () => {
            const allData = getAllDataByCardStore({
              cardStoreAlias: "cardStore",
            ...
            const formulaResult = await countFormula(
              {
                hasFormula: "true",
                formula: "dateDiff(DateTimePicker11x,DateTimePicker9a,4)",
                formulaFuns: "dateDiff",
                formulaTriggers: "DateTimePicker11x,DateTimePicker9a"
              },
              {
                mainDataSourceAlias: "mainEntity",
                field: "Input24l",
            ...
            //对公式结果进行处理
            const resultData = formulaResult.resultData;
            rootStore.formStore.setValue("Input24l", resultData);
          },
          "formula_customSetValue_formStore_Input24lc" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
