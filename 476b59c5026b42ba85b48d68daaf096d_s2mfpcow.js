...
try {
  // 扩展字段总览store
  const sysFieldOverViewStore = {
    // 扩展get 方法
  };
  // 扩展字段总览store监听
  const sysFieldOverViewObservable = {};
  // 设置字段总览监听
  makeObservable(sysFieldOverViewStore, sysFieldOverViewObservable);
  // 将扩展字段总览store挂载到rootStore上
  rootStore.sysFieldOverViewStore = sysFieldOverViewStore;
  // 监听load事件
  rootStore.eventBus.on(rootStore, "load", () => {
    rootStore.reactions.fieldOverView = {};
  });
} catch (e) {
  console.log("字段总览代码出错", e);
  console.error("字段总览代码出错", e);
}
let qzddmxnListStoreExtendRowStore = RowStore;
if (rootStore.qzddmxnListStore.extendRowStore) {
  qzddmxnListStoreExtendRowStore = rootStore.qzddmxnListStore.extendRowStore;
}
class qzddmxnListStoreRowStore extends qzddmxnListStoreExtendRowStore {
  constructor() {
    super(...arguments);
    this.formulacustomSetValueinputnumber110 = null;
    makeObservable(this, {
      formulacustomSetValueinputnumber110: observable
    });
  }
}
rootStore.qzddmxnListStore.extendRowStore = qzddmxnListStoreRowStore;
// 扩展新的store
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValueinputnumber40: null
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValueinputnumber40: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_qzddmxnListStore_inputnumber110: reaction(
      () => [rootStore.qzddmxnListStore.getValue().length, rootStore.qzddmxnListStore.getValue()],
      () => {
        if (!rootStore.qzddmxnListStore.dismissStack) {
          rootStore.qzddmxnListStore.dismissStack = [];
        }
        while (rootStore.qzddmxnListStore.dismissStack.length) {
          rootStore.qzddmxnListStore.dismissStack.pop();
        }
        rootStore.qzddmxnListStore.getValue().forEach((rowStore, tableStoreRowIndex) => {
          dismiss = reaction(
            () => [rowStore.getTrueValue("jiage"), rowStore.getTrueValue("shuliang")],
            async ([qzddmxnListStore_jiage, qzddmxnListStore_shuliang]) => {
              debounce(
                async () => {
                  const allData = getAllDataByCardStore({
                    ...
                    storeField: "jine",
                    storeForm: "",
                    dataSource: "qzddmxnList"
                  });
                  const formulaResult = await countFormula(
                    {
                      hasFormula: "true",
                      formula: "qzddmxnList.jiage*qzddmxnList.shuliang",
                      formulaFuns: "",
                      formulaTriggers: "qzddmxnList.jiage,qzddmxnList.shuliang"
                    },
                    {
                      serviceCode: rootStore?.pageStore?.params?.serviceCode || rootStore?.pageStore?.query?.serviceCode || "",
                      mainDataSourceAlias: "mainEntity",
                      field: "qzddmxnList.jine",
                      billnum: "styygssz",
                  ...
                  rowStore.setValue("jine", resultData);
                },
                "formula_customSetValue_qzddmxnListStore_inputnumber110" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
                300
              )();
            },
            {
              // 是否立即执行
              fireImmediately: isFireImmediately({ storeType: "CardStore", storeAlias: "cardStore" }),
              // 判断是否要执行副函数
    ...
    formula_customSetValue_formStore_inputnumber40: reaction(
      () => [rootStore.qzddmxnListStore.getValue().length, rootStore.qzddmxnListStore.getValue()],
      () => {
        if (!rootStore.qzddmxnListStore.dismissStack) {
          rootStore.qzddmxnListStore.dismissStack = [];
        ...
        rootStore.qzddmxnListStore.getValue().forEach((rowStore, tableStoreRowIndex) => {
          dismiss = reaction(
            () => [rowStore.getTrueValue("jine")],
            async ([qzddmxnListStore_jine]) => {
              debounce(
                async () => {
                  const allData = getAllDataByCardStore({
                    cardStoreAlias: "cardStore",
                    storeControlType: "FormStore",
                    storeField: "dinghuoheji",
                    storeForm: "",
                  ...
                  const formulaResult = await countFormula(
                    {
                      hasFormula: "true",
                      formula: 'sum(qzddmxnList,"qzddmxnList.jine")',
                      formulaFuns: "sum",
                      formulaTriggers: "qzddmxnList,qzddmxnList.jine"
                    },
                    {
                      serviceCode: rootStore?.pageStore?.params?.serviceCode || rootStore?.pageStore?.query?.serviceCode || "",
                      mainDataSourceAlias: "mainEntity",
                  ...
                  //对公式结果进行处理
                  const resultData = formulaResult.resultData;
                  rootStore.formStore.setValue("dinghuoheji", resultData);
                },
                "formula_customSetValue_formStore_inputnumber40" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
                300
              )();
            },
            {
