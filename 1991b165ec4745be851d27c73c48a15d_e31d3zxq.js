...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    if (typeof baseInfo.productTemplate != "undefined" && baseInfo.productTemplate != null) {
      ...
      baseInfo.productTemplate_name = productTplRes[0].name;
    }
    let subObject = { SY01_qualified_supply_id: request.id };
    ...
    for (let j = 0; j < baseInfo.license.length; j++) {
      ...
      let sonObject = { SY01_supply_licences_id: baseInfo.license[j].id };
      baseInfo.license[j].range = ObjectStore.selectByMap("ISY_2.ISY_2.SY01_supply_licences_range", sonObject);
      for (let k = 0; k < baseInfo.license[j].range.length; k++) {
        for (let l = 0; l < licenceRes[0].licenceChildRes.length; l++) {
          if (licenceRes[0].licenceChildRes[l].authType != baseInfo.license[j].range[k].authType) {
            continue;
          }
          ...
          if (licenceRes[0].licenceChildRes[l].authType == "1" && typeof baseInfo.license[j].range[k].authProduct != "undefined") {
            baseInfo.license[j].range[k].authType == "1";
            let materialId = baseInfo.license[j].range[k].authProduct;
            let paramObj = { orgId, materialId };
            let proDetailSql = "select * from pc.product.Product where id = " + materialId;
            let merchantInfo = ObjectStore.queryByYonQL(proDetailSql, "productcenter");
            baseInfo.license[j].range[k].productName = merchantInfo[0].name;
          } else if (licenceRes[0].licenceChildRes[l].authType == "2" && typeof baseInfo.license[j].range[k].authProductType != "undefined") {
          ...
          } else if (licenceRes[0].licenceChildRes[l].authType == "3" && typeof baseInfo.license[j].range[k].authDosageForm != "undefined") {
          ...
          } else if (licenceRes[0].licenceChildRes[l].authType == "4" && typeof baseInfo.license[j].range[k].authSku != "undefined") {
            ...
            let proDetailSql = "select * from pc.product.Product where id = " + materialId;
            let merchantInfo = ObjectStore.queryByYonQL(proDetailSql, "productcenter");
            let proDetailSkuSql = "select * from pc.product.ProductSKU where productId = " + materialId + " and id = " + proSkuId;
            let proDetailSkuList = ObjectStore.queryByYonQL(proDetailSkuSql, "productcenter");
            baseInfo.license[j].range[k].productName = merchantInfo[0].name;
            if (typeof proDetailSkuList != "undefined" && proDetailSkuList != null) {
              if (proDetailSkuList.length > 0) {
                for (let l = 0; l < proDetailSkuList.length; l++) {
                  if (proDetailSkuList[l].id == proSkuId) {
                    baseInfo.license[j].range[k].skuName = proDetailSkuList[l].name;
                  }
                }
              }
            } else {
            }
          } else if (licenceRes[0].licenceChildRes[l].authType == "5" && typeof baseInfo.license[j].range[k].authFeatures != "undefined") {
            baseInfo.license[j].range[k].authType = "5";
            let materialId = baseInfo.license[j].range[k].authProduct;
            let proDetailSql = "select * from pc.product.Product where id = " + materialId;
            let merchantInfo = ObjectStore.queryByYonQL(proDetailSql, "productcenter");
            baseInfo.license[j].range[k].productName = merchantInfo[0].name;
            baseInfo.license[j].range[k].authFeatures = baseInfo.license[j].range[k].authFeatures;
          }
        }
      }
    }
    for (let j = 0; j < baseInfo.attorney.length; j++) {
      ...
      let upOrderId = "yourIdHere";
      let authLicenceId = baseInfo.attorney[j].authorizerCode;
      let paramObj = { authLicenceId, orgId, sId, type, url, childUrl, upOrderId };
      let licenceInfo = extrequire("ISY_2.public.getSunInfoList").execute(paramObj);
      let licenceRes = licenceInfo.licenceRes;
      if (licenceRes.length == 0) {
      ...
      let sonObject = { SY01_supply_attorney_id: baseInfo.attorney[j].id };
      baseInfo.attorney[j].range = ObjectStore.selectByMap("ISY_2.ISY_2.SY01_supply_attorney_range", sonObject);
      for (let k = 0; k < baseInfo.attorney[j].range.length; k++) {
        for (let l = 0; l < licenceRes[0].licenceChildRes.length; l++) {
          if (licenceRes[0].licenceChildRes[l].authType != baseInfo.attorney[j].range[k].authType) {
            continue;
          }
          ...
          if (licenceRes[0].licenceChildRes[l].authType == "1" && typeof baseInfo.attorney[j].range[k].authProduct != "undefined") {
            baseInfo.attorney[j].range[k].authType = "1";
            let materialId = baseInfo.attorney[j].range[k].authProduct;
            let paramObj = { orgId, materialId };
            let proDetailSql = "select * from pc.product.Product where id = " + materialId;
            let merchantInfo = ObjectStore.queryByYonQL(proDetailSql, "productcenter");
            baseInfo.attorney[j].range[k].productName = merchantInfo[0].name;
          } else if (licenceRes[0].licenceChildRes[l].authType == "2" && typeof baseInfo.attorney[j].range[k].authProductType != "undefined") {
          ...
          } else if (licenceRes[0].licenceChildRes[l].authType == "3" && typeof baseInfo.attorney[j].range[k].authDosageForm != "undefined") {
          ...
          } else if (licenceRes[0].licenceChildRes[l].authType == "4" && typeof baseInfo.attorney[j].range[k].authSku != "undefined") {
            ...
            if (proDetailSkuList.length > 0) {
              for (let l = 0; l < proDetailSkuList.length; l++) {
                if (proDetailSkuList[l].id == proSkuId) {
                  baseInfo.attorney[j].range[k].skuName = proDetailSkuList[l].name;
                }
              }
            }
          } else if (licenceRes[0].licenceChildRes[l].authType == "5" && typeof baseInfo.attorney[j].range[k].authFeatures != "undefined") {
            baseInfo.attorney[j].range[k].authType = "5";
            let materialId = baseInfo.attorney[j].range[k].authProduct;
            let proDetailSql = "select * from pc.product.Product where id = " + materialId;
            let merchantInfo = ObjectStore.queryByYonQL(proDetailSql, "productcenter");
            baseInfo.attorney[j].range[k].productName = merchantInfo[0].name;
            baseInfo.attorney[j].range[k].authFeatures = baseInfo.license[j].range[k].authFeatures;
          }
        }
      }
    }
    for (let i = 0; i < baseInfo.productList.length; i++) {
      ...
      if (JSON.stringify(materialId) !== "{}" && materialId !== {} && typeof materialId !== "undefined" && materialId !== null) {
        let paramObj = { orgId, materialId };
        let proDetailSql = "select * from pc.product.Product where id = " + materialId;
        let merchantInfo = ObjectStore.queryByYonQL(proDetailSql, "productcenter");
        baseInfo.productList[i].productCode_code = merchantInfo[0].code;
      }
    }
    return { data: baseInfo };
  }
