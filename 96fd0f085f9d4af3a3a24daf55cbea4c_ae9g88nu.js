let djInfoTable = viewModel.get("UDIShowDjInfoList"); //单据列表
let waiBjInfoList = viewModel.get("UDIShowWaiBjInfoList"); //外包装信息表
let zhongBjInfoList = viewModel.get("UDIShowZhongBjInfoList"); //中包装信息表
let ziBjInfoList = viewModel.get("UDIShowZiBjInfoList"); //zi包装信息表
waiBjInfoList.setState("fixedHeight", 180);
zhongBjInfoList.setState("fixedHeight", 180);
ziBjInfoList.setState("fixedHeight", 200);
...
function getParentUdiId(parentUdiId, bzType) {
  ...
  invokeFunction1(
    "GT22176AT10.publicFunction.getParentUdiId",
    {
      udiId: parentUdiId
    },
    function (err, res) {
      ...
      if (bzType === "wbz") {
        ...
        zbzSetData(res.resUDIDataFileRs);
      }
      if (bzType === "zxbz") {
        ziBjInfoList.clear();
        zxbzSetDate(res.resUDIDataFileRs);
      }
    },
    undefined,
    { domainKey: "sy01" }
...
function getUdiInfo(djId) {
  invokeFunction1(
    "GT22176AT10.publicFunction.getUdiSourceOrg",
    {
      djId: djId
    },
    function (err, res) {
      ...
      if (djId === null || djId === "" || typeof djId == "undefined") {
        ...
        for (let i = 0; i < resultData.length; i++) {
          let tbDjrs = {
            ...
            wuliaomingchen: resultData[i].material //物料id
          };
          if (i === 0) {
...
function getDjInfo() {
  invokeFunction1(
    ...
    function (err, res) {
      ...
      //单据重新选则后
      djInfoTable.on("afterSelect", function (rowIndexs) {
      ...
      //外包装重新选中后
      waiBjInfoList.on("afterSelect", function (rowIndexs) {
      ...
      //中包装重新选中后
      zhongBjInfoList.on("afterSelect", function (rowIndexs) {
