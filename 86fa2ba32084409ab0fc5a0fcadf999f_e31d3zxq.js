...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    let sql = "select material,isSku,materialType,approvalValidityDate,factory from GT22176AT10.GT22176AT10.SY01_material_file where org_id  = " + orgId + " and material = " + productId;
    ...
    //医药商品分类
    gsp_spfl = prodInfos[0].materialType;
    let factory = prodInfos[0].factory;
    if (factory != null && factorytrol == "1") {
      ...
      for (let i = 0; i < reportList.length; i++) {
        ...
        if (new Date(end_date) < new Date()) {
          throw new Error(rownoinfo + "生产厂商证照有效期已经过期，不允许采购！");
        }
      }
    }
    let res = AppContext();
    ...
    let funid = "youridHere"; //API函数getSupLicInfo
    let gspUrl = apiPreAndAppCode.olinefix + "/mdf-node/uniform/api/yonscript/handler?funid=" + funid;
    let str_venderinnfo = postman("post", gspUrl, JSON.stringify(header), JSON.stringify(gspBody));
    let venderinnfo = JSON.parse(str_venderinnfo);
    venderinnfo = venderinnfo.data.supLicInfo;
    if (venderinnfo == undefined || venderinnfo == null) {
      throw new Error(rownoinfo + "收票组织没有当前供应商证照信息,请确认是否已经首营！\n\r");
    }
    //考虑供应商停售
    ...
    if (sytype == "2") {
      ...
      sql = "select * from GT22176AT10.GT22176AT10.SY01_material_file where org_id = " + orgId + " and material='" + productId + "' " + featureCondition + "";
      let prodInfor = ObjectStore.queryByYonQL(sql, "sy01");
      if (prodInfor.length == 0) {
        throw new Error(rownoinfo + "物料【" + productName + "】敏感特征项【" + featureTip + "】 未首营\r\n");
      }
    }
    //查詢批号有效期
    let approvalValidityDate;
    let materialInfoFileEntry = [];
    if (sytype == 0 || sytype == "0") {
      approvalValidityDate = prodInfos[0].approvalValidityDate;
      let infoRes = extrequire("GT22176AT10.publicFunction.getProLicInfo").execute({ orgId: orgId, materialId: productId });
      materialInfoFileEntry = infoRes.proLicInfo.SY01_material_file_childList;
    } else if (sytype == 1 || sytype == "1") {
      let infoRes = extrequire("GT22176AT10.publicFunction.getProSkuLicInfo").execute({ orgId: orgId, materialId: productId, sku: productsku });
      approvalValidityDate = infoRes.proLicInfo.approvalValidityDate;
      materialInfoFileEntry = infoRes.proLicInfo.SY01_material_file_childList;
    } else if (sytype == 2 || sytype == "2") {
      let infoRes = extrequire("GT22176AT10.publicFunction.getProFeaLicInfo").execute({ orgId: orgId, materialId: productId, feature: characteristics });
      approvalValidityDate = infoRes.proLicInfo.approvalValidityDate;
      materialInfoFileEntry = infoRes.proLicInfo.SY01_material_file_childList;
    }
    //校验GSP商品档案表头的有批文效期至
    if (approvalValidity == 1 && new Date(approvalValidityDate) < new Date()) {
      throw new Error(rownoinfo + "商品批文有效期已经过期，不允许采购！");
    }
    //校验GSP商品档案表体的有效期至
    if (approvalValidity == 1) {
      if (materialInfoFileEntry.length != 0) {
        ...
        if (licenseFileIdArray.length > 0) {
          ...
          throw new Error(rownoinfo + "资质/报告过期，不允许采购！(GSP商品档案【" + licenseFileNameArray.join(",") + "】)");
        }
      }
    }
    if (isgspzz) {
