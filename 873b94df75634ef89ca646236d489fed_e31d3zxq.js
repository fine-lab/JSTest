  ...
  execute(request) {
    ...
    if (productInfo.length > 0) {
      for (let i = 0; i < productInfo.length; i++) {
        standardCde[productInfo[i].id] = productInfo[i].extend_standard_code;
        packageSpecification[productInfo[i].id] = productInfo[i].extend_package_specification;
      }
    }
    let cArr = [];
    for (let i = 0; i < childRes.length; i++) {
      let is_handle = "";
      childRes[i].standard_code = standardCde[childRes[i].product_code].extend_standard_code;
      childRes[i].extend_package_specification = packageSpecification[childRes[i].product_code].extend_package_specification;
