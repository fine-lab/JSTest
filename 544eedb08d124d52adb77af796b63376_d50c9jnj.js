...
viewModel.on("afterMount", function (data) {
  ...
  if (param.mode == "add") {
    ...
    // 处理传递过来的数据
    let selectResult = cb.rest.invokeFunction("AT160194EA17D00009.apiFun.suanliTwoData", { selectData: param.selectData }, function (err, res) {}, viewModel, { async: false });
    viewModel.getParams().selectResult = selectResult.result.selectData;
  } else {
...
viewModel.getGridModel("saleSettleCalList").on("beforeInsertRow", function (data) {
  ...
  let selectData = param.selectResult;
  // 获取最终父级SN编码，并回填
...
viewModel.getGridModel("saleSettleDetailList").on("beforeInsertRow", function (data) {
  ...
  // 组合页签二数据
  let selectData = viewModel.getParams().selectResult;
  viewModel.getGridModel("saleSettleDetailList").setDataSource(selectData);
  // 获取父项SN编码
  return true;
});
...
viewModel.on("beforeSave", function (data) {
  ...
  if (data.params.cItemName == "button22tg" || data.params.cItemName == "button19hd") {
    let opDate = viewModel.get("opDate").getValue();
    ...
    for (let i = 0; i < saleSettleDetailList.length; i++) {
      let detail = saleSettleDetailList[i];
      if (detail && detail.sn) {
        snList.push(detail.sn);
      }
    }
    let xxwparam = { opDate: opDate, id: id, snList: snList };
    res.xxwparam = xxwparam;
  }
