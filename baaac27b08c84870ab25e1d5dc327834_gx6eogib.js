      ...
      res.map((item) => {
        ...
        if (codeToSN[tempCode].sn && codeToSN[tempCode].assetcode) {
          ...
          if (item.fixedAssetsInfoCharacter) {
            item.fixedAssetsInfoCharacter.attrext5 = codeToSN[tempCode].sn;
            item.fixedAssetsInfoCharacter.attrext10 = codeToSN[tempCode].equipcode || "";
            item.fixedAssetsInfoCharacter.attrext9 = codeToSN[tempCode].assetcode || "";
            item.fixedAssetsInfoCharacter.attrext4 = codeToSN[tempCode].requiremcode || "";
            item.fixedAssetsInfoCharacter.attrext6 = codeToSN[tempCode].contractnumber || "";
            item.fixedAssetsInfoCharacter.attrext7 = codeToSN[tempCode].showTime || "";
        ...
        if (item.status != "0") {
          ...
          data.push(item);
          debugger;
          let apiResponse = openLinker("POST", authUrl, "GT37595AT2", JSON.stringify({ data: itemData }));
