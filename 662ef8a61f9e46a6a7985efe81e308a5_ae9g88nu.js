run = function (event) {
    ...
    if (objs.length > 0) {
      ...
      getVendors().then(() => {
        for (let i = 0; i < vendorArrays.length; i++) {
          //查询出所有明细并放到objs中
          promises.push(
            getQualifiedVendor(vendorArrays[i].id).then((vendorWtsInfo) => {
              for (let j = 0; j < vendorWtsInfo.length; j++) {
                if (
                  typeof vendorWtsInfo[j].extend_salesman_name != "undefined" ||
                  vendorArrays[i].extend_is_carrier == "1" ||
                  vendorArrays[i].extend_is_carrier == 1 ||
                  vendorArrays[i].extend_is_carrier == "true" ||
                  vendorArrays[i].extend_is_carrier == true
                ) {
                  objs.push({ code: vendorArrays[i].code, name: vendorArrays[i].name, salesman_name: vendorWtsInfo[j].extend_salesman_name });
                }
              }
            })
          );
  ...
  getQualifiedVendor = function (vendorId) {
    return new Promise(function (resolve) {
      cb.rest.invokeFunction("GT22176AT10.reportData.getVendorWts", { vendorId: vendorId }, function (err, res) {
        if (typeof res !== "undefined") {
          let vendorWtsInfo = res.vendorWtsListRes;
          console.log(vendorWtsInfo);
          resolve(vendorWtsInfo);
        } else if (err !== null) {
          alert(err.message);
        }
      });
  ...
  let prev = viewModel.get("button5ua");
  ...
  //下一页
  let next = viewModel.get("button11rd");
