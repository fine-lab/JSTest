...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    for (let i = 0; i < apiResponseProduct.length; i++) {
      ...
      if (warehouse[0].isGoodsPosition) {
        let currentStockLocationSql =
          "select currentStockCharacteristic features from 	stock.currentstock.CurrentStockLocation where warehouse.id ='" + warehouseId + "' and product='" + apiResponseProduct[i].material + "'  ";
        let currentStockLocation = ObjectStore.queryByYonQL(currentStockLocationSql, "ustock");
        apiResponseProduct[i].features = currentStockLocation[0].features;
      } else {
        let currentStockSql =
          "select currentStockCharacteristic features from 	stock.currentstock.CurrentStock where  warehouse.id ='" + warehouseId + "' and product='" + apiResponseProduct[i].material + "' ";
        let currentStock = ObjectStore.queryByYonQL(currentStockSql, "ustock");
