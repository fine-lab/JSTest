...
viewModel.get("operation") &&
  viewModel.get("operation").on("afterValueChange", function (data) {
    ...
    if (operation == 2) {
      ...
      //产品入库
      viewModel.get("billType").setValue("finishedReport");
      viewModel.get("billType").setDisabled(true);
      viewModel.get("button1rj").setVisible(false); //绑定按钮
      viewModel.get("button4te").setVisible(true); //下推按钮
      getBillList();
  ...
  viewModel.get("button4te").on("click", function (data) {
    ...
    for (let i = 0; i < sonList.length; i++) {
      ...
      for (let j = 0; j < udiList.length; j++) {
        if (materialInfo.sourceautoid == udiList[j].sourceautoid && materialInfo.materialNum > udiNum) {
          ...
          if (materialInfo.producedate == undefined || materialInfo.producedate == "") {
            ...
            materialInfo.productDate = udiList[j].produceDate;
          }
          ...
          if (materialInfo.isSerialNoManage == true) {
            ...
            if (salesOutsSNs == undefined || salesOutsSNs == null || salesOutsSNs.length == 0) {
              ...
              materialInfo.purInRecordsSNs = salesOutsSNs;
              materialInfo.StoreProRecordsSNs = salesOutsSNs;
            }
      ...
      materialInfo.checkQualifiedNum = udiNum;
      materialInfo.checkUnqualifiedNum = 0;
      materialInfo._status = "Insert";
      if (operation == 2) {
        materialInfo.makeRuleCode = "pu_arrivalorder";
      } else if (operation == 3) {
        materialInfo.makeRuleCode = "deliveryTostoreout";
      } else if (operation == 5) {
      } else if (operation == 6) {
      } else if (operation == 7) {
        materialInfo.makeRuleCode = "poFinishedReportToStoreprorecordQualifiedReceive";
      }
    ...
    cb.utils.confirm(
      ...
      function () {
        if (operation == 2) {
          udiPushBills(pushMaterialList, 2, udiInfoList);
        } else if (operation == 3) {
          //发货单下推出库单
          udiPushBills(pushMaterialList, 3, udiInfoList);
        } else if (operation == 5) {
          ...
          //完工报告下推产品入库
          udiPushBills(pushMaterialList, 7, udiInfoList);
        }
      },
      function () {
        return;
...
function udiPushBills(materialList, billsType, udiList) {
  //重新赋值
    ...
    cb.rest.invokeFunction(
      ...
      function (err, res) {
        ...
        if (typeof res != "undefined") {
          cb.utils.alert("下推成功!" + res.result);
      ...
      function (err, res) {
        if (err) {
          ...
          for (let i = 0; i < apiResponse.length; i++) {
            ...
            viewModel.get("udiSourceBillSonList").insertRow(i, apiResponse[i]);
          }
        }
      },
      {
