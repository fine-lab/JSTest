      ...
      if (recheckMan == "" || recheckMan == null) {
        ...
        cb.rest.invokeFunction("GT22176AT10.publicFunction.getStaffOfCurUser", { mainOrgId: viewModel.get("org_id").getValue() }, function (err, res) {
          ...
          if (res != undefined && res.staffOfCurrentUser.GSPConfigDefaultUser.defaultFhr != undefined) {
            viewModel.get("recheckMan").setValue(res.staffOfCurrentUser.GSPConfigDefaultUser.defaultFhr);
            viewModel.get("recheckMan_name").setValue(res.staffOfCurrentUser.GSPConfigDefaultUser.defaultFhrName);
            viewModel.get("recheckDept").setValue(res.staffOfCurrentUser.GSPConfigDefaultUser.defaultFhrDep);
            viewModel.get("recheckDept_name").setValue(res.staffOfCurrentUser.GSPConfigDefaultUser.defaultFhrDepName);
          }
        });
      }
      //查询所有医药物料档案
