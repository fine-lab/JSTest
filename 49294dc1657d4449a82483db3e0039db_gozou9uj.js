...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    if (res && res.length > 0) {
      ...
      for (var j = 0; j < res.length; j++) {
        ...
        if (suppliershippingschedulebList && suppliershippingschedulebList.length > 0) {
          for (var m = 0; m < suppliershippingschedulebList.length; m++) {
            ...
            if (suppliershippingschedulebList[m].remaining_quantity && suppliershippingschedulebList[m].remaining_quantity != "" && parseInt(suppliershippingschedulebList[m].remaining_quantity) > 0) {
              allSubmit = false;
            }
        ...
        updateItem["submit_status"] = res[j].submit_status;
      ...
      var access_token = getToken("fc6adcf633a54b649e2a36c4b605de1e", "b7dda5210823426e81c70f09550c3160");
      let url = configParams.gatewayUrl + "/" + zongRongTenant + "/product_ref/product_ref_01/batchUpdateShippingBill";
      let apiResponse = postman("post", url + "?access_token=" + access_token, null, JSON.stringify(reqData));
      return { apiResponse };
    } else {
