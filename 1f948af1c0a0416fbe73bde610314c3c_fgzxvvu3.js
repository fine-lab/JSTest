...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    // 数据库清空
    var object = { danjubiaoshi: "2" };
    var resDel = ObjectStore.deleteByMap("AT176A3EE417500003.AT176A3EE417500003.Bankjournal", object, "BankjournalNewList");
    //结算工作台
    let sqlInjs = "SELECT id,vouchdate,accentity from stwb.settlebench.SettleBench where vouchdate is not null"; //结算工作台
    ...
    for (var i = 0; i < resInjs.length; i++) {
      ...
      let sqlInjsb = "SELECT id,bizbillno,ourbankaccount,natAmt,receipttypeb,bizbilltype from stwb.settlebench.SettleBench_b where mainid = '" + danjuid + "'"; //结算工作台
      ...
      for (var j = 0; j < resInjsb.length; j++) {
        ...
        let object = { danjuid: danjuid2, gongsi: gongsi, danjubianhao: code, zhichu: zhichu, shouru: shouru, yinxing: yinxing, riqi: riqi, danjubiaoshi: danjubiaoshi };
    ...
    let resInjsb2 = ObjectStore.insertBatch("AT176A3EE417500003.AT176A3EE417500003.Bankjournal", arr, "BankjournalNewList");
    return {};
  }
}
