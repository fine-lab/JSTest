...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    let shitUrl = sconfig.BASE.URL + "/api/service/esupplier/findCpoResult/1.0.0?suffix_path=/1/1";
    ...
    if (pushObjs.length > 0) {
      ...
      if (sconfig.BASE.SEND_CODE == "HT") {
        ...
        let encodeStr = sconfig.AUTH_INFO.USER + ":" + sconfig.AUTH_INFO.KEY;
        let authorization = "Basic " + Base64Encode(encodeStr);
        htHeader = { Authorization: authorization };
      }
      ...
      if (responseStatus.toLowerCase() != "success") {
        ...
        orderChangeResutData.forEach((changeIem) => {
          // 推送失败，状态改回新建
