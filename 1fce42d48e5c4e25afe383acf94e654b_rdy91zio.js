  ...
  execute(request) {
    ...
    salesOrder.data = {
      ...
      orderDefineCharacter: {
        ...
        icexiangmu002: request.orderDefineCharacter__icexiangmu002,
        icexiangmu001: request.orderDefineCharacter__icexiangmu001,
        ...
        JXYL: request.JXYL, //ICE订单类型
        ICE01: request.ICE01 //检测部门
      },
      memo: request.memo,
    ...
    if (ysSalesOrderId != undefined && ysSalesOrderId != null && ysSalesOrderId != "") {
      ...
      //增加组织校验 add by yangjiall 20230726
      if (salesOrderlist.data.salesOrgId_code != request.salesOrgId) {
        throw new Error("销售组织不允许修改");
      }
      //增加单据日期校验 add by yangjiall 20230726
      if (salesOrderlist.data.vouchdate.substring(0, 10) != request.vouchdate) {
        throw new Error("单据日期不允许修改");
      }
      //增加交易类型校验 add by yangjiall 20230726
      if (salesOrderlist.data.transactionTypeId_code != request.transactionTypeId) {
        throw new Error("交易类型不允许修改");
      }
      //增加客户校验 add by yangjiall 20230726
      let old_agentId = salesOrderlist.data.invoiceAgentId_code;
      let new_agentId = request.agentId;
      if (old_agentId != new_agentId) {
        throw new Error("客户不允许修改");
      }
      ...
      if (salesOrderlist.data.status == 1) {
        // 订单状态处理
        (salesOrder.data.statusCode = "CONFIRMORDER"), (salesOrder.data.currentStatus = 1), (salesOrder.data.nextStatus = "DELIVERGOODS"), (salesOrder.data.nextStatusName = "待发货");
        // 强制处理销售组织和币种的变更，不接受crm有币种或组织的变更
      }
      ...
      if (paymentStatus > 0 || invoicedAmount > 0 || cumulativeIssueQuantity > 0) {
        //当前订单有上下游单据，返回错误信息
        return { message: "已有上下游单据,不可修改" };
      }
      ...
      for (let y = 0; y < ysSalesOrderDetails.length; y++) {
        ...
        for (let j = 0; j < crmOrderDetails.length; j++) {
          ...
          if (ysSalesOrderDetails_memo == crm_id) {
            ...
            ysSalesOrderDetails[y].ordRealMoney = crmOrderDetails[j].oriSum; //行应收金额
            ...
            if (j == crmOrderDetails.length - 1) {
              ...
              ysSalesOrderDetails[y].ordRealMoney = "0"; //行应收金额
          ...
          if (ys_temp_id == crm_temp_id) {
            ...
            if (y == ysSalesOrderDetails.length - 1) {
              ...
              let item = {
                ...
                oriSum: crmOrderDetails[x].oriSum, //含税金额 含税金额=含税报价 * 计价数量
                ordRealMoney: crmOrderDetails[x].oriSum, //行应收金额
                ...
                oriTaxUnitPrice: (crmOrderDetails[x].oriSum / crmOrderDetails[x].qty).toFixed(6), //含税成交价  成交价为含税报价基础上扣减行折扣（含促销优惠）及返利折扣分摊后的单价
