...
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValueInput20xf() {},
  formulacustomSetValueInputNumber4nd() {}
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValueInput20xf: observable,
  formulacustomSetValueInputNumber4nd: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_formStore_Input20xf: reaction(
      () => [rootStore.formStore.getValue("UserReferInput0qb")],
      async ([formStore_UserReferInput0qb]) => {
        debounce(
          async () => {
            ...
            const formulaResult = await countFormula(
              {
                hasFormula: "true",
                formula: "UserReferInput0qb.mobile",
                formulaFuns: "",
                formulaTriggers: "UserReferInput0qb.mobile"
              },
              {
          ...
          "formula_customSetValue_formStore_Input20xf" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
    ...
    formula_customSetValue_formStore_InputNumber4nd: reaction(
      () => [rootStore.formStore.getValue("DateTimePicker10b"), rootStore.formStore.getValue("DateTimePicker8v")],
      async ([formStore_DateTimePicker10b, formStore_DateTimePicker8v]) => {
        debounce(
          async () => {
            const allData = getAllDataByCardStore({
              cardStoreAlias: "cardStore",
              storeControlType: "FormStore",
              storeField: "InputNumber4n",
              storeForm: "",
              dataSource: "mainEntity"
            });
            const formulaResult = await countFormula(
              {
                hasFormula: "true",
                formula: "dateDiff(DateTimePicker10b,DateTimePicker8v,4)",
                formulaFuns: "dateDiff",
                formulaTriggers: "DateTimePicker10b,DateTimePicker8v"
              },
              {
                mainDataSourceAlias: "mainEntity",
                field: "InputNumber4n",
            ...
            //对公式结果进行处理
            const resultData = formulaResult.resultData;
            rootStore.formStore.setValue("InputNumber4n", resultData);
          },
          "formula_customSetValue_formStore_InputNumber4nd" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
