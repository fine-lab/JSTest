...
// 迁移替换参数开始***********************
const migrationParams = { refer: { undefined: { checkRule_rightValue_bb300062b4ef2e4e7bf14b99ab15940f: { id: "youridHere" } } } };
// 迁移替换参数结束***********************
// 设置子表的行号
...
rootStore.eventBus.on(rootStore, "load", () => {
  ...
  rootStore.actions.checkRules = {
    checkRule_02465976aa3b807c: () => {
      return new Promise((resolve) => {
        (async function checkFunc() {
          try {
            // 校验代码动态生成开始
            const condition_bb300062b4ef2e4e7bf14b99ab15940f = await getCheckRuleConditionRes({
              leftType: "store",
              leftData: {
                "UIT#isBizData": false,
                "UIT#isDesignAdd": true,
                alias: "ReferInput4if",
                ...
                nid: "youridHere",
                refEntityUri: "",
                refCode: "ucfbasedoc.bd_bankcard",
                refReturnArr: '{"id":"formStore,ReferInput4if","name":"formStore,ReferInput4if_name"}',
                refReturn: ["name"],
                uiRefIdField: "formStore,ReferInput4if",
                storeAlias: "formStore",
                storeControlType: "FormStore",
                parentCaption: "formStore",
                showValue: "formStore/参照28ID",
                hoverValue: "formStore/参照28ID",
                realValue: "formStore/参照28ID"
              },
              operational: "eq",
              rightType: "constant",
              rightData: {
                startRealValue: migrationParams?.refer?.[""]?.["checkRule_rightValue_bb300062b4ef2e4e7bf14b99ab15940f"]?.id || "",
                endRealValue: "",
                bizType: "quote",
                storeAlias: "",
                alias: "",
            ...
            const condition_1bb7d6f99f11e5e74df91d4ced3fb42d = await getCheckRuleConditionRes({
              leftType: "store",
              leftData: {
                "UIT#isDesignAdd": true,
                bizType: "text",
                ...
                storeControlType: "FormStore",
                parentCaption: "formStore",
                showValue: "formStore/文本框25",
                hoverValue: "formStore/文本框25",
                realValue: "formStore/文本框25"
              },
              operational: "like",
              rightType: "constant",
              rightData: {
            ...
            if (condition_bb300062b4ef2e4e7bf14b99ab15940f || condition_1bb7d6f99f11e5e74df91d4ced3fb42d) {
              createCheckRuleModal({ performAction: "blockTheNextStep", promptContent: "333", promptMode: "floatModal", resolve });
            } else {
              resolve(true);
            }
            // 校验代码动态生成结束
          } catch (e) {
            console.log("校验规则代码存在语法错误，请先自查是否存在配置错误", e);
            console.error("校验规则代码存在语法错误", e);
            // 如果存在语法错误,直接返回true,不能阻塞动作执行
            resolve(true);
          }
        })();
      });
