...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    let deliveryOrderSql = "select extendGspType from 	voucher.delivery.DeliveryVoucher where id ='" + param.data[0].id + "' ";
    let deliveryOrderRes = ObjectStore.queryByYonQL(deliveryOrderSql, "udinghuo")[0];
    if (deliveryOrderRes.extendGspType != 1 && deliveryOrderRes.extendGspType != "1") {
      return;
    }
    ...
    var strResponse = postman("post", apiRestPre + "/wmsServiceCore/wmsSalesOutPush", null, JSON.stringify(obj));
    return { strResponse };
  }
}
