viewModel.on("customInit", function (data) {
      ...
      Promise.all(promiseArr).then(() => {
        ...
        for (let j = 0; j < rows.length; j++) {
          ...
          if (gmpInfoArray.length > 0) {
            ...
            if (!isPass) {
              gridModelInfo.setColumnValue("extend_release_status", "无需放行");
              break;
            }
          }
        }
  ...
  viewModel.on("beforeSave", function (args) {
    ...
    let orgId = viewModel.get("tcOrgId").getValue();
    let currentRow = gridModelInfo.getRows();
    let promiseArr = [];
    promiseArr.push(
      getGmpParameters().then((res) => {
        gmpInfoArray = res;
        ...
        for (let j = 0; j < gmpInfoArray.length; j++) {
          if (orgId == gmpInfoArray[j].org_id) {
            if (gmpInfoArray[j].isProductPass != 1 && gmpInfoArray[j].isProductPass != "1") {
              for (let i = 0; i < dataObject.osmArriveOrderProduct.length; i++) {
                dataObject.osmArriveOrderProduct[i].extend_release_status = "无需放行";
              }
              break;
            } else {
