  ...
  execute(request) {
    ...
    if (typeof billChildInfo != "undefined" && billChildInfo != null) {
      for (let i = 0; i < billChildInfo.length; i++) {
        if (typeof billChildInfo.product != "undefined" && billChildInfo.product != null) {
          products.push(billChildInfo[i].product);
        }
        if (typeof billChildInfo.sourceautoid != "undefined" && billChildInfo.sourceautoid != null) {
    ...
    let productInfo = {};
    let materialSql = "select * from GT22176AT10.GT22176AT10.SY01_material_file where mainid in (" + str_products + ")";
    let materialInfo = ObjectStore.queryByYonQL(materialSql, "sy01");
    if (typeof materialInfo != "undefined" && materialInfo != null) {
      for (let i = 0; i < materialInfo.length; i++) {
        if (!productInfo.hasOwnProperty(materialInfo[i].mainid)) {
          productInfo[materialInfo[i].mainid] = materialInfo[i];
        }
      }
    }
    let salereturnInfo = {};
    ...
    for (let i = 0; i < billChildInfo.length; i++) {
      if (typeof productInfo[billChildInfo[i].product] != "undefined" && productInfo[billChildInfo[i].product] != null) {
        billChildInfo[i].extend_standard_code = productInfo[billChildInfo[i].product].standardCode;
      }
      if (typeof productInfo[billChildInfo[i].product] != "undefined" && productInfo[billChildInfo[i].product] != null) {
        billChildInfo[i].extend_package_specification = productInfo[billChildInfo[i].product].boxPackSpec;
      }
