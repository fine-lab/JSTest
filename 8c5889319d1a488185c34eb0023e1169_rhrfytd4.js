  ...
  execute(request) {
    ...
    for (let i = 0; i < udiSize; i++) {
      ...
      for (let i = 0; i < productPacking.length; i++) {
        let udiCreateConfig = productPacking[i].udiCreateConfig;
        if (udiCreateConfig == undefined || udiCreateConfig == null) {
          ...
          udiCreateConfig = ObjectStore.queryByYonQL(querySql);
        }
        if (udiCreateConfig != null && udiCreateConfig.length > 0) {
          productPacking[i].udiCreateConfig = udiCreateConfig;
          let productLogo = productPacking[i].productLogo;
          if (productLogo == undefined || productLogo == null) {
            ...
            productPacking[i].productLogo = productLogo;
          }
          UDIFileInfo = [{ UDI: udi, productIdentification: productLogo[0].bzcpbs, packageIdentification: productPacking[i].bzcpbs }];
          UDIFileInfo[0].packagingPhase = productPacking[i].cpbzjb;
          UDIFileInfo[0].identificationQty = productPacking[i].bznhxyjbzcpbssl;
          ...
          UDIFileInfo[0].udiConfigId = productPacking[i].id;
          ...
          for (let j = 0; j < udiCreateConfig.length; j++) {
            ...
            if (value.length != dataSize) {
              unbindingUdi.push({ udiCode: udi, errorMessage: "UDI不符合产品标识的生成规则配置方案！" });
