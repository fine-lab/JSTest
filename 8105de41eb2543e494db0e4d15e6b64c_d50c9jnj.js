...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    // 查询待计费的资产
    let sql =
      "select distinct id, code,name,sourcebillno,begintime useDate,location,fixedAssetsInfoCharacter.attrext3 sn,fixedAssetsInfoCharacter.attrext8 contractNum,fixedAssetsInfoCharacter.attrext8,fixedAssetsInfoCharacter.attrext6 requireNumber,toFixDetail.sourcebillno,toFixDetail.material.name materialName,toFixDetail.material.code materialCode, ";
    ...
    sql += " and fixedAssetsInfoCharacter.attrext8 <> null ";
    if (paramReq.sn) {
      sql += " and fixedAssetsInfoCharacter.attrext3= '" + paramReq.sn + "'";
    }
    ...
    // 从采购入库，查询到货时间(单据日期)
    let inSql = "select srcBill arriveId,prd.define1 arriveDate ";
    inSql += " from st.purinrecord.PurInRecord ";
    inSql += " left join st.purinrecord.PurInRecordDefine prd on prd.id = id ";
    inSql += " where srcBill in (" + arriveId + ") and prd.define1 is not null ";
    ...
    let queryPickUpSql = " select detail.act_time as shipmentDate, code from AT1707A99A16B00005.AT1707A99A16B00005.wlztxx ";
    queryPickUpSql += " left join AT1707A99A16B00005.AT1707A99A16B00005.wlztxxmx detail on detail.wlztxx_id = id ";
    queryPickUpSql += " where code in (" + sourceCode + ") and detail.status_name = '提货'";
    ...
    let querySignDateSql = " select detail.act_time as arriveDate, code from AT1707A99A16B00005.AT1707A99A16B00005.wlztxx ";
    querySignDateSql += " left join AT1707A99A16B00005.AT1707A99A16B00005.wlztxxmx detail on detail.wlztxx_id = id ";
    querySignDateSql += " where code in (" + sourceCode + ") and detail.status_name = '签收'";
