viewModel.on("customInit", function (data) {
  ...
  viewModel.on("afterLoadData", function (args) {
    ...
    if (verifystate == 2) {
      viewModel.get("button69ke").setVisible(true);
      viewModel.get("button44cg").setVisible(false);
    } else {
      if (verifystate == 0) {
        viewModel.get("button69ke").setVisible(false);
        viewModel.get("button44cg").setVisible(true);
      }
    }
  });
  viewModel.get("button44cg").on("click", function (data) {
    var ids = [];
  ...
  viewModel.get("button69ke").on("click", function (data) {
    var ids = [];
    var user = viewModel.getAppContext().user;
    var tenantid = viewModel.getAppContext().tenant.tenantId;
    var userid = user.userId;
  ...
  viewModel.on("afterLoadData", function (args) {
    for (let i = 0; i < gridModel.length; i++) {
      let batchno = gridModel[i].proBatchNo;
      change_price_detailList.setCellValue(i, "proBatchNo", batch_no);
    }
