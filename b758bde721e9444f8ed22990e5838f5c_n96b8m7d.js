...
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValueInput19pb() {},
  interactionRulecustomSetValuereferinput28() {}
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValueInput19pb: observable,
  interactionRulecustomSetValuereferinput28: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_formStore_Input19pb: reaction(
      () => [],
      async ([]) => {
        debounce(
          async () => {
            ...
            // 公式结束loading
            rootStore.formStore.setValue("Input19p", resultData);
          },
          "formula_customSetValue_formStore_Input19pb" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
    ...
    interactionRule_customSetValue_formStore_referinput28: reaction(
      () => [rootStore.formStore.getValue("sourceBusiObj")],
      async ([formStore_sourceBusiObj]) => {
        debounce(
          async () => {
            if (
              expressionParsing({
                leftValue: formStore_sourceBusiObj,
                operational: "eq",
                rightValue: "1",
                extLeftValue: {
                  bizType: "text"
              ...
              rootStore?.formStore.setValue("auditor", "d17d67cb-50a6-4631-a037-c77e27c0973f");
            } else {
              rootStore?.formStore.setValue("auditor", "ba79a24b-4bd4-464d-aa10-578111677946");
            }
          },
          "interactionRule_customSetValue_formStore_referinput28 " + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
