...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    try {
      ...
      initializationMap.set("ROE营业成本", ["主营业务收入", "其他业务支出"]);
      ...
      initializationMap.set("利润类其他收益", []);
      initializationMap.set("利润类费用期间费用", []);
      initializationMap.set("利润类费用税金", ["所得税"]);
      initializationMap.set("利润类费用其他损失", []);
      initializationMap.set("利润类费用其他", ["公允价值变动损益", "投资收益", "其它收益", "税金及附加", "资产减值损失"]);
      initializationMap.set("利润类营业外支出", ["营业外支出"]);
      initializationMap.set("利润类营业外收入", ["营业外收入"]);
      initializationMap.set("利润类营业成本", ["主营业务收入", "其他业务支出"]);
      initializationMap.set("利润类营业收入", ["主营业务收入", "其他业务收入"]);
      initializationMap.set("利润类税费负债类税费", []);
      initializationMap.set("利润类税费资产类税费", []);
