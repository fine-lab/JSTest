...
const computedAttrStore = {
  // 扩展get 方法
  formulacustomSetValueInput22ci() {},
  formulacustomSetValueInputNumber8db() {}
};
// 方法改成计算方法
const computedAttr = {
  formulacustomSetValueInput22ci: observable,
  formulacustomSetValueInputNumber8db: observable
};
// 设置监听
...
rootStore.eventBus.on(rootStore, "load", function () {
  rootStore.reactions.computedAttr = {
    formula_customSetValue_formStore_Input22ci: reaction(
      () => [rootStore.formStore.getValue("ReferInput1le")],
      async ([formStore_ReferInput1le]) => {
        debounce(
          async () => {
            ...
            const formulaResult = await countFormula(
              {
                hasFormula: "true",
                formula: "ReferInput1le.user.staff.mobile",
                formulaFuns: "",
                formulaTriggers: "ReferInput1le.user.staff.mobile"
              },
              {
            ...
            rootStore.formStore.setValue("Input22c", resultData);
          },
          "formula_customSetValue_formStore_Input22ci" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
    ...
    formula_customSetValue_formStore_InputNumber8db: reaction(
      () => [rootStore.formStore.getValue("DatePicker5p"), rootStore.formStore.getValue("DatePicker7o")],
      async ([formStore_DatePicker5p, formStore_DatePicker7o]) => {
        debounce(
          async () => {
            const allData = getAllDataByCardStore({
              cardStoreAlias: "cardStore",
              storeControlType: "FormStore",
              storeField: "InputNumber8d",
              storeForm: "",
              dataSource: "mainEntity"
            });
            const formulaResult = await countFormula(
              {
                hasFormula: "true",
                formula: "dateDiff(DatePicker5p,DatePicker7o,3)",
                formulaFuns: "dateDiff",
                formulaTriggers: "DatePicker5p,DatePicker7o"
              },
              {
                mainDataSourceAlias: "mainEntity",
                field: "InputNumber8d",
            ...
            //对公式结果进行处理
            const resultData = formulaResult.resultData;
            rootStore.formStore.setValue("InputNumber8d", resultData);
          },
          "formula_customSetValue_formStore_InputNumber8db" + "_" + tableStoreRowIndex + "_" + subTableStoreRowIndex,
          300
        )();
      },
      {
